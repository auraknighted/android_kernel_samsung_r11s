...
struct contexthub_ipc_info {
	...
	bool aon_flag;
#endif
	//struct notifier_block shub_ipc_nb;
...
static inline int contexthub_imgloader_init(struct contexthub_ipc_info *chub)
{
	...
	if (!chub->multi_os) {
#ifdef CONFIG_SENSORS_SSP
		struct device_node *node = chub->dev->of_node;
		const char *os_name = of_get_property(node, "os-name", NULL);

		nanohub_info("%s read os-name from dt fw name: %s\n", __func__, os_name);

		ret = contexthub_imgloader_desc_init(chub, &chub->chub_img_desc[i],
						     os_name != NULL ? os_name : OS_IMAGE_DEFAULT, i);
		ret = contexthub_imgloader_desc_init(chub, &chub->chub_img_desc[i+1],
						     "/sensorhub/shub_spu.bin", i+1);
#else
		ret = contexthub_imgloader_desc_init(chub, &chub->chub_img_desc[i],
						     OS_IMAGE_DEFAULT, i);
#endif
	} else {
		for (j = 0 ; i < MAX_FIRMWARE_NUM; i++, j++)
			ret |= contexthub_imgloader_desc_init(chub, &chub->chub_img_desc[i],
							      os_image[j], i);
	}
