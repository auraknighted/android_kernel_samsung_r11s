...
int stm_ts_check_dma_startanddone(struct stm_ts_data *ts)
{
	...
	ret = ts->stm_ts_spi_write(ts, &reg[0], 6, NULL, 0);
	if (ret < 0) {
	...
	do {
		reg[0] = 0xFB;
		ret = ts->stm_ts_spi_read(ts, &reg[0], 5, (u8 *)val, 1);
		if (ret < 0) {
...
static int stm_ts_check_erase_done(struct stm_ts_data *ts)
{
	...
	u8 reg[5] = { 0xFB, 0x20, 0x00, 0x00, 0x6A };
	u8 val[1];
	int ret;
...
int stm_ts_fw_fillflash(struct stm_ts_data *ts, u32 address, u8 *data, int size)
{
	...
	while (remaining > 0) {
		...
		while ((byteblock < DRAM_SIZE) && remaining > 0) {
			...
			rc = ts->stm_ts_spi_write(ts, &buff[0], index + towrite, NULL, 0);
			if (rc < 0) {
		...
		rc = ts->stm_ts_spi_write(ts, &buff2[0], index, NULL, 0);
		if (rc < 0) {
			input_err(true, &ts->client->dev,
					"%s failed to write i2c register. ret:%d\n",
					__func__, rc);
...
static void stm_ts_set_factory_history_data(struct stm_ts_data *ts, u8 level)
{
	...
	ret = ts->stm_ts_spi_write(ts, regaddr, 3, NULL, 0);
	if (ret < 0) {
		input_err(true, &ts->client->dev,
...
int stm_ts_execute_autotune(struct stm_ts_data *ts, bool issaving)
{
	...
	if (issaving == true) {
		...
		ts->tdata->tclm_write_spi(ts->tdata->spi, SEC_TCLM_NVM_ALL_DATA);
#endif
		if (rc < 0) {
...
EXPORT_SYMBOL(stm_ts_spi_fw_update_on_probe);
...
EXPORT_SYMBOL(stm_ts_spi_fw_update_on_hidden_menu);
MODULE_LICENSE("GPL");
