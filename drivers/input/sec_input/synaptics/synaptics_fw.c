...
//		break;
//	default:
//		input_err(true, ts->dev, "%s: Invalid flash area %d\n", __func__, area);
//		return -EINVAL;
//	}
...
//		input_err(true, ts->dev, "%s: Invalid data length %d\n", __func__,
		...
		unsigned int wr_len, unsigned int wr_delay_ms)
{
	...
	while (remaining_length) {
		...
		if (delay_ms == FORCE_ATTN_DRIVEN) {
			input_err(true, ts->dev, "%s: xfer: %d, delay: ATTN-driven\n", __func__, xfer_length);
		} else {
			delay_ms = (delay_ms * xfer_length) / 1000;
			//input_dbg(true, ts->dev, "%s: xfer: %d, delay: %d\n", __func__, xfer_length, delay_ms);
		}
		...
		struct block_data *block, unsigned int delay_ms)
{
	...
	input_err(true, ts->dev, "%s: Erase %s block - address: 0x%x, size: %d\n", __func__,
		AREA_ID_STR(block->id), flash_addr, size);

	if (size == 0) {
		input_err(true, ts->dev, "%s: No need to erase, size = %d\n", __func__, size);
		goto exit;
	}
...
exit:
	input_err(true, ts->dev, "%s: %s area erased\n", __func__, AREA_ID_STR(block->id));
		...
		unsigned int wait_delay_ms, bool force_reflash)
{
	...
	if (IS_NOT_APP_FW_MODE(ts->dev_mode)) {
		input_err(true, ts->dev, "%s: firmware is not in application mode\n", __func__);
		type = UPDATE_FIRMWARE_CONFIG;
		goto reflash;
	}
