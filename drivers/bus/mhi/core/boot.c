...
void mhi_fw_load_handler(struct mhi_controller *mhi_cntrl)
{
	...
	if (!mhi_cntrl->sbl_buf) {
		mhi_cntrl->sbl_buf = dma_alloc_coherent(mhi_cntrl->cntrl_dev,
			size, &mhi_cntrl->sbl_dma_addr, GFP_KERNEL);
		if (!mhi_cntrl->sbl_buf) {
	...
	ret = mhi_fw_load_bhi(mhi_cntrl, mhi_cntrl->sbl_dma_addr, size);
	...
	if (mhi_cntrl->fbc_download) {
		dma_set_mask(mhi_cntrl->cntrl_dev, DMA_BIT_MASK(32));
		...
		dma_set_mask(mhi_cntrl->cntrl_dev, DMA_BIT_MASK(36));
	}
