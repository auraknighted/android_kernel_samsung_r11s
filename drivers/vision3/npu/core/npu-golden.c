...
static void npu_golden_dma_sync(__attribute__((unused))struct addr_info *entry)
{
	return;
}
#else
static void npu_golden_dma_sync(struct addr_info *entry)
{
	__dma_map_area(entry->vaddr, entry->size, DMA_FROM_DEVICE);
}
#endif
...
static int __npu_golden_compare(const struct npu_frame *frame)
{
	...
	list_for_each_entry(golden_entry, &npu_golden_ctx.data.file_list, list) {
		...
		/* Cache invalidation */
		npu_golden_dma_sync(target_entry);
