...
static void __send_interrupt(u32 cmdType, struct command *cmd)
{
	...
	case COMMAND_CORE_CTL:
		...
		while (timeout && interface.sfr->grp[0].ms & 0x1) {
			timeout--;
			if (!(timeout % 100))
	...
	case COMMAND_DONE:
		/* Wait clr bit from FW */
		while (timeout && interface.sfr->grp[0].ms & 0x4) {
			timeout--;
			if (!(timeout % 100))
				npu_err("Mailbox %d delayed for %d\n", cmdType,
					MAILBOX_CLEAR_CHECK_TIMEOUT - timeout);
...
int npu_interface_open(struct npu_system *system)
{
	...
	ret = devm_request_irq(dev, system->irq1, mailbox_isr3, IRQF_TRIGGER_HIGH, "exynos-npu", NULL);
	if (ret) {
