...
static int npu_platform_get_irq(struct npu_system *system)
{
	...
	irq = platform_get_irq(system->pdev, 0);
	if (irq < 0) {
	...
	irq = platform_get_irq(system->pdev, 1);
	if (irq < 0) {
...
//	irq = platform_get_irq(system->pdev, 2);
//	if (irq < 0) {
...
int npu_system_resume(struct npu_system *system, u32 mode)
{
	...
	npu_info("reset FW working memory : paddr 0x%08llx, vaddr 0x%p, daddr 0x%08llx, size %lu\n",
