...
void *sdma_dump_addr;
void *gnpu0_dump_addr;
void *gnpu1_dump_addr;
EXPORT_SYMBOL(sdma_dump_addr);
EXPORT_SYMBOL(gnpu0_dump_addr);
EXPORT_SYMBOL(gnpu1_dump_addr);

#define NPU_DUMP_FW_SFR_SIZE	(0x20000)
...
int npu_dump_fw_sfr(struct npu_system *system)
{
	int ret = 0;
	void *sdma_vaddr, *gnpu0_vaddr, *gnpu1_vaddr;

	sdma_vaddr = (void *)npu_get_io_area(system, "sfrsdma")->vaddr;
	if (sdma_vaddr) {
		sdma_dump_addr = kzalloc(NPU_DUMP_FW_SFR_SIZE, GFP_ATOMIC);
		if (sdma_dump_addr) {
			memcpy_fromio(sdma_dump_addr, sdma_vaddr, NPU_DUMP_FW_SFR_SIZE);
			pr_err("NPU sdma dump - %pK / %d\n", sdma_dump_addr, NPU_DUMP_FW_SFR_SIZE);
		} else {
			pr_err("sdma_dump_addr is NULL\n");
			ret = -ENOMEM;
			goto exit_err;
		}

	}
	gnpu0_vaddr = (void *)npu_get_io_area(system, "sfrgnpu0")->vaddr;
	if (gnpu0_vaddr) {
		gnpu0_dump_addr = kzalloc(NPU_DUMP_FW_SFR_SIZE, GFP_ATOMIC);
		if (gnpu0_dump_addr) {
			memcpy_fromio(gnpu0_dump_addr, gnpu0_vaddr, NPU_DUMP_FW_SFR_SIZE);
			pr_err("NPU gnpu0 dump - %pK / %d\n", gnpu0_dump_addr, NPU_DUMP_FW_SFR_SIZE);
		} else {
			pr_err("gnpu0_dump_addr is NULL\n");
			ret = -ENOMEM;
			goto exit_err;
		}

	}
	gnpu1_vaddr = (void *)npu_get_io_area(system, "sfrgnpu1")->vaddr;
	if (gnpu1_vaddr) {
		gnpu1_dump_addr = kzalloc(NPU_DUMP_FW_SFR_SIZE, GFP_ATOMIC);
		if (gnpu1_dump_addr) {
			memcpy_fromio(gnpu1_dump_addr, gnpu1_vaddr, NPU_DUMP_FW_SFR_SIZE);
			pr_err("NPU gnpu1 dump - %pK / %d\n", gnpu1_dump_addr, NPU_DUMP_FW_SFR_SIZE);
		} else {
			pr_err("gnpu1_dump_addr is NULL\n");
			ret = -ENOMEM;
			goto exit_err;
		}
...
exit_err:
	if (sdma_dump_addr)
		kfree(sdma_dump_addr);
	if (gnpu0_dump_addr)
		kfree(gnpu0_dump_addr);
	if (gnpu1_dump_addr)
		kfree(gnpu1_dump_addr);
	return ret;
}
EXPORT_SYMBOL(npu_dump_fw_sfr);

int npu_util_dump_handle_error_k(struct npu_device *device)
{
	int ret = 0;
	ret = npu_dump_fw_sfr(&device->system);
	if (ret)
		pr_err("failed in npu_dump_fw_sfr\n");
