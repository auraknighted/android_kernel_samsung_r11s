...
int dsp_core_probe(struct dsp_core *core, void *dspdev)
{
	int ret;
	...
	core->miscdev.name = DSP_DEV_NAME;
	core->miscdev.fops = &dsp_file_ops;
	core->miscdev.parent = core->dspdev->dev;
	...
	ret = dsp_graph_manager_probe(&core->graph_manager, core);
	if (ret)
		goto p_err_graph;
