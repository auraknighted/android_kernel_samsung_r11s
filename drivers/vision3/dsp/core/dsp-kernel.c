...
static struct dsp_kernel_manager *dump_kmgr;
...
void dsp_kernel_dump(void)
{
	dsp_enter();
	if (dump_kmgr->dl_init)
		dsp_dl_print_status();
	dsp_leave();
}
...
static int __dsp_kernel_manager_dl_init(struct dsp_kernel_manager *kmgr)
{
	...
	ivp_pm_id = kmgr->gmgr->core->dspdev->system.memory.shared_id.ivp_pm;
	dl_out_id = kmgr->gmgr->core->dspdev->system.memory.shared_id.dl_out;
