...
/* dsim version */
#define DSIM_VER_EVT0			0x02080000
#define DSIM_VER_REF			0x02050000
 ...
 */
static void clear_freq_hop_pll(struct dsim_clks *clks, struct stdphy_pms *dphy_pms)
{
	if ((dphy_pms->freq_hop_m != dphy_pms->m) ||
			(dphy_pms->freq_hop_k != dphy_pms->k)) {

		dphy_pms->freq_hop_m = dphy_pms->m;
		dphy_pms->freq_hop_k = dphy_pms->k;
	}
}
#endif
...
u32 dsim_reg_get_dphy_status(u32 id)
{
	...
	val = dsim_read(id, DSIM_DPHY_STATUS);
	return val;
}
...
static void __dphy_dump(u32 id, struct dsim_regs *regs)
{
	...
	dpu_print_hex_dump(regs->phy_regs, regs->phy_regs + 0x0000, 0x88);
	/* MC */
	cal_log_info(id, "-[MC : offset + 0x100]-\n");
	dpu_print_hex_dump(regs->phy_regs, regs->phy_regs + 0x0200, 0x54);
