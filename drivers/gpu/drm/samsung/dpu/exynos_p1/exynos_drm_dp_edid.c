...
int dp_mode_compare_for_mirror(void *priv, struct list_head *lh_a, struct list_head *lh_b)
{
	...
	/*
	 * do not check which is preferred if both have the same resolution as the preferred
	 * because higher fps is better at this case
	 * but preferred is always better at Test SINK for CTS
	 */
	if (dp->prefer_found) {
		if (!dp_mode_resolution_match(&dp->pref_mode, a) || dp->test_sink) {
...
void dp_find_best_mode(struct dp_device *dp, int flag)
{
	...
	list_for_each_entry_safe(mode, t, &connector->probed_modes, head) {
		...
		if (flag & MODE_FILTER_PRINT) {
			dp_info(dp, "%s@%d%c (%d) stat(%d)\n", mode->name,
				drm_mode_vrefresh(mode),
				((mode->type & DRM_MODE_TYPE_PREFERRED) ? '*' : ' '),
				mode->clock, mode->status);
		}
	}

	if (!found) {
		dp_info(dp, "no best mode, fail-safe\n");
...
bool dp_find_prefer_mode(struct dp_device *dp, int flag)
{
	...
	list_for_each_entry_safe(mode, t, &connector->probed_modes, head) {
		if ((mode->type & DRM_MODE_TYPE_PREFERRED) &&
				(dp_is_better_mode(&dp->pref_mode, mode) > 0 || !dp->prefer_found)) {
			dp_info(dp, "pref: %s@%d, type: %d, stat: %d, ratio: %d, flag: %d\n",
				mode->name, drm_mode_vrefresh(mode), mode->type,
				mode->status, mode->picture_aspect_ratio, mode->flags);
	...
	if (dp->prefer_found) {
		dp_info(dp, "prefer: %s@%d found\n", dp->pref_mode.name,
				drm_mode_vrefresh(&dp->pref_mode));
	} else {
		dp_info(dp, "no prefer mode\n");
	}
