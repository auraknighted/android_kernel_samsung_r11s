		...
		const struct exynos_drm_writeback_state *state)
{
	...
	case DRM_FORMAT_NV12:
		...
		break;
	default:
		pr_debug("%s: format(%d) doesn't require BASE ADDR Calc.\n",
				...
				const struct exynos_drm_writeback_state *state)
{
	...
	default:
		config->src.w = fb->width;
		config->src.h = fb->height;
		pr_err("%s unsupported wb_type(%d)\n", __func__, state->type);
		break;
	}
				...
				struct drm_connector_state *conn_state)
{
	...
	if (ret) {
		pr_err("Invalid fb size %ux%u for %s mode size %ux%u\n",
				fb->width, fb->height,
		...
		struct exynos_drm_writeback_state *wb_state)
{
	...
	if (wb->rdb.repeater_buf_active == false) {
		...
		if (ret || (rdb->repeater_shared_buf.buffer_count > MAX_SHARED_BUF_NUM)) {
			pr_debug("%s: repeater buffer request failed\n", __func__);
			goto err_request_buf;
		}
		if (IS_ERR_OR_NULL(decon->dev)) {
			pr_err("%s: decon device connected to wb invalid\n", __func__);
			goto err_request_buf;
		}
		for (i = 0; i < MAX_SHARED_BUF_NUM; i++) {
			...
			if (IS_ERR_OR_NULL(rdb->attachment[i]) ||
				...
				!!!rdb->dma_addr[i]) {
				pr_err("%s: repeater buffer map failed\n", __func__);
				goto err_map_buf;
			}
		}
	...
	struct exynos_drm_writeback_state *wb_state)
{
	...
	if (wb->rdb.repeater_buf_active &&
		...
		if (repeater_get_valid_buffer(&wb->rdb.repeater_valid_buf_idx) < 0) {
			pr_err("%s:failed to get valid repeater buffer idx(%d)\n",
					__func__, wb->rdb.repeater_valid_buf_idx);
		...
		struct drm_connector_state *state)
{
	...
	DPU_EVENT_LOG(DPU_EVT_WB_ATOMIC_COMMIT, exynos_crtc, wb);
...
static irqreturn_t odma_irq_handler(int irq, void *priv)
{
	...
	if ((irqs & ODMA_WRITE_SLAVE_ERROR) || (irqs & ODMA_STATUS_DEADLOCK_IRQ)) {
		pr_err("wb(%d) error irq occur(0x%x)\n", wb->id, irqs);
		wb_dump(wb);
		goto irq_end;
	}
...
static int wb_init_resources(struct writeback_device *wb)
{
	...
	if (i >= 0) {
		...
		wb->win_config.votf_o_base_addr = res->start;
		wb->regs.votf_base_regs = of_iomap(np, i);
		if (!wb->regs.votf_base_regs) {
			pr_err("failed to remap VOTF SFR region\n");
			return -EINVAL;
		}
	...
	if (i >= 0) {
		...
		if (ret) {
			pr_err("failed to iommu_map MFC votf SFR region\n");
			return -EINVAL;
		}
		...
		ret = iommu_dma_reserve_iova(wb->dev, res->start,
				res->end - res->start + 1);
		if (ret) {
			pr_err("failed to dma_reserve MFC votf SFR region\n");
			return -EINVAL;
		}
	}
