...
static void exynos_hibernation_exit_handler(struct kthread_work *work)
{
	...
	pr_debug("Display hibernation exit handler is called\n");
...
exynos_hibernation_register(struct exynos_drm_crtc *exynos_crtc)
{
	...
	if (!IS_ENABLED(CONFIG_DRM_SAMSUNG_HIBERNATION_EARLY_WAKEUP) || (decon->id)) {
		pr_info("hibernation early wakeup is disabled\n");
		return hibernation;
	}
	...
	if (IS_ERR(hibernation->exit_thread)) {
		...
		pr_err("failed to run exit hibernation thread\n");
		return hibernation;
	}
	...
	if (ret) {
		pr_err("failed to create hiber exit file\n");
		return hibernation;
	}
	hibernation->early_wakeup_enable = true;

	pr_info("hibernation early wakeup is enabled\n");
