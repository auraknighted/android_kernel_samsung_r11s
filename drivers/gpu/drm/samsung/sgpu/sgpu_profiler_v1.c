	...
	if (pinfo.cpufreq_pm_qos_added  == 0) {
		...
		if (policy != NULL)
			freq_qos_tracer_add_request(&policy->constraints,
					&sgpu_profiler_cpu_min_qos[PROFILER_CL0],
					FREQ_QOS_MIN, FREQ_QOS_MIN_DEFAULT_VALUE);

		policy = cpufreq_cpu_get(MID_BASE_ID);
		if (policy != NULL)
			freq_qos_tracer_add_request(&policy->constraints,
					&sgpu_profiler_cpu_min_qos[PROFILER_CL1],
					FREQ_QOS_MIN, FREQ_QOS_MIN_DEFAULT_VALUE);
		
		policy = cpufreq_cpu_get(BIG_BASE_ID);
		if (policy != NULL)
			freq_qos_tracer_add_request(&policy->constraints,
					&sgpu_profiler_cpu_min_qos[PROFILER_CL2],
					FREQ_QOS_MIN, FREQ_QOS_MIN_DEFAULT_VALUE);
	}
...
static void profiler_pbooster(struct profiler_interframe_data *dst)
{
	...
	if (profiler_get_is_profiler_governor(df) && rtp.target_frametime > 0) {
		...
		for (id = PROFILER_CL0; id < NUM_OF_CPU_DOMAIN; id++) {
