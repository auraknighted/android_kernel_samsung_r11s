...
typedef union {
	...
	struct {
		...
		/* bit[9] Enable of override value for phy_enable.  This phy_enable feature is usefull only if phy_enable pin is present 
		 *  at phy top through IPNAME_PHY_ENABLE_SUPPORT macro. User should consider this 
		 *  register override as redundant if the feature is not enabled. */
...
typedef union {
	...
	struct {
		...
		/* bit[5] Value from ASIC for phy_enable This phy_enable feature is usefull only if phy_enable pin is present 
		 *  at phy top through IPNAME_PHY_ENABLE_SUPPORT macro. User should consider this 
		 *  register override as redundant if the feature is not enabled */
...
typedef union {
	...
	struct {
		...
		/* bit[3] diconnect timer enable for fast mode (simulation only) */
...
typedef union {
	u16 data;
	struct {
		/* bit[7:0] packet on extention time during receiving resume */
		unsigned resume_pkt_on_ext_time:8;
		/* bit[15:8] packet on extention time during transmitting wakeup */
