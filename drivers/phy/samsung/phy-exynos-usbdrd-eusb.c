...
void __iomem *dpphy_ctrl_addr;
EXPORT_SYMBOL_GPL(dpphy_ctrl_addr);
void __iomem *dpphy_tca_addr;
EXPORT_SYMBOL_GPL(dpphy_tca_addr);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	for (i = 0; i < tune_num; i++) {
		if (!strncmp(phy_drd->usbphy_sub_info.tune_param[i].name, tune_name,
			...
			phy_exynos_snps_usbdp_tune_each_cr(&phy_drd->usbphy_sub_info,
							tune_name, tune_val);
		}
	}
...
static ssize_t
exynos_usbdrd_phy_port_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct exynos_usbdrd_phy *phy_drd = dev_get_drvdata(dev);
	int len = 0;

	len += snprintf(buf + len, PAGE_SIZE, "Used phy port : %d\n",
			phy_drd->usbphy_sub_info.used_phy_port);

	return len;
}

static ssize_t
exynos_usbdrd_phy_port_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t n)
{
	struct exynos_usbdrd_phy *phy_drd = dev_get_drvdata(dev);
	int phy_port_value, ret;

	ret = kstrtouint(buf, 0, &phy_port_value);
	if (ret)
		return ret;

	phy_drd->phy_port_test_en = true;
	phy_drd->usbphy_info.used_phy_port = phy_port_value;
	phy_drd->usbphy_sub_info.used_phy_port = phy_port_value;

	return n;
}
static DEVICE_ATTR_RW(exynos_usbdrd_phy_port);

static struct attribute *exynos_usbdrd_attrs[] = {
	&dev_attr_exynos_usbdrd_hs_phy_tune.attr,
	&dev_attr_exynos_usbdrd_phy_tune.attr,
	&dev_attr_exynos_usbdrd_phy_port.attr,
	NULL
};
...
static int exynos_usbdrd_get_sub_phyinfo(struct exynos_usbdrd_phy *phy_drd)
{
	...
	usbdp_combo_phy_reg = phy_drd->usbphy_sub_info.ctrl_base;
...
#if IS_ENABLED(CONFIG_CHECK_CTYPE_SIDE)
	if (!phy_drd->phy_port_test_en) {
		/* USE external function to check typec side */
		ret = value = usbpd_manager_get_side_check();
		if (value == 1)
...
static void exynos_usbdrd_utmi_init(struct exynos_usbdrd_phy *phy_drd)
{
	...
	/* WA: Below function should not be called for super-speed
	 * and It doesn't seem to effect high-speed */
...
void exynos_usbdrd_ldo_control(struct exynos_usbdrd_phy *phy_drd, int on)
{
	...
	if (phy_drd->vdd075_usb == NULL || phy_drd->vdd12_usb == NULL ||
			phy_drd->vdd18_usb == NULL || phy_drd->vdd33_usb == NULL) {
	...
	if (on) {
		...
		ret2 = regulator_enable(phy_drd->vdd12_usb);
		ret3 = regulator_enable(phy_drd->vdd18_usb);
		ret4 = regulator_enable(phy_drd->vdd33_usb);
		if (ret1 || ret2 || ret3 || ret4) {
			dev_err(phy_drd->dev, "Failed to enable USB LDOs: %d %d %d %d\n",
					ret1, ret2, ret3, ret4);
		}
		phy_drd->is_ldo_on = 1;
	} else {
		...
		ret2 = regulator_disable(phy_drd->vdd12_usb);
		ret3 = regulator_disable(phy_drd->vdd18_usb);
		ret4 = regulator_disable(phy_drd->vdd33_usb);
		if (ret1 || ret2 || ret3 || ret4) {
			dev_err(phy_drd->dev, "Failed to disable USB LDOs: %d %d %d %d\n",
					ret1, ret2, ret3, ret4);
		}
		phy_drd->is_ldo_on = 0;
	}
...
/*
 * This function is test purpose only.
 * Do not call this in normal operation.
 * If this is executed in normal operation,
 * USB3.0 will work only in one direction.
 */
int exynos_usbdrd_phy_port_external_control(bool used_phy_port)
{
	struct exynos_usbdrd_phy *phy_drd;
	...
	pr_info("%s phy port current = %d, new = %d\n", __func__,
		phy_drd->usbphy_info.used_phy_port, used_phy_port);

	phy_drd->phy_port_test_en = true;
	phy_drd->usbphy_info.used_phy_port = used_phy_port;
	phy_drd->usbphy_sub_info.used_phy_port = used_phy_port;

	return 0;
}
EXPORT_SYMBOL_GPL(exynos_usbdrd_phy_port_external_control);
...
skip_clock:
	...
	phy_drd->phy_port_test_en = false;
	...
	if (!of_property_read_u32(dev->of_node, "has_combo_phy", &ret)) {
		if (ret) {
			...
			dpphy_ctrl_addr = phy_drd->reg_dpphy_ctrl;
			...
			dpphy_tca_addr = phy_drd->reg_dpphy_tca;
	...
	phy_drd->vdd12_usb = regulator_get(dev, "vdd12_usb");
	if (IS_ERR(phy_drd->vdd12_usb) || phy_drd->vdd12_usb == NULL) {
		dev_err(dev, "%s - vdd12_usb regulator_get fail %p %d\n",
			__func__, phy_drd->vdd12_usb, IS_ERR(phy_drd->vdd12_usb));
	}
