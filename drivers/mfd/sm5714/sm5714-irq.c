...
static const inline struct sm5714_irq_data *irq_to_sm5714_irq(struct sm5714_dev *sm5714, int irq)
{
...
static irqreturn_t sm5714_irq_thread(int irq, void *data)
{
	...
	ret = wait_event_timeout(sm5714->suspend_wait, !sm5714->suspended, msecs_to_jiffies(200));
	if (!ret) {
...
int sm5714_irq_init(struct sm5714_dev *sm5714)
{
	...
	pr_err("%s:%s irq=%d, irq->gpio=%d\n", MFD_DEV_NAME, __func__, sm5714->irq, sm5714->irq_gpio);
