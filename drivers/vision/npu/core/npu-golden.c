...
#if (NCP_VERSION == 25)
	npu_info("No check for model name\n");
	npu_golden_ctx.data.flags.net_id_matching = 0;
#else
...
#if (NCP_VERSION == 25)
		if (strcmp(ncp_header->model_name, npu_golden_ctx.data.match.model_name)) {
			npu_ufdbg("net ID [fFrame:%s != golden:%s] mismatch. skipping.\n",
				frame, ncp_header->model_name, npu_golden_ctx.data.match.model_name);
			return 0;
		}
#else
