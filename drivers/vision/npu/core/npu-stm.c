...
#ifdef CONFIG_NPU_STM_NEED_BUSP_DIV
...
#else
	return __enable_npu_stm_sfr(system, 0xCAFE, hid);
#endif
}
...
static void npu_stm_set_stm_sel(struct npu_system *system, u32 stm_sel)
{
	...
	npu_write_hw_reg(npu_get_io_area(system, "sfrdnc"), 0x20018, (stm_sel << 4), 0x000001F0, 0);
}
#else
static void npu_stm_set_stm_sel(___attribute__((unused))struct npu_system *system,
							attribute__((unused))u32 stm_sel)
{
	return;
}
#endif
...
static void npu_stm_set_stm_sel(struct npu_system *system, u32 stm_sel)
{
	npu_write_hw_reg(npu_get_io_area(system, "sfrnpus"), 0x0078, stm_sel, 0x0000007F, 0);
}
#endif
...
{
	return npu_read_hw_reg(npu_stm_ts_stat.sfr_mct_g, 0x0, 0xFFFFFFFF, 0);
}
...
enable_more:
	...
	npu_info("complete %s, cnt (%d)\n", __func__, npu_stm_data.enable_cnt);
