...
#ifdef CONFIG_NPU_SECURE_MODE
	if (npu_device->system.saved_warm_boot_flag) {
		/* If warm boot flag is set, protodrv is kept active
		 * even though FW is in suspend state
		 */
		if (get_state() == PROTO_DRV_STATE_OPENED)
			return 0;
	}
#endif
...
{
#ifdef CONFIG_NPU_SECURE_MODE
	if (npu_device->system.mbox_hdr->warm_boot_enable) {
		/* If warm boot flag is set, protodrv is kept active
		 * even though FW is in suspend state
		 */
		if (get_state() == PROTO_DRV_STATE_OPENED)
