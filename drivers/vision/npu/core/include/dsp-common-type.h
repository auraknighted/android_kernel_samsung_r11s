...
enum dsp_common_param_type {
	/* target specific graph data; core fata for DSP firmware */
	DSP_COMMON_MEM_GRAPH_BIN = 0,
	DSP_COMMON_MEM_CMDQ_BIN,  // CMDQ binary
	DSP_COMMON_MEM_KERNEL_BIN_STR,  // kernel binary string
	DSP_COMMON_MEM_IFM,  // input
	DSP_COMMON_MEM_OFM,  // output
	DSP_COMMON_MEM_IMB,  // memory for dsp intermediate buffer
	DSP_COMMON_MEM_WGT,  // weight
	DSP_COMMON_MEM_BIAS,  // bias
	DSP_COMMON_MEM_SCALAR,  // scalar
	DSP_COMMON_MEM_CUSTOM,  // custom type, meaning only by load type
	DSP_COMMON_MEM_NPU_IM,  // npu intermediate
	DSP_COMMON_MEM_KERNEL_BIN,  // kernel.elf
	DSP_COMMON_MEM_KERNEL_DESC,  // gkt.xml
	DSP_COMMON_MEM_EMPTY = 0x7000,
};
...
struct dsp_common_param_v4 {
	unsigned int			param_type;
	union {
		unsigned int		param_index;
		unsigned int		kernel_id;
	};
	struct dsp_common_mem_v4	param_mem;
};
...
struct dsp_common_graph_info_v4 {
	unsigned int			global_id;
	unsigned int			n_tsgd;
	unsigned int			n_param;
	unsigned int			n_kernel;
	struct dsp_common_param_v4	param_list[0];
};
...
struct dsp_common_execute_info_v4 {
	unsigned int			global_id;
	unsigned int			n_update_param;
	struct dsp_common_param_v4	param_list[0];
};
