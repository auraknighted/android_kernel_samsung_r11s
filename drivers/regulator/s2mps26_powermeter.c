...
#define POWER_B2		(9155300)
#define POWER_B3		(18311000)
#define POWER_B4		(18311000)
#define POWER_B5		(27465800)
#define POWER_B6		(27465800)
#define POWER_B7		(27465800)
#define POWER_B8		(9155300)
#define POWER_B9		(9155300)
#define POWER_B10		(27465800)
#define POWER_B11		(27465800)
#define POWER_B12		(27465800)
...
static const uint64_t power_coeffs[] =
	{POWER_B1, POWER_B2, POWER_B3, POWER_B4, POWER_B5, POWER_B6,
	 POWER_B7, POWER_B8, POWER_B9, POWER_B10, POWER_B11, POWER_B12};
...
static uint64_t adc_get_count(struct adc_info *adc_meter)
{
	...
	ret = s2mps26_read_reg(i2c, ACC_COUNT_L, &acc_count_l);
	if (ret)
		pr_err("%s: failed to read register\n", __func__);

	ret = s2mps26_read_reg(i2c, ACC_COUNT_H, &acc_count_h);
	if (ret)
		pr_err("%s: failed to read register\n", __func__);
...
static uint64_t adc_get_acc(struct adc_info *adc_meter, int chan)
{
	...
	for (i = 0; i < MAX_ADC_OUTPUT; i++) {
		ret = s2mps26_read_reg(i2c, adc_channel_reg[chan] + i, adc_acc + i);
		if (ret) {
...
static int adc_check_async(struct adc_info *adc_meter)
{
	...
	ret = s2mps26_update_reg(i2c, ADC_CTRL0, ASYNC_RD, ASYNC_RD);
	if (ret)
		goto err;

	usleep_range(2000, 2100);
	ret = s2mps26_read_reg(i2c, ADC_CTRL0, &val);
	if (ret)
		goto err;
			   ...
			   struct device_attribute *attr, char *buf)
{
	...
	ret = s2mps26_read_reg(adc_meter->i2c, ADC_CTRL1, &val);
	if (ret)
		pr_err("%s: failed to read register\n", __func__);
			    ...
			    const char *buf, size_t count)
{
	...
	ret = s2mps26_update_reg(adc_meter->i2c, ADC_CTRL1, val, ADCEN_MASK);
	if (ret)
...
static int s2mps26_create_powermeter_sysfs(struct s2mps26_dev *s2mps26)
{
	struct device *s2mps26_adc_dev;
	...
	s2mps26_adc_dev = pmic_device_create(s2mps26->adc_meter, device_name);
	s2mps26->powermeter_dev = s2mps26_adc_dev;

	/* Create sysfs entries */
	for (i = 0; i < ARRAY_SIZE(powermeter_attr); i++) {
		err = device_create_file(s2mps26_adc_dev, &powermeter_attr[i].dev_attr);
		if (err)
			goto remove_pmic_device;
	}

#if IS_ENABLED(CONFIG_SEC_PM)
	if (!IS_ERR_OR_NULL(s2mps26->ap_sub_pmic_dev)) {
		err = sysfs_create_link(&s2mps26->ap_sub_pmic_dev->kobj,
				&s2mps26_adc_dev->kobj, "power_meter");
		if (err)
...
remove_pmic_device:
	for (i--; i >= 0; i--)
		device_remove_file(s2mps26_adc_dev, &powermeter_attr[i].dev_attr);
	pmic_device_destroy(s2mps26_adc_dev->devt);

	return -1;
}
#endif

int s2mps26_adc_set_enable(struct adc_info *adc_meter, int en)
{
	int ret = 0;
	uint8_t val = 0;
...
EXPORT_SYMBOL_GPL(s2mps26_adc_set_enable);
...
static int adc_assign_mux_channel(struct adc_info *adc_meter)
{
	...
	for (i = MUX0SEL; i <= MUX5SEL; i++) {
		...
		ret = s2mps26_write_reg(adc_meter->i2c, i, mux_val);
		if (ret) {
...
static int adc_set_channel(struct adc_info *adc_meter)
{
	...
	adc_meter->adc_reg[ADC_CH2] = ADC_B3;
	adc_meter->adc_reg[ADC_CH3] = ADC_B5;
	adc_meter->adc_reg[ADC_CH4] = ADC_B8;
	adc_meter->adc_reg[ADC_CH5] = ADC_B9;
	adc_meter->adc_reg[ADC_CH6] = ADC_B10;
	adc_meter->adc_reg[ADC_CH7] = ADC_B10;
...
static int adc_init_hw(struct adc_info *adc_meter)
{
	...
	ret = s2mps26_write_reg(adc_meter->i2c, ADC_CTRL3, 0x7F);
	if (ret)
		goto err;

	/* EVT1 Power-meter sampling delay option */
	if (adc_meter->pmic_rev) {
		ret = s2mps26_update_reg(adc_meter->i2c, ADC_CTRL0, 0x0C, 0x0C);
		if (ret)
			goto err;
	}
...
int s2mps26_adc_init(struct adc_info *adc_meter)
{
	int ret = 0;

	/* ADC configuration */
	ret = s2mps26_write_reg(adc_meter->i2c, ADC_CONFIG, 0x0B);
	if (ret)
		goto err;
	/* PM period, 1.038ms, minimum is '1 */
	ret = s2mps26_write_reg(adc_meter->i2c, PERIOD_CTRL0, 0x11);
	if (ret)
		goto err;
...
void s2mps26_powermeter_init(struct s2mps26_dev *s2mps26)
{
	...
	adc_meter = devm_kzalloc(s2mps26->dev, size, GFP_KERNEL);
	if (!adc_meter) {
	...
	adc_meter->power_val = devm_kzalloc(s2mps26->dev, size, GFP_KERNEL);

	size = sizeof(uint8_t) * ARRAY_SIZE(adc_channel_reg);
	adc_meter->adc_reg = devm_kzalloc(s2mps26->dev, size, GFP_KERNEL);

	adc_meter->pmic_rev = s2mps26->pmic_rev;
	adc_meter->i2c = s2mps26->adc_i2c;
	mutex_init(&adc_meter->adc_lock);
	s2mps26->adc_meter = adc_meter;
	...
	if (s2mps26_adc_init(adc_meter) < 0) {
		pr_err("%s: s2mps26_adc_init fail\n", __func__);
		return;
	}

#if IS_ENABLED(CONFIG_DRV_SAMSUNG_PMIC)
	if (s2mps26_create_powermeter_sysfs(s2mps26) < 0) {
...
EXPORT_SYMBOL_GPL(s2mps26_powermeter_init);

void s2mps26_powermeter_deinit(struct s2mps26_dev *s2mps26)
{
#if IS_ENABLED(CONFIG_DRV_SAMSUNG_PMIC)
	struct device *s2mps26_adc_dev = s2mps26->powermeter_dev;
	int i = 0;
...
#endif
	/* ADC turned off */
	s2mps26_adc_set_enable(s2mps26->adc_meter, ADC_DISABLE);
}
EXPORT_SYMBOL(s2mps26_powermeter_deinit);
