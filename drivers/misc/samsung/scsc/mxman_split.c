...
static bool mxman_wlan_subsystem_active(struct mxman *mxman)
{
...
static bool mxman_wpan_subsystem_active(struct mxman *mxman)
{
	switch (mxman->mxman_state) {
...
static void mxman_pmu_message_handler(void *data, u32 cmd)
{
	...
	case MIFPMU_ERROR_WPAN:
		mxman_fail(mxman, SCSC_PANIC_CODE_FW << 15, __func__);
		break;
	default:
...
static void process_panic_record(struct mxman *mxman, bool dump)
{
	...
	*/
	if ((mxman->scsc_panic_code & SCSC_PANIC_ORIGIN_MASK) == SCSC_PANIC_ORIGIN_FW) {
		...
		if (fw_if->get_panic_record_offset(fw_if, SCSC_MIF_ABS_TARGET_WPAN)) {
			m4_panic_record =
				(u32 *)(mxman->fw + fw_if->get_panic_record_offset(fw_if, SCSC_MIF_ABS_TARGET_WPAN));
			m4_panic_record_ok = fw_parse_m4_panic_record(m4_panic_record, &m4_panic_record_length, dump);
#ifdef CONFIG_SCSC_MX450_GDB_SUPPORT
		} else if (fw_if->get_panic_record_offset(fw_if, SCSC_MIF_ABS_TARGET_FXM_1)) {
			m4_1_panic_record =
				(u32 *)(mxman->fw + fw_if->get_panic_record_offset(fw_if, SCSC_MIF_ABS_TARGET_WPAN));
...
static void mxman_failure_work(struct work_struct *work)
{
	 ...
	 */
#ifdef CONFIG_SCSC_INDEPENDENT_SUBSYSTEM
	if (mxman_wlan_subsystem_active(mxman)) {
#endif
	...
	if (mxman_wpan_subsystem_active(mxman)) {
	...
	if (mxman->mxman_state != MXMAN_STATE_FAILED && mxman->mxman_state != MXMAN_STATE_FROZEN) {
		WARN_ON(mxman->mxman_state != MXMAN_STATE_FAILED && mxman->mxman_state != MXMAN_STATE_FROZEN);
...
void mxman_close(struct mxman *mxman, enum scsc_subsystem sub)
{
	...
	SCSC_TAG_INFO(MXMAN, "Mxman stop current state %d users_wlan=%d users_wpan=%d\n", mxman->mxman_state, mxman->users, mxman->users_wpan);
	switch (mxman->mxman_state) {
	case MXMAN_STATE_STARTED_WLAN:
		if (sub == SCSC_SUBSYSTEM_WPAN) {
...
#if IS_ENABLED(CONFIG_SCSC_MXLOGGER)
			mxlogger_generate_sync_record(scsc_mx_get_mxlogger_wpan(mxman->mx), MXLOGGER_SYN_SUSPEND);
#endif
			mxmgmt_transport_send(scsc_mx_get_mxmgmt_transport_wpan(mxman->mx), MMTRANS_CHAN_ID_MAXWELL_MANAGEMENT,
				&message, sizeof(message));
		}
...
#if IS_ENABLED(CONFIG_SCSC_MXLOGGER)
			mxlogger_generate_sync_record(scsc_mx_get_mxlogger_wpan(mxman->mx), MXLOGGER_SYN_RESUME);
#endif
