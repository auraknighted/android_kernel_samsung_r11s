...
static void platform_mif_dump_register(struct scsc_mif_abs *interface)
{
	...
	regmap_read(platform->boot_cfg, 0, &val);
	SCSC_TAG_INFO_DEV(PLAT_MIF, platform->dev, "BOOT_SOURCE 0x%08x\n", val);
	regmap_read(platform->boot_cfg, 4, &val);
	SCSC_TAG_INFO_DEV(PLAT_MIF, platform->dev, "BOOT_CFG_ACK 0x%08x\n", val);
	spin_unlock_irqrestore(&platform->mif_spinlock, flags);
}
