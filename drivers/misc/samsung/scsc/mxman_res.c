...
#if IS_ENABLED(CONFIG_SCSC_MEMLOG)
static int mxman_res_is_memlog_valid(void)
{
...
struct memlog_obj *mxman_res_get_memlog_obj(struct scsc_mif_abs *mif, const char *desc_name)
{
...
int mxman_res_mappings_logger_init(struct mxman *mxman, void *start_dram)
{
	...
	obj = mxman_res_get_memlog_obj(mif, desc_name);
	if (!obj) {
	...
	mif->get_mifram_ref(mif, start_dram, &mifram_ref);
	SCSC_TAG_ERR(MXMAN, "Start dram 0x%x\n", mifram_ref);
	mif->get_mifram_ref(mif, start_dram_section2, &mifram_ref);
	SCSC_TAG_ERR(MXMAN, "Start section 2 0x%x\n", mifram_ref);
#endif
	...
	start_dram_section2 = (char *)start_dram + MX_DRAM_SIZE_SECTION_LOG + MX_DRAM_SIZE_SECTION_WLAN;
	/* Initialize allocator for mem region for logging WPAN */
	miframman_init(scsc_mx_get_ramman2_wpan(mxman->mx), start_dram_section2, MX_DRAM_SIZE_SECTION_WPAN,
		       start_dram_section2);
