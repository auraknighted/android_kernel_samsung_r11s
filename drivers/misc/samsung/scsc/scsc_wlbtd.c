...
int chipset_logging_send_msg_to_netlink(const char *file_content, int length, u8 flags)
{
	...
	SCSC_TAG_ERR(WLBTD, "Message sent successfully\n", rc);
	return 0;
...
int wlbtd_chipset_logging(const char *file_content, size_t bytes)
{
	unsigned long completion_jiffies = 0;
	...
	SCSC_TAG_DEBUG(WLBTD, "start wlbtd chipset logging, bytes = %d\n", bytes);
