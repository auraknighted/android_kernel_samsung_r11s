...
#if defined(CONFIG_SCSC_INDEPENDENT_SUBSYSTEM)
	if (mxman_if_get_state(mxman) == MXMAN_STATE_FAILED)
		return 0;
#else
...
#if defined(CONFIG_SCSC_INDEPENDENT_SUBSYSTEM)
	mxman_if_fail(scsc_mx_get_mxman(mx), host_panic_code, reason);
#else
...
#if defined(CONFIG_SCSC_INDEPENDENT_SUBSYSTEM)
	return mxman_if_force_panic(mxman);
#else
...
{
	return MXLOGGER_IMP_SIZE;
}
EXPORT_SYMBOL(slsi_get_imp_buff_size);
...
size_t scsc_log_collect_buffer(struct scsc_service *service, enum scsc_log_chunk_type fw_buffer, void *buffer, size_t size)
{
	...
	bytes = mxlogger_dump_fw_buf(scsc_mx_get_mxlogger(mx), fw_buffer, buffer, size, SCSC_MIF_ABS_TARGET_WLAN);
...
EXPORT_SYMBOL(scsc_log_collect_buffer);
#endif
