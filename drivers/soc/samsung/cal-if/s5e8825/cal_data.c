...
#include "pmucal/flexpmu_cal_cpu_s5e8825.h"
#include "pmucal/flexpmu_cal_local_s5e8825.h"
#include "pmucal/flexpmu_cal_p2vmap_s5e8825.h"
#include "pmucal/flexpmu_cal_system_s5e8825.h"
#include "pmucal/flexpmu_cal_define_s5e8825.h"

#if IS_ENABLED(CONFIG_CP_PMUCAL)
#include "../pmucal/include/pmucal_cp.h"
#include "pmucal/pmucal_cp_s5e8825.h"
#endif

#if IS_ENABLED(CONFIG_GNSS_PMUCAL)
#include "../pmucal/include/pmucal_gnss.h"
#include "pmucal/pmucal_gnss_s5e8825.h"
#endif

#if IS_ENABLED(CONFIG_CHUB_PMUCAL)
#include "../pmucal/include/pmucal_chub.h"
#include "pmucal/pmucal_chub_s5e8825.h"
#endif
...
#include "cmucal/clkout_s5e8825.c"
#include "acpm_dvfs_s5e8825.h"
#include "asv_s5e8825.h"
#include "../ra.h"
#include <linux/smc.h>
...
#define S5E8825_CMU_ALIVE_BASE	(0x11800000)
...
unsigned int s5e8825_frac_rpll_list[] = {
	PLL_AUD,
	PLL_MMC,
};
...
void s5e8825_cal_data_init(void)
{
	int i;
	...
	cmu_top = ioremap(S5E8825_CMU_TOP_BASE, SZ_4K);
	if (!cmu_top)
		pr_err("%s: cmu_top ioremap failed\n", __func__);

	cmu_bus0 = ioremap(S5E8825_CMU_BUS0_BASE, SZ_16K);
	if (!cmu_bus0)
		pr_err("%s: cmu_bus0 ioremap failed\n", __func__);

	cmu_alive = ioremap(S5E8825_CMU_ALIVE_BASE, SZ_8K);
	if (!cmu_alive)
		pr_err("%s: cmu_alive ioremap failed\n", __func__);

//	ccmu_cpucl2 = ioremap(S5E8825_CCMU_CPUCL2_BASE, SZ_4K);
//	if (!ccmu_cpucl2)
//		pr_err("%s: ccmu_cpucl2 ioremap failed\n", __func__);

	frac_rpll_size = ARRAY_SIZE(s5e8825_frac_rpll_list);
	for (i = 0; i < frac_rpll_size; i++)
		frac_rpll_list[i] = s5e8825_frac_rpll_list[i];

//	cmu_adc_rco_400_contorl();
}

void (*cal_data_init)(void) = s5e8825_cal_data_init;
...
/*
static void __s5e8825_set_cmuewf(unsigned int index, unsigned int en, void *cmu_cmu)
{
	unsigned int reg;
...
int s5e8825_set_cmuewf(unsigned int index, unsigned int en, void *cmu_cmu, int *ewf_refcnt)
{
	unsigned int reg;
	int ret = 0;
	int tmp;
	...
	if (en) {
		__s5e8825_set_cmuewf(index, en, cmu_cmu);
...
int (*wa_set_cmuewf)(unsigned int index, unsigned int en, void *cmu_cmu, int *ewf_refcnt) = s5e8825_set_cmuewf;
*/

void s5e8825_set_cmu_smpl_warn(void)
{
}
void (*cal_set_cmu_smpl_warn)(void) = s5e8825_set_cmu_smpl_warn;
...
/*
char *s5e8825_get_pd_name_by_cmu(unsigned int addr)
{
	int i, map_size;
...
char *(*cal_get_pd_name_by_cmu)(unsigned int addr) = s5e8825_get_pd_name_by_cmu;
*/
