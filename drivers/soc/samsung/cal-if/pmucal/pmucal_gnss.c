...
int pmucal_is_gnss_regs(int reg)
{
	...
	for (i = 0; i < pmucal_gnss_list.num_reset_assert; i++) {
		if (reg == pmucal_gnss_list.reset_assert[i].base_pa +
				pmucal_gnss_list.reset_assert[i].offset) {
			is_gnss_regs = 1;
			goto out;
		}
	}
	...
	for (i = 0; i < pmucal_gnss_list.num_reset_release; i++) {
		if (reg == pmucal_gnss_list.reset_release[i].base_pa +
				pmucal_gnss_list.reset_release[i].offset) {
			is_gnss_regs = 1;
			goto out;
		}
	}
	...
	for (i = 0; i < pmucal_gnss_list.num_gnss_reset_req_clear; i++) {
		if (reg == pmucal_gnss_list.gnss_reset_req_clear[i].base_pa +
				pmucal_gnss_list.gnss_reset_req_clear[i].offset) {
			is_gnss_regs = 1;
			goto out;
		}
	}
	...
	for (i = 0; i < pmucal_gnss_list.num_active_clear; i++) {
		if (reg == pmucal_gnss_list.active_clear[i].base_pa +
				pmucal_gnss_list.active_clear[i].offset) {
			is_gnss_regs = 1;
			goto out;
		}
	}
