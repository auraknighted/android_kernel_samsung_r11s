			      ...
			      struct sk_buff *skb)
{
	...
	if (skb_queue_empty(multi_q)) {
		...
		/* P220725-00270
		 * The first packet MUST not be len=0, or the last one.
		 * Maybe it was dropped.
		 * Therefore, the others MUST be dropped here.
		 */
		if ((ctrl.more && fh->len == 0) || !ctrl.more) {
			mif_err("drop this packet (more:%d ID:%d len:%d)\n",
					ctrl.more, ctrl.id, fh->len);
			return -1;
		}
	}
