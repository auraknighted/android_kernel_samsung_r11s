...
static int pktproc_send_iov_to_cp(struct pktproc_queue_ul *q, struct mif_iov *iov)
{
	struct pktproc_q_info_ul *q_info = q->q_info;
	struct pktproc_desc_ul *desc;
	void *target_addr;
	int len = iov->iov.iov_len;
	bool use_dit;
	u32 space;
	...
	if (!use_dit) {
		...
		__copy_from_user_inatomic(target_addr, iov->iov.iov_base, len);
	}
...
#if IS_ENABLED(CONFIG_CPIF_USERSPACE_NETWORK)
		q->send_iov = pktproc_send_iov_to_cp;
#endif
