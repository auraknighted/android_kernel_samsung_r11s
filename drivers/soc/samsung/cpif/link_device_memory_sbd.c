...
int create_sbd_mem_map(struct sbd_link_device *sl)
{
	...
	for (i = 0; i < sl->num_channels; i++) {
		...
		for (dir = 0; dir < ULDL; dir++) {
			...
			mif_iov_queue_head_init(&rb->iov_q);
#endif
		}
	}
...
{
	int ret;
	struct iovec *iov = &mif_iov->iov;
	unsigned int qlen = rb->len;
	unsigned int in = *rb->wp;
	unsigned int out = *rb->rp;
	unsigned int count = iov->iov_len;
	unsigned int space = (rb->buff_size - rb->payload_offset);
	u8 *dst;
	...
	//copy_from_user(dst, iov->iov_base, count);
	__copy_from_user_inatomic(dst, iov->iov_base, count);

	if (sipc_ps_ch(rb->ch)) {
		unsigned int ch = mif_iov->ch;

		rb->buff_len_array[in] = (iov->iov_len & 0xFFFF);
		rb->buff_len_array[in] |= (ch << 16);
	} else {
		rb->buff_len_array[in] = iov->iov_len;
	}
