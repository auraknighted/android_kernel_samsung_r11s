...
static void tpmon_set_irq_mask_to_mcps(u32 val)
{
	if (mcps_enable) {
		char mask[MAX_IRQ_AFFINITY_STRING];

		snprintf(mask, MAX_IRQ_AFFINITY_STRING, "%x", 1 << val);
		mif_info("set %s to mcps\n", mask);
		set_mcps_cp_irq_mask(mask);
	}
}
#else
static inline void tpmon_set_irq_mask_to_mcps(u32 val) { return; }
#endif
...
static void tpmon_set_irq_affinity_mbox(struct tpmon_data *data)
{
	...
	tpmon_set_irq_mask_to_mcps(val);
