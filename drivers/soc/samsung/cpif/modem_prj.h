...
#if IS_ENABLED(CONFIG_CPIF_USERSPACE_NETWORK)
	/* LKL reference count */
	atomic_t usnet_opened;
	enum pdn_type p_type;
#endif
...
#if IS_ENABLED(CONFIG_CPIF_USERSPACE_NETWORK)
	struct miscdevice ndev_miscdev;
#endif
...
#define US_NET_CLIENTS_MAX	8

struct us_net_stack {
	u16 us_net_index;
	u16 reserved_port_range[US_NET_PORTS];
	pid_t user_pid;

	struct us_net *usnet_obj;
	struct us_net_iod *usnet_iod;
	...
	struct notifier_block pdn_nb;

	struct pktproc_queue_usnet *q;

	/* for loopback */
	struct us_net_loopback *usnet_lb;
};

struct us_net_loopback {
	__u16 ch;
	bool do_loopback;
};

struct us_net_loopback_data {
	__u16 ch;
	bool do_loopback;

	struct sk_buff_head loopback_skb_q;
#ifdef CONFIG_USNET_TIMER_LOOPBACK
	struct hrtimer loopback_q_timer;
#endif
...
struct us_net_loopback_pktproc_pointers {
	u32 fore_ptr;
	u32 rear_ptr;
	u32 done_ptr;
};
...
struct us_net {
	struct us_net_stack *usnet_clients[US_NET_CLIENTS_MAX];
	struct us_net_loopback_data loopback[US_NET_CLIENTS_MAX];
	...
	/* stores who has inet pdn */
	struct io_device *inet_pdn_iod;
	struct notifier_block inet_pdn_nb;
};
...
struct link_device {
	...
	/* UL io vector func for Userspace network */
	int (*send_iov)(struct link_device *ld, struct io_device *iod,
		    struct mif_iov *iov);
