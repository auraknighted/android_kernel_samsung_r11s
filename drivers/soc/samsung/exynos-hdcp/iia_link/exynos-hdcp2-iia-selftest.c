...
static int utc_ske_send_eks(struct hdcp_tx_ctx *tx_ctx, struct hdcp_rx_ctx *rx_ctx)
{
	...
	ret = cap_protocol_msg(SKE_SEND_EKS, m, &m_len, tx_ctx, rx_ctx);
	if (ret) {
...
static int utc_rtt_challenge(struct hdcp_tx_ctx *tx_ctx, struct hdcp_rx_ctx *rx_ctx)
{
	...
	ret = cap_protocol_msg(RTT_CHALLENGE, m, &m_len, tx_ctx, rx_ctx);
	if (ret) {
...
static int utc_lc_init(struct hdcp_tx_ctx *tx_ctx, struct hdcp_rx_ctx *rx_ctx)
{
	...
	ret = cap_protocol_msg(LC_INIT, m, &m_len, tx_ctx, rx_ctx);
	if (ret) {
			...
			struct hdcp_rx_ctx *rx_ctx)
{
	...
	ret = cap_protocol_msg(AKE_STORED_KM, m, &m_len, tx_ctx, rx_ctx);
	if (ret) {
				...
				struct hdcp_rx_ctx *rx_ctx)
{
	...
	ret = cap_protocol_msg(AKE_NO_STORED_KM, m, &m_len, tx_ctx, rx_ctx);
	if (ret) {
				...
				struct hdcp_rx_ctx *rx_ctx)
{
	...
	ret = cap_protocol_msg(AKE_TRANSMITTER_INFO, m, &m_len, tx_ctx, NULL);
	if (ret) {
...
static int utc_ake_init(struct hdcp_tx_ctx *tx_ctx, struct hdcp_rx_ctx *rx_ctx)
{
	...
	ret = cap_protocol_msg(AKE_INIT, m, &m_len, tx_ctx, NULL);
	if (ret) {
