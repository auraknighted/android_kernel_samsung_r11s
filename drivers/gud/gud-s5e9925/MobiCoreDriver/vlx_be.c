...
static int vlx_create_be_client(struct protocol_fe *pfe)
{
	...
	/* Update the local pfe header with FE versions from the fe2be_data */
	vlx_fe->pfe.hdr.hdr_version = vlx_fe->pfe.fe2be_data->hdr_version;
	vlx_fe->pfe.hdr.fe_version = vlx_fe->pfe.fe2be_data->fe_version;

	/* Get the FE version */
	mc_dev_info("%s Connected to FE version 0x%x",
		    __func__, get_fe_driver_version(&vlx_fe->pfe.hdr));

	/* Set the local pfe header with BE version */
	mc_dev_devel("%s Set the BE version 0x%x", __func__, MC_PV_VERSION);
	if (!set_be_driver_version(&vlx_fe->pfe.hdr, MC_PV_VERSION))
		mc_dev_info("%s BE version is not set", __func__);

	/* Channel fe2be already in place from vlx_be_start_serverCreate().
	 * FE connecting, creating BE client for the FE server (be2fe comm)
	 */
	...
	/* Store in local vlx_fe the FE vRPC link and FE vRPC shared buffer */
	vlx_fe->be2fe_vrpc = vrpc_fe;
	vlx_fe->pfe.be2fe_data = vrpc_data(vrpc_fe);

	/* Update the be2fe_data with BE version for the FE */
	vlx_fe->pfe.be2fe_data->be_version = vlx_fe->pfe.hdr.be_version;
...
static int vlx_be_start_server(struct tee_vlx_fe *vlx_fe)
{
	...
	/* Create BE server for the FE client (fe2be comm) */
	...
	/* Backend server now in place, waiting for Frontend to connect...
	 * Callback channel (be2fe) will only be setup at Frontend connection,
	 * API vlx_create_be_client()
	 */
	mc_dev_info("Started server for FE %s", vlx_fe->vm_info->id);
	return 0;
}

/* Called when Backend start, init Backend server */
