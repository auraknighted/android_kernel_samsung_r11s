...
static int mcfrc_run(struct mcfrc_ctx *mcfrc_context, struct mcfrc_task *task)
{
    struct mcfrc_dev *mcfrc_device = mcfrc_context->mcfrc_dev;
    int ret = 0;
    unsigned long flags = 0;
										...
										struct mcfrc_buffer_dma *dma_buffer)
{
	...
	if (buffer->type == HWMCFRC_BUFFER_DMABUF) {
		...
		dev_dbg(mcfrc_device->dev, "%s: dmabuf of fd %d is %pk\n", __func__
				 ...
				 enum dma_data_direction dir)
{
	...
	dev_dbg(mcfrc_device->dev
		, "%s: about to prepare buffer %pk, dir DMA_TO_DEVICE? %d, dir DMA_FROM_DEVICE? %d\n"
	...
	if (ret) {
		...
		dev_dbg(mcfrc_device->dev, "%s: releasing buffer %pk\n"
			, __func__, dma_buffer);
...
static void mcfrc_destroy_context(struct kref *kreference)
{
	...
	dev_dbg(mcfrc_device->dev, "%s: deleting context %pk from list\n"
		, __func__, ctx);
	list_del(&ctx->node);
 ...
 */
static int mcfrc_process(struct mcfrc_ctx *ctx, struct mcfrc_task *task)
{
	...
	dev_dbg(mcfrc_device->dev, "%s: acquiring kref of ctx %pk\n",
		__func__, ctx);
	kref_get(&ctx->kref);

	mutex_lock(&ctx->mutex);

	dev_dbg(mcfrc_device->dev, "%s: inside context lock (ctx %pk task %pk)\n"
		, __func__, ctx, task);
...
err_prepare_task:
	...
	dev_dbg(mcfrc_device->dev, "%s: releasing lock of ctx %pk\n"
			, __func__, ctx);
	mutex_unlock(&ctx->mutex);

	dev_dbg(mcfrc_device->dev, "%s: releasing kref of ctx %pk\n"
		, __func__, ctx);
...
static int mcfrc_open(struct inode *inode, struct file *filp)
{
	...
	spin_lock_irqsave(&mcfrc_device->lock_ctx, flags);
	dev_dbg(mcfrc_device->dev, "%s: adding new context %pk to contexts list\n"
		, __func__, ctx);
...
static int mcfrc_release(struct inode *inode, struct file *filp)
{
	...
	dev_dbg(ctx->mcfrc_dev->dev, "%s: releasing context %pk\n"
		, __func__, ctx);
	dev_info(ctx->mcfrc_dev->dev, "%s: releasing context %pk\n"
		, __func__, ctx);
...
static int mcfrc_clk_devm_get_prepare(struct mcfrc_dev *mcfrc)
{
	...
	if (IS_ERR(mcfrc->clk_producer)) {
		if (PTR_ERR(mcfrc->clk_producer) != -ENOENT) {
			dev_err(dev, "Failed(%ld) to get 'gate' clock\n",
				PTR_ERR(mcfrc->clk_producer));
...
static int mcfrc_clock_gating(struct mcfrc_dev *mcfrc, bool on)
{
	...
	if (!IS_ERR(mcfrc->clk_producer)) {
		...
		dev_dbg(mcfrc->dev, "%s failed to gating clock : %d\n"
			, __func__, on);
		return -1;
	}
