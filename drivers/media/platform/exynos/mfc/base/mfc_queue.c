		...
		struct mfc_buf_queue *from_queue, unsigned long i_ino, enum mfc_queue_top_type top)
{
	...
	mfc_debug(4, "Looking for this ino: %d\n", i_ino);
	list_for_each_entry(mfc_buf, &from_queue->head, list) {
...
struct mfc_buf *mfc_find_buf_ino(struct mfc_ctx *ctx, struct mfc_buf_queue *queue, int i_ino)
{
	unsigned long flags;
	struct mfc_buf *mfc_buf = NULL;
	int mb_ino;
	...
	mfc_debug(4, "Looking for this inode: %d\n", i_ino);
	list_for_each_entry(mfc_buf, &queue->head, list) {
		mb_ino = mfc_buf->i_ino;
		mfc_debug(4, "inode: %ld, addr[0]: 0x%08llx\n", mb_ino, mfc_buf->addr[0][0]);
		if (i_ino == mb_ino) {
			spin_unlock_irqrestore(&ctx->buf_queue_lock, flags);
			return mfc_buf;
		}
