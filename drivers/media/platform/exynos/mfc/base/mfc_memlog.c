...
void mfc_dev_init_memlog(struct platform_device *pdev)
{
	...
	int ret;
	unsigned int memlog_size = dev->pdata->memlog_size;
	unsigned int memlog_sfr_size = dev->pdata->memlog_sfr_size;
	...
	if (memlog_size == 0)
                memlog_size = MFC_MEMLOG_SIZE;

        if (memlog_sfr_size == 0)
                memlog_sfr_size = MFC_MEMLOG_SFR_SIZE;

	log_obj = memlog_alloc_printf(desc,
					(memlog_size - (dev->num_core * memlog_sfr_size)),
					NULL,
					"log-mem",
					0);
