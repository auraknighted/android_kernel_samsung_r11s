...
void mfc_set_linear_stride_size(struct mfc_ctx *ctx, struct mfc_fmt *fmt)
{
	...
	if (ctx->type == MFCINST_DECODER) {
		...
		if (!ctx->rgb_bpp && (stride % stride_align != 0)) {
			mfc_ctx_err("[FRAME] Forced to change stride(%d) for %dbyte alignment\n",
					stride, stride_align);
			stride = ALIGN(ctx->img_width, stride_align);
		}
	}
	...
	case V4L2_PIX_FMT_RGB32:
		raw->stride[0] = ctx->buf_stride * (ctx->rgb_bpp / 8);
		raw->stride[1] = 0;
		raw->stride[2] = 0;
		if (raw->stride[0] % stride_align != 0) {
			mfc_ctx_err("[FRAME] Forced to change stride(%d) for %dbyte alignment\n",
					raw->stride[0], stride_align);
			raw->stride[0] = ALIGN(raw->stride[0], stride_align);
		}
		break;
