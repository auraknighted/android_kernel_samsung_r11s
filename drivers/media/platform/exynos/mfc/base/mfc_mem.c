	...
	struct mfc_user_shared_handle *handle, char *name)
{
	...
	mfc_debug(2, "[MEMINFO][SH][%s] shared handle fd: %d, vaddr: 0x%p, buf size: %zu, data size: %zu\n",
...
int mfc_get_iovmm_from_fd(struct mfc_ctx *ctx, struct mfc_special_buf *special_buf, int fd)
{
	...
	mfc_debug(2, "[IOVMM] fd: %d addr: %#llx\n", fd, special_buf->daddr);

	return 0;

err_iovmm:
	mfc_debug(2, "[IOVMM] Failed to get iovmm from fd: %d (addr: %#llx)\n",
			fd, special_buf->daddr);
