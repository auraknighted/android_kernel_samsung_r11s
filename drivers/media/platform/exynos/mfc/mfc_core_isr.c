			...
			unsigned int reason, unsigned int err)
{
	...
	mfc_core_qos_get_disp_ratio(ctx, mfc_core_get_decoded_frame_cnt(),
			mfc_core_get_display_frame_cnt());
	qos_update = mfc_core_qos_mb_calculate(core, core_ctx, mfc_core_get_processing_cycle(),
			mfc_core_get_dec_frame_type());
...
static int __mfc_handle_seq_dec(struct mfc_core *core, struct mfc_ctx *ctx)
{
	...
	if (ctx->img_width == 0 || ctx->img_height == 0) {
		...
		if (dev->pdata->support_sbwc && (core_ctx->state != MFCINST_SPECIAL_PARSING)) {
			...
			} else if (ctx->is_sbwc && IS_AV1_DEC(ctx) && dec->av1_film_grain_present) {
				ctx->is_sbwc = 0;
				ctx->sbwc_disabled |= MFC_SBWC_OFF_AV1_FGN;
				mfc_debug(2, "[SBWC] disable sbwc, AV1 Film Grain\n");
			...
			if (ctx->is_sbwc &&
				!(dev->debugfs.feature_option & MFC_OPTION_DEC_ENC_SBWC_ENABLE) &&
				dev->num_enc_inst) {
				ctx->is_sbwc = 0;
...
irqreturn_t mfc_core_top_half_irq(int irq, void *priv)
{
	...
	if ((reason == MFC_REG_R2H_CMD_SEQ_DONE_RET) ||
		...
		mfc_core_qos_set_portion(core, ctx);
	}
...
irqreturn_t mfc_core_irq(int irq, void *priv)
{
	...
	if (ctx->otf_handle) {
		if (mfc_core_otf_ctx_ready_set_bit(core_ctx, &core->work_bits, true) == 0)
			mfc_core_otf_ctx_ready_set_bit(core_ctx, &core->work_bits, false);
	}
