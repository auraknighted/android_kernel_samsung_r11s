...
static inline void csi_trigger_gtask(struct is_device_sensor *sensor, struct is_device_csi *csi)
{
	...
	if (ischain && !test_bit(IS_SENSOR_STAND_ALONE, &sensor->state)) {
...
#endif
#if defined(OVERFLOW_DTP_ENABLE_CSIS)
	struct is_device_sensor *device;
#endif
...
#elif defined(OVERFLOW_DTP_ENABLE_CSIS)
				device = container_of(csi->subdev, struct is_device_sensor, subdev_csi);
				set_bit(IS_SENSOR_FRONT_DTP_STOP, &device->state);
				err("DTP forcely set by %s", err_str);
#endif
				break;
			case CSIS_ERR_LOST_FE_VC:
