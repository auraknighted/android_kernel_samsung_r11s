...
#ifdef TEMP_WIDE_DUALIZED
#define RSV_2LD_1 "D50XLOD00SA"
#define RSV_2LD_2 "D50XLOD01SA"
#define RSV_2LD_3 "D50XLOD00PA"

bool is_sec_fw_module_compare_all(char *fw_ver1, char *fw_ver2)
{
	if (fw_ver1[FW_CORE_VER] != fw_ver2[FW_CORE_VER]
		|| fw_ver1[FW_PIXEL_SIZE] != fw_ver2[FW_PIXEL_SIZE]
		|| fw_ver1[FW_PIXEL_SIZE + 1] != fw_ver2[FW_PIXEL_SIZE + 1]
		|| fw_ver1[FW_ISP_COMPANY] != fw_ver2[FW_ISP_COMPANY]
		|| fw_ver1[FW_SENSOR_MAKER] != fw_ver2[FW_SENSOR_MAKER]
		|| fw_ver1[FW_PUB_YEAR] != fw_ver2[FW_PUB_YEAR]
		|| fw_ver1[FW_PUB_MON] != fw_ver2[FW_PUB_MON]
		|| fw_ver1[FW_PUB_NUM] != fw_ver2[FW_PUB_NUM]
		|| fw_ver1[FW_PUB_NUM + 1] != fw_ver2[FW_PUB_NUM + 1]
		|| fw_ver1[FW_MODULE_COMPANY] != fw_ver2[FW_MODULE_COMPANY]
		|| fw_ver1[FW_VERSION_INFO] != fw_ver2[FW_VERSION_INFO]) {
		return false;
	}
...
#ifdef TEMP_WIDE_DUALIZED
	if (is_sec_fw_module_compare_all(finfo->header_ver, RSV_2LD_1)
		|| is_sec_fw_module_compare_all(finfo->header_ver, RSV_2LD_2)
		|| is_sec_fw_module_compare_all(finfo->header_ver, RSV_2LD_3)) {
		specific->sensor_id[SENSOR_POSITION_REAR] = SENSOR_NAME_S5K2LD;
	}
#endif
...
int is_sec_sensorid_find_front(struct is_core *core)
{
	...
	if (is_sec_fw_module_compare(finfo->header_ver, FW_3J1_X)) {
		specific->sensor_id[SENSOR_POSITION_FRONT] = SENSOR_NAME_S5K3J1;
...
int is_sec_fw_find(struct is_core *core)
{
	...
	case SENSOR_NAME_S5K2LD:
		...
		break;
	case SENSOR_NAME_S5KGN3:
		snprintf(finfo->load_setfile_name, sizeof(IS_GN3_SETF), "%s", IS_GN3_SETF);
		snprintf(finfo->load_rta_fw_name, sizeof(IS_RTA), "%s", IS_RTA);
		break;
	case SENSOR_NAME_S5KHM3:
		snprintf(finfo->load_setfile_name, sizeof(IS_HM3_SETF), "%s", IS_HM3_SETF);
		snprintf(finfo->load_rta_fw_name, sizeof(IS_RTA), "%s", IS_RTA);
		break;
	default:
		snprintf(finfo->load_setfile_name, sizeof(IS_GN3_SETF), "%s", IS_GN3_SETF);
	...
	switch (front_id) {
	case SENSOR_NAME_IMX374:
		snprintf(finfo_front->load_front_setfile_name, sizeof(IS_IMX374_SETF), "%s", IS_IMX374_SETF);
		break;
	case SENSOR_NAME_S5KGH1:
		snprintf(finfo_front->load_front_setfile_name, sizeof(IS_GH1_SETF), "%s", IS_GH1_SETF);
		break;
	default:
		snprintf(finfo_front->load_front_setfile_name, sizeof(IS_IMX374_SETF), "%s", IS_IMX374_SETF);
		break;
