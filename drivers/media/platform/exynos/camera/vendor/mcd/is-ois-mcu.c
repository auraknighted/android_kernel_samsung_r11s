...
int ois_mcu_deinit(struct v4l2_subdev *subdev)
{
	...
	if (ois_hw_check
		...
		&& !(ois_wide_init || ois_tele_init || ois_tele2_init)
#endif
		) {
					...
					bool *x_result_3rd, bool *y_result_3rd, int *sin_x_3rd, int *sin_y_3rd)
{
	...
	if (value == true) {
		...
		*y_result_3rd = true;
		...
		err("OIS autotest_2nd is failed result = 0x%04x\n", result);
		if ((result & 0x03) == 0x00) {
			*x_result = true;
			*y_result = true;
...
int ois_mcu_set_power_mode(struct v4l2_subdev *subdev, int forceMode)
{
	struct is_ois *ois = NULL;
	struct ois_mcu_dev *mcu = NULL;
	struct is_mcu *is_mcu = NULL;
