...
{
	is_vender_fw_prepare(vender, position);
	...
	case V4L2_CID_IS_CAMERA_TYPE:
		...
		case IS_WARM_BOOT:
			if (specific ->need_cold_reset) {
				mvinfo("[VENDER] FW first launching mode for reset\n", device, video);
...
{
	return (((cis_shared_data *)cis_data)->is_data.sensor_hdr_mode == SENSOR_HDR_MODE_2AEB_2VC ? true : false);
}
...
int is_vender_replace_sensorid_with_second_sensorid(struct is_vender *vender, int position) {
	...
	if(position >= SENSOR_POSITION_REAR && position < SENSOR_POSITION_MAX) {
		specific->sensor_id[position] = sensor_id[position][1];
	}
	else {
