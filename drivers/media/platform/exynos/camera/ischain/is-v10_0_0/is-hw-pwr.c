...
{
#if IS_ENABLED(CMU_CSIS_AUTO_CLK_CONT)
	void __iomem *base;
	unsigned int cfg;
	unsigned int mask = (1 << 29); /* ENABLE_POWER_MANAGEMENT */

	/* CMU_CSIS - CSIS_CMU_CSIS_CONTROLLER_OPTION */
	base = ioremap(0x17000800, 0x4);
	if (!base) {
		pr_err("Failed to map CMU_CSIS\n");
		return -ENOMEM;
	}
