...
void is_ischain_isp_in_calc_stripe_start_pos(struct is_crop *incrop,
	struct camera2_stream *stream, bool is_leader,
	u32 region_num, u32 h_pix_num[])
{
	...
	for (region_id = 0; region_id < region_num; ++region_id) {
		if (!region_id) {
			...
			/* Middle region */
			if (is_leader || !stream->stripe_region_num) {
				stripe_w = incrop->w * (region_id + 1) / region_num;
				stripe_w = ALIGN((stripe_w - h_pix_num[region_id - 1]), 4);
				stripe_w = ALIGN_UPDOWN_STRIPE_WIDTH(stripe_w);
			} else {
				stripe_w = stream->stripe_h_pix_nums[region_id] - h_pix_num[region_id - 1];
			}
		} else {
			/* Right region */
		...
		bool is_leader)
{
	...
	if (!region_id) {
		/* Left region */
		is_ischain_isp_in_calc_stripe_start_pos(incrop, stream, is_leader,
