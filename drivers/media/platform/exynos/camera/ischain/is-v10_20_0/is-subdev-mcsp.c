	...
	int index)
{
	...
	if (mcs_output->dma_cmd != node->request)
	...
	if ((mcs_output->crop_width != incrop->w) || (mcs_output->crop_height != incrop->h) ||
	    (mcs_output->width != otcrop->w) || (mcs_output->height != otcrop->h))
	...
	if ((pindex - PARAM_MCS_OUTPUT0) == MCSC_INPUT_HF && !node->bytesperline[0]) {
		...
		mwarn("DMA stride(y:%#x uv:%#x) should be %d aligned",
			device, mcs_output->dma_stride_y, mcs_output->dma_stride_c, 16);
	}
