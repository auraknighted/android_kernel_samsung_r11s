	...
	struct camera2_node *node)
{
	...
	if (IS_ENABLED(LOGICAL_VIDEO_NODE)) {
		...
		internal_framemgr = GET_SUBDEV_I_FRAMEMGR(&device->orbxc);
		if (!internal_framemgr)
			mrerr("internal framemgr is NULL\n", device, frame);

		internal_frame = &internal_framemgr->frames[0];
		if (!internal_frame) {
			mrerr("internal frame is NULL\n", device, frame);
			return -EINVAL;
		}
		targetAddr = NULL;
		targetAddr_k = NULL;
		ret = is_hw_get_capture_slot(frame, &targetAddr,
				&targetAddr_k, IS_VIDEO_MCH0S_NUM);
		if (targetAddr) {
			for (j = 0; j < internal_frame->planes; j++)
				targetAddr[j] = internal_frame->dvaddr_buffer[j];
		}
		if (targetAddr_k) {
			/* IS_VIDEO_MCH0S_NUM */
			for (j = 0; j < internal_frame->planes; j++)
				targetAddr_k[j] = internal_frame->kvaddr_buffer[j];
		}
