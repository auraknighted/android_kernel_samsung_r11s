...
static int is_hw_3aa_disable(struct is_hw_ip *hw_ip, u32 instance, ulong hw_map)
{
	 ...
	 */
	is_lib_isp_stop(hw_ip, &hw_3aa->lib[instance], instance, 1);
	...
	ulong hw_map)
{
	...
	/* DMA settings */
	is_hardware_dma_cfg("3xs", hw_ip, frame, cur_idx, frame->num_buffers,
	...
	cmd_before_bds = is_hardware_dma_cfg_32bit("3xc", hw_ip, frame, cur_idx, frame->num_buffers,
		&param_set->dma_output_before_bds.cmd,
		param_set->dma_output_before_bds.plane,
		param_set->output_dva_before_bds,
		frame->txcTargetAddress);

	cmd_after_bds = is_hardware_dma_cfg_32bit("3xp", hw_ip, frame, cur_idx, frame->num_buffers,
		&param_set->dma_output_after_bds.cmd,
	...
	cmd_efd = is_hardware_dma_cfg_32bit("efd", hw_ip, frame, cur_idx, frame->num_buffers,
		&param_set->dma_output_efd.cmd,
		param_set->dma_output_efd.plane,
		param_set->output_dva_efd,
		frame->efdTargetAddress);

	cmd_mrg = is_hardware_dma_cfg_32bit("mrg", hw_ip, frame, cur_idx, frame->num_buffers,
	...
	u32 instance, ulong hw_map)
{
	...
	ret = is_lib_isp_apply_tune_set(&hw_3aa->lib[instance], setfile_index, instance, scenario);
	if (ret)
		return ret;
