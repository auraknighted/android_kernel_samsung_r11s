...
{
	struct is_hw_orbmch *hw_orbmch;
	struct is_orbmch_config *config;
	u32 *input_dva;
	u32 dma_en = false;
	int ret;
	...
	hw_orbmch = (struct is_hw_orbmch *)hw_ip->priv_info;
	config = &hw_orbmch->iq_config[instance];
	...
	case ORBMCH_RDMA_MCH_PREV_DESC:
		/* internal buffer */
		input_dva = param_set->prev_output_dva;
		dma_en = param_set->dma.cur_input_cmd;
		break;
	case ORBMCH_RDMA_MCH_CUR_DESC:
		input_dva = param_set->output_dva;
	...
	msdbg_hw(2, "%s: dma enable(%d)\n", instance, hw_ip, hw_orbmch->rdma[dma_id].name, dma_en);
	...
	u32 instance, u32 set_id, u32 dma_id)
{
	...
	msdbg_hw(2, "%s: dma enable(%d)\n", instance, hw_ip, hw_orbmch->wdma[dma_id].name, dma_en);
	...
	u32 instance, u32 set_id)
{
	...
	memset(&hw_orbmch->iq_config, 0x0, sizeof(struct is_orbmch_config));
...
static int __is_hw_orbmch_set_rta_regs(struct is_hw_ip *hw_ip, u32 instance, u32 set_id)
{
	struct is_hw_orbmch *hw_orbmch;
	...
	struct is_group *group, bool flag)
{
	...
	is_hardware_alloc_internal_buffer(device, &device->orbxc, group->leader.vid,
		...
		ORB_DESC_BUF_NUM, "ORB_PREV_DESC");
...
int is_hw_orbmch_set_config(struct is_hw_ip *hw_ip, u32 chain_id, u32 instance, u32 fcount, void *conf)
{
	...
	hw_orbmch = (struct is_hw_orbmch *)hw_ip->priv_info;
	memcpy(&hw_orbmch->iq_config, conf, sizeof(hw_orbmch->iq_config));
