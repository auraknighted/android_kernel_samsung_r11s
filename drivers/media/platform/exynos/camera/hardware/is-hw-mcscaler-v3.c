...
static int debug_mcsc;
module_param(debug_mcsc, int, 0644);

static int debug_subblk_ctrl;
module_param(debug_subblk_ctrl, int, 0644);

static spinlock_t	mcsc_out_slock;
				...
				struct mcs_param *mcs_param)
{
	...
	for (out_id = MCSC_OUTPUT0; out_id < cap->max_output; out_id++) {
		...
		if (use_sub_flip || (mcs_param->output[out_id].dma_cmd != DMA_OUTPUT_COMMAND_DISABLE
			...
			is_hw_mcsc_wdma_clear(hw_ip, frame, mcs_param, out_id, cap);
		}
	}
}
	...
	struct mcs_param *param, struct is_frame *frame, u32 instance)
{
	...
	u32 *rdma_addr, b, dva, hdr_dva, payload_size = 0;
	...
	if (param->output[MCSC_OUTPUT5].dma_cmd == DMA_OUTPUT_VOTF_ENABLE) {
		dma_frame = is_votf_get_frame(hw_ip->group[instance], TRS, device->m5p.id, 0);
		if (!dma_frame) {
			msdbg_hw(2, "[F:%d] high freqeuncy : dma_frame is null", instance, hw_ip,
					atomic_read(&hw_ip->fcount));
			dma_frame = frame;
			rdma_addr = hw_mcsc_get_target_addr(MCSC_INPUT_HF, dma_frame);
		} else {
			use_votf = true;
			is_scaler_set_votf_config(hw_ip->regs[REG_SETA], hw_ip->id, true);
			rdma_addr = (u32 *)dma_frame->dvaddr_buffer;
		}
	} else
		rdma_addr = hw_mcsc_get_target_addr(MCSC_INPUT_HF, dma_frame);
	...
	if (hf_param->dma_cmd == DMA_OUTPUT_COMMAND_DISABLE || !rdma_addr[0]) {
		...
		msdbg_hw(2, "[F:%d]high frequency: hf disabled, hf_param->dma_cmd(%d), rdma_addr[0](%x)\n",
	...
	for (b = 0; b < frame->num_buffers; b++) {
		...
		msdbg_hw(2, "[F:%d]high frequency: rdma_cfg [T:%d][addr:%x][haddr:%x], stripe_start_pos_x(%d)\n",
...
config:
	...
	/* M5P RDMA supports VOTF */
	if (mcs_param->output[MCSC_OUTPUT5].dma_cmd == DMA_OUTPUT_COMMAND_ENABLE) {
		ret = is_hw_mcsc_set_votf_size_config(mcs_param->output[MCSC_OUTPUT5].width,
							mcs_param->output[MCSC_OUTPUT5].height);
		if (ret) {
			mserr_hw("is_hw_mcsc_set_votf_size_config is fail", instance, hw_ip);
			goto err_votf;
		}
	}
	...
	if (unlikely(debug_mcsc))
		is_scaler_dump(hw_ip->regs[REG_SETA]);

	/* Sync mcs_param of frame to hw_ip for stripe processing */
	memcpy(&hw_ip->region[instance]->parameter.mcs, mcs_param, sizeof(struct mcs_param));
