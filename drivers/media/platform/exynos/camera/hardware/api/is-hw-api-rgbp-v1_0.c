...
#include "sfr/is-sfr-rgbp-v1_0.h"
...
void rgbp_hw_s_path(void __iomem *base, u32 set_id, struct is_rgbp_config *config)
{
	...
	chain_set.wdma_rep_sel = 0x1;
	chain_set.demux_dmsc_en = 0x1;
	...
	RGBP_SET_F(base, RGBP_R_CHAIN_MUX_SELECT, RGBP_F_MUX_WDMA_REP_SELECT, chain_set.wdma_rep_sel);
...
int rgbp_hw_s_reset(void __iomem *base)
{
	...
	while (1) {
		temp = RGBP_GET_R(base, RGBP_R_SW_RESET);
		if (temp == 0)
			break;
...
int rgbp_hw_rdma_create(struct is_common_dma *dma, void __iomem *base, u32 input_id)
{
	...
	case RGBP_RDMA_CL:
		...
		strncpy(name, "RGBP_RDMA_CL", sizeof(name) - 1);
		break;
	default:
		err_hw("[RGBP] invalid dma_id[%d]", dma->id);
		return SET_ERROR;
	...
	u32 instance, u32 enable, u32 in_crop_size_x, u32 cache_hint, u32 *sbwc_en, u32 *payload_size)
{
	...
	case RGBP_WDMA_HF:
		...
		break;
	case RGBP_WDMA_SF:
		width = param_set->dma_output_sf.width;
		height = param_set->dma_output_sf.height;
		break;
	case RGBP_WDMA_R:
	case RGBP_WDMA_G:
		...
		break;
	case RGBP_WDMA_B:
		width = param_set->dma_output_rgb.width;
		height = param_set->dma_output_rgb.height;
		break;
	default:
...
int rgbp_hw_wdma_create(struct is_common_dma *dma, void __iomem *base, u32 input_id)
{
	...
	case RGBP_WDMA_HF:
		...
		strncpy(name, "RGBP_WDMA_HF", sizeof(name) - 1);
		break;
	case RGBP_WDMA_SF:
		base_reg = base + rgbp_regs[RGBP_R_STAT_WDMASATFLG_EN].sfr_offset;
		available_bayer_format_map = 0x777 & IS_BAYER_FORMAT_MASK;
		strncpy(name, "RGBP_WDMA_SF", sizeof(name) - 1);
		break;
	case RGBP_WDMA_R:
		base_reg = base + rgbp_regs[RGBP_R_RGB_WDMAREPR_EN].sfr_offset;
		available_bayer_format_map = 0x777 & IS_BAYER_FORMAT_MASK;
		strncpy(name, "RGBP_WDMA_R", sizeof(name) - 1);
		break;
	case RGBP_WDMA_G:
		base_reg = base + rgbp_regs[RGBP_R_RGB_WDMAREPG_EN].sfr_offset;
		available_bayer_format_map = 0x777 & IS_BAYER_FORMAT_MASK;
		strncpy(name, "RGBP_WDMA_G", sizeof(name) - 1);
		break;
	case RGBP_WDMA_B:
		base_reg = base + rgbp_regs[RGBP_R_RGB_WDMAREPB_EN].sfr_offset;
		available_bayer_format_map = 0x777 & IS_BAYER_FORMAT_MASK;
		strncpy(name, "RGBP_WDMA_B", sizeof(name) - 1);
		break;
...
void rgbp_hw_s_corex_init(void __iomem *base, bool enable)
{
	...
	while (1) {
		reset = RGBP_GET_R(base, RGBP_R_COREX_RESET);
		if (reset == 0)
			break;
		if (reset_count > RGBP_TRY_COUNT) {
...
unsigned int rgbp_hw_g_int0_state(void __iomem *base, bool clear, u32 num_buffers, u32 *irq_state)
{
	...
	src_err = src_all & RGBP_INT0_EN_MASK;
...
unsigned int rgbp_hw_g_int1_state(void __iomem *base, bool clear, u32 num_buffers, u32 *irq_state)
{
	...
	src_err = src_all & RGBP_INT1_EN_MASK;
...
void rgbp_hw_s_block_crc(void __iomem *base, u32 seed)
{
	...
	RGBP_SET_F(base, RGBP_R_RGB_CCM_STREAM_CRC, RGBP_F_RGB_CCM_CRC_SEED, seed);
...
{
	RGBP_SET_F(base, RGBP_R_YUV_SC_INPUT_SIZE, RGBP_F_YUV_SC_IN_HSIZE, width);
	RGBP_SET_F(base, RGBP_R_YUV_SC_INPUT_SIZE, RGBP_F_YUV_SC_IN_VSIZE, height);
}
...
{
	RGBP_SET_F(base, RGBP_R_YUV_SC_SRC_SIZE, RGBP_F_YUV_SC_SRC_HSIZE, width);
	RGBP_SET_F(base, RGBP_R_YUV_SC_SRC_SIZE, RGBP_F_YUV_SC_SRC_VSIZE, height);
}
...
{
	RGBP_SET_F(base, RGBP_R_YUV_SC_OUT_CROP_SIZE, RGBP_F_YUV_SC_OUT_CROP_SIZE_H, width);
	RGBP_SET_F(base, RGBP_R_YUV_SC_OUT_CROP_SIZE, RGBP_F_YUV_SC_OUT_CROP_SIZE_V, height);
...
void rgbp_hw_s_block_bypass(void __iomem *base, u32 set_id)
{
	...
	RGBP_SET_F(base, RGBP_R_YUV_YUV444TO422_COEFFS, RGBP_F_YUV_YUV444TO422_MCOEFFS_0_0, 0);
	RGBP_SET_F(base, RGBP_R_YUV_YUV444TO422_COEFFS, RGBP_F_YUV_YUV444TO422_MCOEFFS_0_1, 32);
	RGBP_SET_F(base, RGBP_R_YUV_YUV444TO422_COEFFS, RGBP_F_YUV_YUV444TO422_MCOEFFS_0_2, 64);
	RGBP_SET_F(base, RGBP_R_YUV_YUV444TO422_COEFFS, RGBP_F_YUV_YUV444TO422_MCOEFFS_0_3, 32);
	RGBP_SET_F(base, RGBP_R_YUV_YUV444TO422_COEFFS_1, RGBP_F_YUV_YUV444TO422_MCOEFFS_0_4, 0);
	...
	RGBP_SET_F(base, RGBP_R_RGB_CCM_BYPASS, RGBP_F_RGB_CCM_BYPASS, 1);
...
{
	RGBP_SET_F(base, RGBP_R_YUV_SC_CTRL0, RGBP_F_YUV_SC_ENABLE, enable);
...
{
	RGBP_SET_F(base, RGBP_R_YUV_SC_SRC_SIZE, RGBP_F_YUV_SC_SRC_HSIZE, h_size);
	RGBP_SET_F(base, RGBP_R_YUV_SC_SRC_SIZE, RGBP_F_YUV_SC_SRC_VSIZE, v_size);
}

void is_scaler_get_yuvsc_src_size(void __iomem *base, u32 set_id, u32 *h_size, u32 *v_size)
{
	*h_size = RGBP_GET_F(base, RGBP_R_YUV_SC_SRC_SIZE, RGBP_F_YUV_SC_SRC_HSIZE);
	*v_size = RGBP_GET_F(base, RGBP_R_YUV_SC_SRC_SIZE, RGBP_F_YUV_SC_SRC_VSIZE);
}
...
{
	RGBP_SET_F(base, RGBP_R_YUV_SC_INPUT_SIZE, RGBP_F_YUV_SC_IN_HSIZE, h_size);
	RGBP_SET_F(base, RGBP_R_YUV_SC_INPUT_SIZE, RGBP_F_YUV_SC_IN_VSIZE, v_size);
}
...
{
	RGBP_SET_F(base, RGBP_R_YUV_SC_OUT_CROP_SIZE, RGBP_F_YUV_SC_OUT_CROP_SIZE_H, h_size);
	RGBP_SET_F(base, RGBP_R_YUV_SC_OUT_CROP_SIZE, RGBP_F_YUV_SC_OUT_CROP_SIZE_V, v_size);
}
...
void rgbp_hw_s_yuvsc_coef(void __iomem *base, u32 set_id, u32 hratio, u32 vratio)
{
	...
	yuvsc_set_poly_scaler_h_coef(base, set_id, h_coef);
	yuvsc_set_poly_scaler_v_coef(base, set_id, v_coef);
}
...
{
	RGBP_SET_F(base, RGBP_R_Y_UPSC_SRC_SIZE, RGBP_F_Y_UPSC_SRC_HSIZE, h_size);
	RGBP_SET_F(base, RGBP_R_Y_UPSC_SRC_SIZE, RGBP_F_Y_UPSC_SRC_VSIZE, v_size);
}
...
{
	*h_size = RGBP_GET_F(base, RGBP_R_Y_UPSC_SRC_SIZE, RGBP_F_Y_UPSC_SRC_HSIZE);
	*v_size = RGBP_GET_F(base, RGBP_R_Y_UPSC_SRC_SIZE, RGBP_F_Y_UPSC_SRC_VSIZE);
}
...
{
	RGBP_SET_F(base, RGBP_R_Y_UPSC_INPUT_SIZE, RGBP_F_Y_UPSC_IN_HSIZE, h_size);
	RGBP_SET_F(base, RGBP_R_Y_UPSC_INPUT_SIZE, RGBP_F_Y_UPSC_IN_VSIZE, v_size);
}
...
{
	RGBP_SET_F(base, RGBP_R_Y_UPSC_OUT_CROP_SIZE, RGBP_F_Y_UPSC_OUT_CROP_SIZE_H, h_size);
	RGBP_SET_F(base, RGBP_R_Y_UPSC_OUT_CROP_SIZE, RGBP_F_Y_UPSC_OUT_CROP_SIZE_V, v_size);
}
KUNIT_EXPORT_SYMBOL(rgbp_hw_s_upsc_out_crop_size);

void rgbp_hw_s_ds_y_upsc_us(void __iomem *base, u32 set_id, u32 set)
{
	RGBP_SET_F(base, RGBP_R_YUV_SC_CTRL1, RGBP_F_YUV_SC_DS_Y_UPSC_US, set);
}
...
void rgbp_hw_s_upsc_coef(void __iomem *base, u32 set_id, u32 hratio, u32 vratio)
{
	...
	rgbp_hw_s_h_init_phase_offset(base, set_id, h_phase_offset);
	rgbp_hw_s_v_init_phase_offset(base, set_id, v_phase_offset);
	upsc_set_poly_scaler_h_coef(base, set_id, h_coef);
	upsc_set_poly_scaler_v_coef(base, set_id, v_coef);
}
KUNIT_EXPORT_SYMBOL(rgbp_hw_s_upsc_coef);

void rgbp_hw_s_gamma_enable(void __iomem *base, u32 set_id, u32 enable, u32 bypass)
{
	RGBP_SET_F(base, RGBP_R_Y_GAMMALR_BYPASS, RGBP_F_Y_GAMMALR_ENABLE, enable);
	RGBP_SET_F(base, RGBP_R_Y_GAMMAHR_BYPASS, RGBP_F_Y_GAMMAHR_ENABLE, enable);
...
{
	RGBP_SET_F(base, RGBP_R_Y_DECOMP_BYPASS, RGBP_F_Y_DECOMP_ENABLE, enable);
...
{
	RGBP_SET_F(base, RGBP_R_STAT_WDMADECOMP_VOTF_EN, RGBP_F_STAT_WDMADECOMP_VOTF_EN, enable);
	RGBP_SET_F(base, RGBP_R_STAT_WDMADECOMP_VOTF_EN, RGBP_F_STAT_WDMADECOMP_VOTF_STALL, stall);
}

void rgbp_hw_s_sbwc(void __iomem *base, u32 set_id, bool enable)
{
	/* Register added in v1.1 */
}
