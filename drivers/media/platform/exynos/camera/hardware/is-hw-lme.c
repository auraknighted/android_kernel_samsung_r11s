...
#ifdef DDK_LIB_CALL
		is_lib_isp_stop(hw_ip, &hw_lme->lib[instance], instance, 1);
#endif
	} else {
	...
	ulong hw_map)
{
	...
	is_hw_lme_update_param(hw_ip, region, param_set, frame->pmap, instance);
	...
	if (is_reprocessing) {
		...
		is_hardware_dma_cfg_64bit("lmes_prev", hw_ip,
			frame, cur_idx,
			&param_set->dma.prev_input_cmd,
			param_set->dma.input_plane,
		...
		IS_DECLARE_PMAP(pmap), u32 instance, ulong hw_map)
{
	...
	is_hw_lme_update_param(hw_ip, region, param_set, pmap, instance);
#ifdef DDK_LIB_CALL
	...
	u32 instance, ulong hw_map)
{
	...
	ret = is_lib_isp_apply_tune_set(&hw_lme->lib[instance], setfile_index, instance, scenario);
	*/
