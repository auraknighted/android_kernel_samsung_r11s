	...
	struct is_group *group)
{
	...
	hw_ypp->lib_support = is_get_lib_support();

	hw_ypp->lib[instance].func = hw_ypp->lib_func;
	if (IS_ENABLED(YPP_DDK_LIB_CALL)) {
		ret = is_lib_isp_chain_create(hw_ip, &hw_ypp->lib[instance],
				instance);
		if (ret) {
...
static int is_hw_ypp_deinit(struct is_hw_ip *hw_ip, u32 instance)
{
	...
	if (IS_ENABLED(YPP_DDK_LIB_CALL)) {
		is_lib_isp_object_destroy(hw_ip, &hw_ypp->lib[instance],
				instance);
		hw_ypp->lib[instance].object = NULL;
	}
...
static int is_hw_ypp_close(struct is_hw_ip *hw_ip, u32 instance)
{
	...
	if (IS_ENABLED(YPP_DDK_LIB_CALL))
		is_lib_isp_chain_destroy(hw_ip, &hw_ypp->lib[instance],
				instance);
...
static int is_hw_ypp_disable(struct is_hw_ip *hw_ip, u32 instance, ulong hw_map)
{
	...
	if (test_bit(HW_RUN, &hw_ip->state)) {
		if (IS_ENABLED(YPP_DDK_LIB_CALL))
			is_lib_isp_stop(hw_ip, &hw_ypp->lib[instance],
					instance, 1);
	} else {
		...
		IS_DECLARE_PMAP(pmap), u32 instance, ulong hw_map)
{
	...
	param_set = &hw_ypp->param_set[instance];
	__is_hw_ypp_update_param(hw_ip, region, param_set, pmap, instance);
	...
	ulong hw_map)
{
	...
	__is_hw_ypp_update_param(hw_ip, region, param_set, frame->pmap, instance);
	...
	ulong hw_map)
{
	...
	if (IS_ENABLED(YPP_DDK_LIB_CALL)) {
		ret = is_lib_isp_get_meta(hw_ip, &hw_ypp->lib[frame->instance],
				frame);
		if (ret)
	...
	u32 instance, ulong hw_map)
{
	...
	hw_ypp = (struct is_hw_ypp *)hw_ip->priv_info;
	if (IS_ENABLED(YPP_DDK_LIB_CALL))
		ret = is_lib_isp_apply_tune_set(&hw_ypp->lib[instance],
...
int is_hw_ypp_restore(struct is_hw_ip *hw_ip, u32 instance)
{
	...
	if (IS_ENABLED(YPP_DDK_LIB_CALL)) {
		ret = is_lib_isp_reset_recovery(hw_ip, &hw_ypp->lib[instance],
				instance);
		if (ret) {
