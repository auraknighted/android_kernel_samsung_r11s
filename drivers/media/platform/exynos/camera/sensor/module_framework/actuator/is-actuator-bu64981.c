...
#define ACTUATOR_NAME		"BU64981"

static int sensor_bu64981_write_position(struct i2c_client *client, u32 val)
{
	int ret = 0;
	...
	if (val > BU64981_POS_MAX_SIZE) {
		err("Invalid af position(position : %d, Max : %d).\n",
					val, BU64981_POS_MAX_SIZE);
		ret = -EINVAL;
		goto p_err;
	}
	...
	ret = is_sensor_addr8_write8(client, val_high, val_low);

p_err:
	return ret;
}

int sensor_bu64981_actuator_init(struct v4l2_subdev *subdev, u32 val)
{
	int ret = 0;
	u8 product_id = 0;
	...
	ret = is_sensor_addr8_write8(client, 0x80, 0x00);
	if (ret < 0)
		goto p_err_mutex;

	ret = is_sensor_addr8_write8(client, 0xCD, 0x91);
	if (ret < 0)
		goto p_err_mutex;
...
int sensor_bu64981_actuator_get_status(struct v4l2_subdev *subdev, u32 *info)
{
	int ret = 0;
	u8 val = 0;
...
int sensor_bu64981_actuator_set_position(struct v4l2_subdev *subdev, u32 *info)
{
	int ret = 0;
	struct is_actuator *actuator;
	...
	I2C_MUTEX_LOCK(actuator->i2c_lock);
	position = *info;
	if (position > BU64981_POS_MAX_SIZE) {
		err("Invalid af position(position : %d, Max : %d).\n",
					position, BU64981_POS_MAX_SIZE);
		ret = -EINVAL;
		goto p_err_mutex;
	}
	...
	/* position Set */
	ret = sensor_bu64981_write_position(client, position);
	if (ret < 0)
		goto p_err_mutex;
...
static int sensor_bu64981_actuator_g_ctrl(struct v4l2_subdev *subdev, struct v4l2_control *ctrl)
{
	int ret = 0;
	u32 val = 0;
	...
	case V4L2_CID_ACTUATOR_GET_STATUS:
		ret = sensor_bu64981_actuator_get_status(subdev, &val);
		if (ret < 0) {
...
static int sensor_bu64981_actuator_s_ctrl(struct v4l2_subdev *subdev, struct v4l2_control *ctrl)
{
	int ret = 0;

	switch(ctrl->id) {
	case V4L2_CID_ACTUATOR_SET_POSITION:
		ret = sensor_bu64981_actuator_set_position(subdev, &ctrl->value);
		if (ret) {
...
long sensor_bu64981_actuator_ioctl(struct v4l2_subdev *subdev, unsigned int cmd, void *arg)
{
	int ret = 0;
	...
	case SENSOR_IOCTL_ACT_S_CTRL:
		ret = sensor_bu64981_actuator_s_ctrl(subdev, ctrl);
		if (ret) {
	...
	case SENSOR_IOCTL_ACT_G_CTRL:
		ret = sensor_bu64981_actuator_g_ctrl(subdev, ctrl);
		if (ret) {
...
static const struct v4l2_subdev_core_ops core_ops = {
	.init = sensor_bu64981_actuator_init,
	.ioctl = sensor_bu64981_actuator_ioctl,
};
...
static int sensor_bu64981_actuator_probe(struct i2c_client *client,
		const struct i2c_device_id *id)
{
	...
	actuator->id = ACTUATOR_NAME_BU64981;
	actuator->subdev = subdev_actuator;
	actuator->device = sensor_id;
	actuator->client = client;
	actuator->position = 0;
	actuator->max_position = BU64981_POS_MAX_SIZE;
	actuator->pos_size_bit = BU64981_POS_SIZE_BIT;
	actuator->pos_direction = BU64981_POS_DIRECTION;
...
static const struct of_device_id sensor_actuator_bu64981_match[] = {
	{
		.compatible = "samsung,exynos-is-actuator-bu64981",
	},
	{},
};
MODULE_DEVICE_TABLE(of, sensor_actuator_bu64981_match);

static const struct i2c_device_id sensor_actuator_bu64981_idt[] = {
	{ ACTUATOR_NAME, 0 },
	{},
};

static struct i2c_driver sensor_actuator_bu64981_driver = {
	.probe  = sensor_bu64981_actuator_probe,
	.driver = {
		.name	= ACTUATOR_NAME,
		.owner	= THIS_MODULE,
		.of_match_table = sensor_actuator_bu64981_match,
		.suppress_bind_attrs = true,
	},
	.id_table = sensor_actuator_bu64981_idt,
};

#ifdef MODULE
builtin_i2c_driver(sensor_actuator_bu64981_driver);
#else
static int __init sensor_actuator_bu64981_init(void)
{
	int ret;

	ret = i2c_add_driver(&sensor_actuator_bu64981_driver);
	if (ret)
		err("failed to add %s driver: %d\n",
			sensor_actuator_bu64981_driver.driver.name, ret);

	return ret;
}
late_initcall_sync(sensor_actuator_bu64981_init);
#endif
