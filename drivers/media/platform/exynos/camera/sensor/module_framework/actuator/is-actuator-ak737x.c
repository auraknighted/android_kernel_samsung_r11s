...
int sensor_ak737x_actuator_init(struct v4l2_subdev *subdev, u32 val)
{
	...
	if (current_time < module->act_available_time) {
		...
		info("[%s] need to actuator first_i2c_delay : %d[us]\n", __func__, first_i2c_delay);
	}
...
int sensor_ak737x_actuator_get_hall_value(struct v4l2_subdev *subdev, u16 *hall_value)
{
	...
	*hall_value = 0;
	...
	ret = is_sensor_addr8_read8(client, 0x02, &current_mode);
	if (ret) {
	...
	is_sensor_addr8_read8(client, 0x84, &hall_hbyte);
	is_sensor_addr8_read8(client, 0x85, &hall_lbyte);
		...
		const struct i2c_device_id *id)
{
	...
	if (of_property_read_u32(dnode, "vendor_sleep_to_standby_delay", &sleep_to_standby_delay)) {
	...
	if (of_property_read_u32(dnode, "vendor_active_to_standby_delay", &active_to_standby_delay)) {
	...
	if (of_property_read_u32(dnode, "vendor_soft_landing_seqid", &actuator->vendor_soft_landing_seqid)) {
		actuator->vendor_soft_landing_seqid = 0;
		warn("vendor_soft_landing_seqid read is empty");
	}
