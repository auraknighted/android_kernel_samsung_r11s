...
static int __init sensor_module_3m2_probe(struct platform_device *pdev)
{
	...
	ret = is_sensor_module_parse_dt(dev, sensor_module_3m2_power_setpin);
	if (ret)
		return ret;
	...
	module->enum_id = atomic_read(&device->module_count);
	atomic_inc(&device->module_count);
	probe_info("%s pdata->id(%d), module enum_id = %d \n", __func__,
					pdata->id, module->enum_id);

	module = &device->module_enum[module->enum_id];
