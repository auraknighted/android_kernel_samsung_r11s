...
static u32 sensor_hm3_max_load_sram_num;
#endif
...
#if IS_ENABLED(USE_CAMERA_HM3_DEBUG_I2C)
static int sensor_hm3_frame_count;

#define SENSOR_HM3_FRAME_COUNT_LOG_MAX 8
#endif
						...
						cis_shared_data *cis_data)
{
	...
	if (cis_data->cur_hdr_mode == cis_data->pre_hdr_mode
		&& cis_data->cur_hdr_mode == SENSOR_HDR_MODE_2AEB_2VC) {//AEB ON
...
#ifdef USE_SENSOR_TEST_SETTING
	if (is_vener_ts_get_sensor_test_flag(SENSOR_NAME_S5KHM3)) {
		is_vender_ts_get_setting_for_test(SENSOR_NAME_S5KHM3,
			&sensor_hm3_global,
			&sensor_hm3_global_size,
			&sensor_hm3_setfiles,
			&sensor_hm3_setfile_sizes,
			&sensor_hm3_pllinfos);
#if IS_ENABLED(USE_CAMERA_SENSOR_RETENTION)
		is_vender_test_sensor_get_retention_setting_for_test(SENSOR_NAME_S5KHM3,
			&sensor_hm3_global_retention,
			&sensor_hm3_global_retention_size,
			&sensor_hm3_retention,
			&sensor_hm3_retention_size,
			&sensor_hm3_load_sram,
			&sensor_hm3_load_sram_size);
#endif
	}
#endif
...
{
	u16 aeb_check1, aeb_check2, aeb_check3, aeb_check4, aeb_check5, aeb_check6;
	struct is_cis *cis;
	struct i2c_client *client;
	...
	is_sensor_write16(client, 0x602E, 0x0E00);
	is_sensor_read16(client, 0x6F12, &aeb_check1);
	is_sensor_write16(client, 0x602E, 0x0E18);
	is_sensor_read16(client, 0x6F12, &aeb_check2);
	is_sensor_write16(client, 0x602E, 0x0B32);
	is_sensor_read16(client, 0x6F12, &aeb_check3);
	is_sensor_write16(client, 0x602C, 0x2000);
	is_sensor_write16(client, 0x602E, 0x1664);
	is_sensor_read16(client, 0x6F12, &aeb_check4); // 0x20001664
	is_sensor_read16(client, 0x6F12, &aeb_check5); // 0x20001666
	is_sensor_read16(client, 0x6F12, &aeb_check6); // 0x20001668

	if (sensor_hm3_frame_count < SENSOR_HM3_FRAME_COUNT_LOG_MAX)
		info("[%s] 4000p 0E00(%#x), 0E18(%#x), 0B32(%#x), 2000p 1664(%#x), 1666(%#x), 1668(%#x)\n",
				__func__, aeb_check1, aeb_check2, aeb_check3, aeb_check4, aeb_check5, aeb_check6);

	// true : on , false : off
	return (aeb_check1 == (u16)0x0223 && aeb_check4 == (u16)0x0001);
}
...
int sensor_hm3_cis_retention_prepare(struct v4l2_subdev *subdev)
{
	...
	ret |= is_sensor_write16(cis->client, 0xE550, 0x6000);
	ret |= is_sensor_write16(cis->client, 0xFCFC, 0x4000);
...
int sensor_hm3_cis_wait_streamoff(struct v4l2_subdev *subdev)
{
	...
	if (ret < 0) {
		sensor_hm3_cis_log_status(subdev);
		panic("sensor_hm3_cis_wait_streamoff");
	}
#endif
...
int sensor_hm3_cis_stream_on(struct v4l2_subdev *subdev)
{
	...
	if (cis_data->cur_hdr_mode == SENSOR_HDR_MODE_SINGLE
		...
		is_sensor_read16(cis->client, 0x0B30, &fast_change_idx);
...
int sensor_hm3_cis_set_factory_control(struct v4l2_subdev *subdev, u32 command)
{
	...
	case FAC_CTRL_BIT_TEST:
		info("[%s] FAC_CTRL_BIT_TEST to be checked\n", __func__);

		break;
	default:
		info("[%s] not support command(%d)\n", __func__, command);
	}
	...
	const struct i2c_device_id *id)
{
	...
	if (mclk_freq_khz == 19200) {
		if (strcmp(setfile, "default") == 0 ||
			...
			sensor_hm3_max_load_sram_num = ARRAY_SIZE(sensor_hm3_setfile_A_19p2_sizes_load_sram);
#endif
		} else {
		...
		if (strcmp(setfile, "default") == 0 ||
			...
			sensor_hm3_max_load_sram_num = ARRAY_SIZE(sensor_hm3_setfile_A_sizes_load_sram);
#endif
		} else {
...
#ifdef USE_SENSOR_TEST_SETTING
	is_vender_ts_set_default_setting_for_test(SENSOR_NAME_S5KHM3,
		sensor_hm3_global,
		sensor_hm3_global_size,
		sensor_hm3_setfiles,
		sensor_hm3_setfile_sizes,
		(void *)sensor_hm3_pllinfos,
		sensor_hm3_max_setfile_num);

#if IS_ENABLED(USE_CAMERA_SENSOR_RETENTION)
	is_vender_ts_set_default_retention_setting_for_test(SENSOR_NAME_S5KHM3,
		sensor_hm3_global_retention,
		sensor_hm3_global_retention_size,
		sensor_hm3_retention,
		sensor_hm3_retention_size,
		sensor_hm3_max_retention_num,
		sensor_hm3_load_sram,
		sensor_hm3_load_sram_size,
		sensor_hm3_max_load_sram_num);
#endif
#endif
