...
int sensor_gc02m1_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
...
int sensor_gc02m1_set_analog_gain(struct v4l2_subdev *subdev,  struct ae_param *again)
{
	int ret = 0;
	int hold = 0;
	struct is_cis *cis;
	...
	analog_gain = sensor_gc02m1_cis_calc_again_code(again->val);
	analog_permile = sensor_gc02m1_cis_calc_again_permile(analog_gain);
	digital_gain = sensor_gc02m1_cis_calc_dgain_code(again->val, analog_permile);
	...
	/* Digital gain int*/
	ret = is_sensor_addr8_write8(client, 0xb1, digital_gain_b1);
	if (ret < 0)
		goto p_err;
...
int sensor_gc02m1_cis_group_param_hold(struct v4l2_subdev *subdev, bool hold)
{
	...
	if (ret < 0)
		err("sensor_gc02m1_cis_group_param_hold_func fail!!");
...
static struct is_cis_ops cis_ops = {
	...
	.cis_set_exposure_time = sensor_gc02m1_set_exposure_time,
	...
	.cis_set_analog_gain = sensor_gc02m1_set_analog_gain,
	...
	.cis_set_digital_gain = sensor_gc02m1_set_digital_gain,
