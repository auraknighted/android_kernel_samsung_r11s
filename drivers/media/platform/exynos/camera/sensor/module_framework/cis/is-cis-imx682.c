...
#include "is-cis-imx682-setA.h"
#include "is-cis-imx682-setB.h"
...
int sensor_imx682_cis_check_rev(struct is_cis *cis)
{
	int ret = 0;
	u8 status = 0;
	u8 rev = 0;
	...
	is_sensor_write8(client, SENSOR_IMX682_OTP_PAGE_SETUP_ADDR, 0x7F);

	/* Turn ON OTP Read MODE */
	is_sensor_write8(client, SENSOR_IMX682_OTP_READ_TRANSFER_MODE_ADDR,  0x01);

	/* Check status - 0x01 : read ready*/
	is_sensor_read8(client, SENSOR_IMX682_OTP_STATUS_REGISTER_ADDR,  &status);
	if ((status & 0x1) == false)
		err("status fail, (%d)", status);

	/* CHIP REV 0x0018 */
	ret = is_sensor_read8(client, SENSOR_IMX682_OTP_CHIP_REVISION_ADDR, &rev);
	if (ret < 0) {
...
static void sensor_imx682_set_integration_min(u32 mode, cis_shared_data *cis_data)
{
	...
	case SENSOR_IMX682_2X2BIN_V2H2_2304X1296_240FPS:
...
int sensor_imx682_cis_LRC_write(struct v4l2_subdev *subdev)
{
	...
	ret = is_sensor_write8_sequential(client, start_addr1, cal_data, data_size);
	if (ret < 0)
		err("cis_imx682 LRC write Error(%d)\n", ret);

	ret = is_sensor_write8_sequential(client, start_addr2, cal_data + data_size, data_size);
	if (ret < 0)
		err("cis_imx682 LRC write Error(%d)\n", ret);
...
u32 sensor_imx682_get_fine_integration_value(struct is_cis *cis,  u16 *fine_integ)
{
	u32 ret = 0;
	...
	ret = is_sensor_read16(client, SENSOR_IMX682_FINE_INTEG_TIME_ADDR, &fine_integ_time);
	if (ret < 0) {
	...
	if (fine_integ_time > 0)
		...
		*fine_integ = SENSOR_IMX682_FINE_INTEGRATION_TIME;
...
int sensor_imx682_cis_set_seamless_mode_change_param(struct is_cis *cis)
{
	int ret = 0;
...
int sensor_imx682_cis_log_status(struct v4l2_subdev *subdev)
{
	...
	if (unlikely(!ret))
		pr_info("[SEN:INFO]model_id(0x%x)\n", data16);
	else
		goto i2c_err;

	ret = is_sensor_read8(client, SENSOR_IMX682_REVISION_NUM_ADDR, &data8);
	if (unlikely(!ret))
		pr_info("[SEN:INFO]revision_number(0x%x)\n", data8);
	else
		goto i2c_err;

	ret = is_sensor_read8(client, SENSOR_IMX682_FRAME_COUNT_ADDR, &data8);
	if (unlikely(!ret))
		pr_info("[SEN:INFO]frame_count(0x%x)\n", data8);
	else
		goto i2c_err;

	ret = is_sensor_read8(client, SENSOR_IMX682_SETUP_MODE_SELECT_ADDR, &data8);
	if (unlikely(!ret))
		pr_info("[SEN:INFO]setup_mode(streaming)(0x%x)\n", data8);
	else
		goto i2c_err;

	ret = is_sensor_read16(client, SENSOR_IMX682_COARSE_INTEG_TIME_ADDR, &data16);
	if (unlikely(!ret))
		pr_info("[SEN:INFO]coarse_integration_time(0x%x)\n", data16);
	else
		goto i2c_err;

	ret = is_sensor_read16(client, SENSOR_IMX682_ANALOG_GAIN_ADDR, &data16);
	if (unlikely(!ret))
		pr_info("[SEN:INFO]gain_code_global(0x%x)\n", data16);
	else
		goto i2c_err;

	ret = is_sensor_read16(client, SENSOR_IMX682_FRAME_LENGTH_LINE_ADDR, &data16);
	if (unlikely(!ret))
		pr_info("[SEN:INFO]frame_length_line(0x%x)\n", data16);
	else
		goto i2c_err;

	ret = is_sensor_read16(client, SENSOR_IMX682_LINE_LENGTH_PCK_ADDR, &data16);
	if (unlikely(!ret))
		pr_info("[SEN:INFO]line_length_pck(0x%x)\n", data16);
	else
		goto i2c_err;
...
static int sensor_imx682_cis_group_param_hold_func(struct v4l2_subdev *subdev, unsigned int hold)
{
	...
	ret = is_sensor_write8(client, SENSOR_IMX682_GROUP_PARAM_HOLD_ADDR, hold);
	if (ret < 0) {
...
int sensor_imx682_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	...
	is_sensor_write8(client, SENSOR_IMX682_EBD_CONTROL_ADDR, 0x00);
...
int sensor_imx682_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	 ...
	 */
	if (cis_data->is_data.scene_mode == AA_SCENE_MODE_SUPER_NIGHT
...
int sensor_imx682_cis_long_term_exposure(struct v4l2_subdev *subdev)
{
	...
	if (lte_mode->sen_strm_off_on_enable) {
		...
		if (target_exp >= 125000) {
			...
			ret = is_sensor_write8(cis->client, SENSOR_IMX682_CIT_LSHIFT_ADDR, cit_lshift_count);
		}
	} else {
		cit_lshift_count = 0;
		ret = is_sensor_write8(cis->client, SENSOR_IMX682_CIT_LSHIFT_ADDR, cit_lshift_count);
...
static const struct of_device_id sensor_cis_imx682_match[] = {
	{
		.compatible = "samsung,exynos-is-cis-imx682",
	},
	{},
};
...
#ifdef MODULE
builtin_i2c_driver(sensor_cis_imx682_driver);
#else
