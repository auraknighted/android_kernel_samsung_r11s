/*
 * Samsung Exynos5 SoC series Sensor driver
 *
 *
 * Copyright (c) 2016 Samsung Electronics Co., Ltd
 *
 * This program is free software; you can redistribute it and/or modify
 * it under the terms of the GNU General Public License version 2 as
 * published by the Free Software Foundation.
 */

#ifndef IS_CIS_2X5_SET_A_H
#define IS_CIS_2X5_SET_A_H

#include "is-cis.h"
#include "is-cis-2x5.h"

/* Reference : S5K2X5SP03_EVT0_Ver_1.00.xlsx */

const u32 sensor_2x5_setfile_A_Global[] = {
	/*++ Clock Open */
	0xFCFC, 0x4000, 0x02,
	0x6028, 0x4000, 0x02,
	0x602A, 0x6018, 0x02,
	0x6F12, 0x0001, 0x02,
	0x6028, 0x4000, 0x02,
	0x602A, 0x7002, 0x02,
	0x6F12, 0x000C, 0x02,
	0x602A, 0x6014, 0x02,
	0x6F12, 0x0001, 0x02,
	I2C_MODE_DELAY, 8000, 0x00,
	0x602A, 0x6214, 0x02,
	0x6F12, 0xE9F0, 0x02,
	0x602A, 0x6218, 0x02,
	0x6F12, 0xE150, 0x02,
	0x602A, 0x0136, 0x02,
	0x6F12, 0x1800, 0x02,
	/*-- Clock Open */
	/*++ TnP */
	0x6028, 0x2001, 0x02,
	0x602A, 0x2CA8, 0x02,
	0x6F12, 0x0022, 0x02,
	0x6F12, 0x2D49, 0x02,
	0x6F12, 0x2D48, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0xA7FF, 0x02,
	0x6F12, 0x2063, 0x02,
	0x6F12, 0x0022, 0x02,
	0x6F12, 0x2C49, 0x02,
	0x6F12, 0x2C48, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0xA1FF, 0x02,
	0x6F12, 0x6063, 0x02,
	0x6F12, 0x0022, 0x02,
	0x6F12, 0x2B49, 0x02,
	0x6F12, 0x2B48, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0x9BFF, 0x02,
	0x6F12, 0xA063, 0x02,
	0x6F12, 0x0022, 0x02,
	0x6F12, 0x2A49, 0x02,
	0x6F12, 0x2A48, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0x95FF, 0x02,
	0x6F12, 0xE063, 0x02,
	0x6F12, 0x0022, 0x02,
	0x6F12, 0x2949, 0x02,
	0x6F12, 0x2948, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0x8FFF, 0x02,
	0x6F12, 0x2064, 0x02,
	0x6F12, 0xBDE8, 0x02,
	0x6F12, 0x1040, 0x02,
	0x6F12, 0xFBF7, 0x02,
	0x6F12, 0xBABE, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xE857, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0x56F0, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xCA31, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x6C69, 0x02,
	0x6F12, 0x2001, 0x02,
	0x6F12, 0x2400, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xCA7D, 0x02,
	0x6F12, 0x0001, 0x02,
	0x6F12, 0x1C49, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xCAEB, 0x02,
	0x6F12, 0x0001, 0x02,
	0x6F12, 0x8625, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xCBCB, 0x02,
	0x6F12, 0x0001, 0x02,
	0x6F12, 0x7C97, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xCCA9, 0x02,
	0x6F12, 0x0001, 0x02,
	0x6F12, 0x3BF9, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xCD71, 0x02,
	0x6F12, 0x0001, 0x02,
	0x6F12, 0x22D3, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xCEBD, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0xA5C1, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xCF2F, 0x02,
	0x6F12, 0x0002, 0x02,
	0x6F12, 0x13BB, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xDBBD, 0x02,
	0x6F12, 0x0001, 0x02,
	0x6F12, 0xEB6F, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xDD3D, 0x02,
	0x6F12, 0x0001, 0x02,
	0x6F12, 0xE7FD, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xE347, 0x02,
	0x6F12, 0x0001, 0x02,
	0x6F12, 0xEA47, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xE3A1, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0xE3A1, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xE03B, 0x02,
	0x6F12, 0x2001, 0x02,
	0x6F12, 0x2480, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xE3ED, 0x02,
	0x6F12, 0x0001, 0x02,
	0x6F12, 0x9C7D, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xE40D, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0EBD, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xE461, 0x02,
	0x6F12, 0x0001, 0x02,
	0x6F12, 0x2D97, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xE6EB, 0x02,
	0x6F12, 0x0001, 0x02,
	0x6F12, 0xA945, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xE793, 0x02,
	0x6F12, 0x0001, 0x02,
	0x6F12, 0x09E9, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0xEA60, 0x02,
	0x6F12, 0x70B5, 0x02,
	0x6F12, 0x344D, 0x02,
	0x6F12, 0x3248, 0x02,
	0x6F12, 0x0022, 0x02,
	0x6F12, 0xE864, 0x02,
	0x6F12, 0x3348, 0x02,
	0x6F12, 0xA862, 0x02,
	0x6F12, 0x3349, 0x02,
	0x6F12, 0x3348, 0x02,
	0x6F12, 0x04F0, 0x02,
	0x6F12, 0xC5F8, 0x02,
	0x6F12, 0x334C, 0x02,
	0x6F12, 0x0022, 0x02,
	0x6F12, 0x3349, 0x02,
	0x6F12, 0x2067, 0x02,
	0x6F12, 0x3348, 0x02,
	0x6F12, 0x04F0, 0x02,
	0x6F12, 0xBEF8, 0x02,
	0x6F12, 0xA066, 0x02,
	0x6F12, 0x3248, 0x02,
	0x6F12, 0x2863, 0x02,
	0x6F12, 0x0022, 0x02,
	0x6F12, 0x3149, 0x02,
	0x6F12, 0x3248, 0x02,
	0x6F12, 0x04F0, 0x02,
	0x6F12, 0xB6F8, 0x02,
	0x6F12, 0x6066, 0x02,
	0x6F12, 0x3149, 0x02,
	0x6F12, 0x0020, 0x02,
	0x6F12, 0x0246, 0x02,
	0x6F12, 0xA1F8, 0x02,
	0x6F12, 0x8A05, 0x02,
	0x6F12, 0x2F49, 0x02,
	0x6F12, 0x3048, 0x02,
	0x6F12, 0x04F0, 0x02,
	0x6F12, 0xACF8, 0x02,
	0x6F12, 0xE064, 0x02,
	0x6F12, 0x0022, 0x02,
	0x6F12, 0x2E49, 0x02,
	0x6F12, 0x2F48, 0x02,
	0x6F12, 0x04F0, 0x02,
	0x6F12, 0xA6F8, 0x02,
	0x6F12, 0xA064, 0x02,
	0x6F12, 0x0022, 0x02,
	0x6F12, 0x2D49, 0x02,
	0x6F12, 0x2E48, 0x02,
	0x6F12, 0x04F0, 0x02,
	0x6F12, 0xA0F8, 0x02,
	0x6F12, 0x2066, 0x02,
	0x6F12, 0x0022, 0x02,
	0x6F12, 0x2C49, 0x02,
	0x6F12, 0x2D48, 0x02,
	0x6F12, 0x04F0, 0x02,
	0x6F12, 0x9AF8, 0x02,
	0x6F12, 0xE065, 0x02,
	0x6F12, 0x2C48, 0x02,
	0x6F12, 0x0022, 0x02,
	0x6F12, 0x00F2, 0x02,
	0x6F12, 0xE455, 0x02,
	0x6F12, 0xB0F8, 0x02,
	0x6F12, 0xE465, 0x02,
	0x6F12, 0x0620, 0x02,
	0x6F12, 0x2880, 0x02,
	0x6F12, 0x2949, 0x02,
	0x6F12, 0x2948, 0x02,
	0x6F12, 0x04F0, 0x02,
	0x6F12, 0x8DF8, 0x02,
	0x6F12, 0x6065, 0x02,
	0x6F12, 0x0320, 0x02,
	0x6F12, 0x2880, 0x02,
	0x6F12, 0x0022, 0x02,
	0x6F12, 0x2749, 0x02,
	0x6F12, 0x2748, 0x02,
	0x6F12, 0x04F0, 0x02,
	0x6F12, 0x85F8, 0x02,
	0x6F12, 0x6061, 0x02,
	0x6F12, 0x2749, 0x02,
	0x6F12, 0x2E80, 0x02,
	0x6F12, 0x2548, 0x02,
	0x6F12, 0xC1F8, 0x02,
	0x6F12, 0x9C00, 0x02,
	0x6F12, 0x0022, 0x02,
	0x6F12, 0x2549, 0x02,
	0x6F12, 0x2648, 0x02,
	0x6F12, 0x04F0, 0x02,
	0x6F12, 0x7AF8, 0x02,
	0x6F12, 0x1C49, 0x02,
	0x6F12, 0x2065, 0x02,
	0x6F12, 0x2448, 0x02,
	0x6F12, 0xC1F8, 0x02,
	0x6F12, 0xDC05, 0x02,
	0x6F12, 0x2449, 0x02,
	0x6F12, 0x081A, 0x02,
	0x6F12, 0x1949, 0x02,
	0x6F12, 0xA1F8, 0x02,
	0x6F12, 0xE005, 0x02,
	0x6F12, 0x42F2, 0x02,
	0x6F12, 0x6E20, 0x02,
	0x6F12, 0xC1F8, 0x02,
	0x6F12, 0xB805, 0x02,
	0x6F12, 0x2049, 0x02,
	0x6F12, 0x0968, 0x02,
	0x6F12, 0x4883, 0x02,
	0x6F12, 0x70BD, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xFBC7, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0x56F0, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xFE0D, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xF783, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0xDFCD, 0x02,
	0x6F12, 0x2001, 0x02,
	0x6F12, 0x2400, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xF357, 0x02,
	0x6F12, 0x0001, 0x02,
	0x6F12, 0x7D09, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xF749, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xF3E3, 0x02,
	0x6F12, 0x0001, 0x02,
	0x6F12, 0xD5E5, 0x02,
	0x6F12, 0x2001, 0x02,
	0x6F12, 0x2520, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xF1C5, 0x02,
	0x6F12, 0x0001, 0x02,
	0x6F12, 0x770F, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xF049, 0x02,
	0x6F12, 0x0001, 0x02,
	0x6F12, 0x7589, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xEEED, 0x02,
	0x6F12, 0x0001, 0x02,
	0x6F12, 0xC36D, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xED7D, 0x02,
	0x6F12, 0x0001, 0x02,
	0x6F12, 0xC9D9, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0x6210, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xED73, 0x02,
	0x6F12, 0x0001, 0x02,
	0x6F12, 0x44C3, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xECA5, 0x02,
	0x6F12, 0x0001, 0x02,
	0x6F12, 0x7C97, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xEBE5, 0x02,
	0x6F12, 0x2001, 0x02,
	0x6F12, 0x2480, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xEBB1, 0x02,
	0x6F12, 0x0002, 0x02,
	0x6F12, 0x1821, 0x02,
	0x6F12, 0x2001, 0x02,
	0x6F12, 0x00E4, 0x02,
	0x6F12, 0x2001, 0x02,
	0x6F12, 0x2400, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0x0AE0, 0x02,
	0x6F12, 0x70B5, 0x02,
	0x6F12, 0xFE48, 0x02,
	0x6F12, 0x0022, 0x02,
	0x6F12, 0x016D, 0x02,
	0x6F12, 0x0C0C, 0x02,
	0x6F12, 0x8DB2, 0x02,
	0x6F12, 0x2946, 0x02,
	0x6F12, 0x2046, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0x7CFE, 0x02,
	0x6F12, 0xFA48, 0x02,
	0x6F12, 0x90F8, 0x02,
	0x6F12, 0x8405, 0x02,
	0x6F12, 0x10B1, 0x02,
	0x6F12, 0xF94A, 0x02,
	0x6F12, 0x0220, 0x02,
	0x6F12, 0x1080, 0x02,
	0x6F12, 0x01F0, 0x02,
	0x6F12, 0xDFF9, 0x02,
	0x6F12, 0x2946, 0x02,
	0x6F12, 0x2046, 0x02,
	0x6F12, 0xBDE8, 0x02,
	0x6F12, 0x7040, 0x02,
	0x6F12, 0x0122, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0x6CBE, 0x02,
	0x6F12, 0x30B5, 0x02,
	0x6F12, 0xF44C, 0x02,
	0x6F12, 0x0D46, 0x02,
	0x6F12, 0x2DED, 0x02,
	0x6F12, 0x028B, 0x02,
	0x6F12, 0x83B0, 0x02,
	0x6F12, 0x0889, 0x02,
	0x6F12, 0xB0F5, 0x02,
	0x6F12, 0x807F, 0x02,
	0x6F12, 0x03D1, 0x02,
	0x6F12, 0xED48, 0x02,
	0x6F12, 0x90F8, 0x02,
	0x6F12, 0x1705, 0x02,
	0x6F12, 0xC8B3, 0x02,
	0x6F12, 0x94F8, 0x02,
	0x6F12, 0x3300, 0x02,
	0x6F12, 0x18B9, 0x02,
	0x6F12, 0x0021, 0x02,
	0x6F12, 0x3B20, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0xB1FE, 0x02,
	0x6F12, 0xE848, 0x02,
	0x6F12, 0x90F8, 0x02,
	0x6F12, 0x1705, 0x02,
	0x6F12, 0xDFED, 0x02,
	0x6F12, 0xE98A, 0x02,
	0x6F12, 0x68B3, 0x02,
	0x6F12, 0xE948, 0x02,
	0x6F12, 0xE949, 0x02,
	0x6F12, 0x0823, 0x02,
	0x6F12, 0x0068, 0x02,
	0x6F12, 0xB1F8, 0x02,
	0x6F12, 0x6412, 0x02,
	0x6F12, 0x4843, 0x02,
	0x6F12, 0x020B, 0x02,
	0x6F12, 0x01A8, 0x02,
	0x6F12, 0x0090, 0x02,
	0x6F12, 0x0521, 0x02,
	0x6F12, 0xE548, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0xACFE, 0x02,
	0x6F12, 0xDE49, 0x02,
	0x6F12, 0x0844, 0x02,
	0x6F12, 0x90F8, 0x02,
	0x6F12, 0x2215, 0x02,
	0x6F12, 0x00EE, 0x02,
	0x6F12, 0x101A, 0x02,
	0x6F12, 0xF8EE, 0x02,
	0x6F12, 0x400A, 0x02,
	0x6F12, 0x9FED, 0x02,
	0x6F12, 0xE10A, 0x02,
	0x6F12, 0x90F8, 0x02,
	0x6F12, 0x2305, 0x02,
	0x6F12, 0x20EE, 0x02,
	0x6F12, 0x808A, 0x02,
	0x6F12, 0x00EE, 0x02,
	0x6F12, 0x900A, 0x02,
	0x6F12, 0xF8EE, 0x02,
	0x6F12, 0x600A, 0x02,
	0x6F12, 0x20EE, 0x02,
	0x6F12, 0x800A, 0x02,
	0x6F12, 0xDDED, 0x02,
	0x6F12, 0x010A, 0x02,
	0x6F12, 0xF8EE, 0x02,
	0x6F12, 0x600A, 0x02,
	0x6F12, 0x30EE, 0x02,
	0x6F12, 0x480A, 0x02,
	0x6F12, 0x60EE, 0x02,
	0x6F12, 0xA80A, 0x02,
	0x6F12, 0x00EE, 0x02,
	0x6F12, 0x208A, 0x02,
	0x6F12, 0x08E0, 0x02,
	0x6F12, 0x0FE0, 0x02,
	0x6F12, 0xFFE7, 0x02,
	0x6F12, 0x2889, 0x02,
	0x6F12, 0x00EE, 0x02,
	0x6F12, 0x100A, 0x02,
	0x6F12, 0xB8EE, 0x02,
	0x6F12, 0x400A, 0x02,
	0x6F12, 0x20EE, 0x02,
	0x6F12, 0x288A, 0x02,
	0x6F12, 0xB0EE, 0x02,
	0x6F12, 0x480A, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0xCDF8, 0x02,
	0x6F12, 0xB0EE, 0x02,
	0x6F12, 0x480A, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0x71F9, 0x02,
	0x6F12, 0x0020, 0x02,
	0x6F12, 0x84F8, 0x02,
	0x6F12, 0x3300, 0x02,
	0x6F12, 0x03B0, 0x02,
	0x6F12, 0xBDEC, 0x02,
	0x6F12, 0x028B, 0x02,
	0x6F12, 0x30BD, 0x02,
	0x6F12, 0x2DE9, 0x02,
	0x6F12, 0xF35F, 0x02,
	0x6F12, 0xC949, 0x02,
	0x6F12, 0x8146, 0x02,
	0x6F12, 0x49F2, 0x02,
	0x6F12, 0x0D00, 0x02,
	0x6F12, 0xDFF8, 0x02,
	0x6F12, 0x10A3, 0x02,
	0x6F12, 0xA1F8, 0x02,
	0x6F12, 0x5200, 0x02,
	0x6F12, 0x0024, 0x02,
	0x6F12, 0x41F6, 0x02,
	0x6F12, 0x4C1B, 0x02,
	0x6F12, 0x0AEB, 0x02,
	0x6F12, 0x0401, 0x02,
	0x6F12, 0x5646, 0x02,
	0x6F12, 0x11F8, 0x02,
	0x6F12, 0x0B00, 0x02,
	0x6F12, 0x78BB, 0x02,
	0x6F12, 0xC248, 0x02,
	0x6F12, 0x0568, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0x04FE, 0x02,
	0x6F12, 0x0228, 0x02,
	0x6F12, 0x02D0, 0x02,
	0x6F12, 0x0328, 0x02,
	0x6F12, 0x05D0, 0x02,
	0x6F12, 0x08E0, 0x02,
	0x6F12, 0x06EB, 0x02,
	0x6F12, 0x8401, 0x02,
	0x6F12, 0xD1F8, 0x02,
	0x6F12, 0x2852, 0x02,
	0x6F12, 0x03E0, 0x02,
	0x6F12, 0x06EB, 0x02,
	0x6F12, 0x8401, 0x02,
	0x6F12, 0xD1F8, 0x02,
	0x6F12, 0x1C52, 0x02,
	0x6F12, 0xC4EB, 0x02,
	0x6F12, 0x0411, 0x02,
	0x6F12, 0x01EB, 0x02,
	0x6F12, 0x4102, 0x02,
	0x6F12, 0x09EB, 0x02,
	0x6F12, 0xC206, 0x02,
	0x6F12, 0x331D, 0x02,
	0x6F12, 0xB049, 0x02,
	0x6F12, 0x0128, 0x02,
	0x6F12, 0x0FD9, 0x02,
	0x6F12, 0xA1F1, 0x02,
	0x6F12, 0xF600, 0x02,
	0x6F12, 0x0078, 0x02,
	0x6F12, 0x58B1, 0x02,
	0x6F12, 0x01EB, 0x02,
	0x6F12, 0x0417, 0x02,
	0x6F12, 0x0021, 0x02,
	0x6F12, 0x4FF4, 0x02,
	0x6F12, 0x804C, 0x02,
	0x6F12, 0x57F8, 0x02,
	0x6F12, 0x2100, 0x02,
	0x6F12, 0x4000, 0x02,
	0x6F12, 0x6045, 0x02,
	0x6F12, 0x06D2, 0x02,
	0x6F12, 0x0022, 0x02,
	0x6F12, 0x09E0, 0x02,
	0x6F12, 0x1846, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0xE3FD, 0x02,
	0x6F12, 0x11E0, 0x02,
	0x6F12, 0x19E0, 0x02,
	0x6F12, 0xB0FA, 0x02,
	0x6F12, 0x80F2, 0x02,
	0x6F12, 0xC2F1, 0x02,
	0x6F12, 0x1202, 0x02,
	0x6F12, 0xD2B2, 0x02,
	0x6F12, 0x03EB, 0x02,
	0x6F12, 0x0108, 0x02,
	0x6F12, 0xD040, 0x02,
	0x6F12, 0x88F8, 0x02,
	0x6F12, 0x6021, 0x02,
	0x6F12, 0x03EB, 0x02,
	0x6F12, 0x8102, 0x02,
	0x6F12, 0x491C, 0x02,
	0x6F12, 0xC2F8, 0x02,
	0x6F12, 0x4001, 0x02,
	0x6F12, 0x0429, 0x02,
	0x6F12, 0xE2D3, 0x02,
	0x6F12, 0xA248, 0x02,
	0x6F12, 0x2A46, 0x02,
	0x6F12, 0x00EB, 0x02,
	0x6F12, 0xC400, 0x02,
	0x6F12, 0xD0F8, 0x02,
	0x6F12, 0x8010, 0x02,
	0x6F12, 0x301D, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0xCCFD, 0x02,
	0x6F12, 0x641C, 0x02,
	0x6F12, 0x032C, 0x02,
	0x6F12, 0xABD3, 0x02,
	0x6F12, 0x4846, 0x02,
	0x6F12, 0x0199, 0x02,
	0x6F12, 0xBDE8, 0x02,
	0x6F12, 0xFC5F, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0xC8BD, 0x02,
	0x6F12, 0x01F1, 0x02,
	0x6F12, 0x8041, 0x02,
	0x6F12, 0x0120, 0x02,
	0x6F12, 0x0880, 0x02,
	0x6F12, 0x7047, 0x02,
	0x6F12, 0x2DE9, 0x02,
	0x6F12, 0xF043, 0x02,
	0x6F12, 0xDFF8, 0x02,
	0x6F12, 0x5892, 0x02,
	0x6F12, 0x87B0, 0x02,
	0x6F12, 0x0C46, 0x02,
	0x6F12, 0x99F8, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0028, 0x02,
	0x6F12, 0x68D1, 0x02,
	0x6F12, 0x8C48, 0x02,
	0x6F12, 0x90F8, 0x02,
	0x6F12, 0xE808, 0x02,
	0x6F12, 0x0028, 0x02,
	0x6F12, 0x63D1, 0x02,
	0x6F12, 0x914D, 0x02,
	0x6F12, 0x09F1, 0x02,
	0x6F12, 0x0406, 0x02,
	0x6F12, 0x4946, 0x02,
	0x6F12, 0x3768, 0x02,
	0x6F12, 0xE86A, 0x02,
	0x6F12, 0x3060, 0x02,
	0x6F12, 0x6846, 0x02,
	0x6F12, 0x01F0, 0x02,
	0x6F12, 0xF8F8, 0x02,
	0x6F12, 0x8449, 0x02,
	0x6F12, 0x6869, 0x02,
	0x6F12, 0x2D88, 0x02,
	0x6F12, 0x0968, 0x02,
	0x6F12, 0x4843, 0x02,
	0x6F12, 0x000A, 0x02,
	0x6F12, 0x4543, 0x02,
	0x6F12, 0x07B9, 0x02,
	0x6F12, 0x2F46, 0x02,
	0x6F12, 0x00EE, 0x02,
	0x6F12, 0x105A, 0x02,
	0x6F12, 0xF8EE, 0x02,
	0x6F12, 0x400A, 0x02,
	0x6F12, 0x00EE, 0x02,
	0x6F12, 0x107A, 0x02,
	0x6F12, 0xB8EE, 0x02,
	0x6F12, 0x401A, 0x02,
	0x6F12, 0x80EE, 0x02,
	0x6F12, 0x810A, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0xEAFD, 0x02,
	0x6F12, 0x7548, 0x02,
	0x6F12, 0xF7EE, 0x02,
	0x6F12, 0x000A, 0x02,
	0x6F12, 0xB0F8, 0x02,
	0x6F12, 0x6805, 0x02,
	0x6F12, 0x30EE, 0x02,
	0x6F12, 0x601A, 0x02,
	0x6F12, 0x00EE, 0x02,
	0x6F12, 0x100A, 0x02,
	0x6F12, 0xB8EE, 0x02,
	0x6F12, 0x400A, 0x02,
	0x6F12, 0xDFED, 0x02,
	0x6F12, 0x731A, 0x02,
	0x6F12, 0x0020, 0x02,
	0x6F12, 0x40F6, 0x02,
	0x6F12, 0xFF77, 0x02,
	0x6F12, 0x20EE, 0x02,
	0x6F12, 0x210A, 0x02,
	0x6F12, 0xF6EE, 0x02,
	0x6F12, 0x001A, 0x02,
	0x6F12, 0xEC46, 0x02,
	0x6F12, 0x41EE, 0x02,
	0x6F12, 0x000A, 0x02,
	0x6F12, 0x0CEB, 0x02,
	0x6F12, 0x4003, 0x02,
	0x6F12, 0xDA88, 0x02,
	0x6F12, 0x00EE, 0x02,
	0x6F12, 0x102A, 0x02,
	0x6F12, 0xB8EE, 0x02,
	0x6F12, 0x401A, 0x02,
	0x6F12, 0xB0EE, 0x02,
	0x6F12, 0x610A, 0x02,
	0x6F12, 0x01EE, 0x02,
	0x6F12, 0x200A, 0x02,
	0x6F12, 0xBCEE, 0x02,
	0x6F12, 0xC00A, 0x02,
	0x6F12, 0x10EE, 0x02,
	0x6F12, 0x101A, 0x02,
	0x6F12, 0x1FFA, 0x02,
	0x6F12, 0x81F8, 0x02,
	0x6F12, 0x3946, 0x02,
	0x6F12, 0x8845, 0x02,
	0x6F12, 0x0CD2, 0x02,
	0x6F12, 0x00EE, 0x02,
	0x6F12, 0x102A, 0x02,
	0x6F12, 0xB8EE, 0x02,
	0x6F12, 0x401A, 0x02,
	0x6F12, 0xB0EE, 0x02,
	0x6F12, 0x610A, 0x02,
	0x6F12, 0x01EE, 0x02,
	0x6F12, 0x200A, 0x02,
	0x6F12, 0xBCEE, 0x02,
	0x6F12, 0xC00A, 0x02,
	0x6F12, 0x10EE, 0x02,
	0x6F12, 0x101A, 0x02,
	0x6F12, 0x89B2, 0x02,
	0x6F12, 0x401C, 0x02,
	0x6F12, 0xD980, 0x02,
	0x6F12, 0x0828, 0x02,
	0x6F12, 0xDADB, 0x02,
	0x6F12, 0x6148, 0x02,
	0x6F12, 0x99F8, 0x02,
	0x6F12, 0x0010, 0x02,
	0x6F12, 0xB0F8, 0x02,
	0x6F12, 0x4425, 0x02,
	0x6F12, 0x6046, 0x02,
	0x6F12, 0x01F0, 0x02,
	0x6F12, 0xA4F8, 0x02,
	0x6F12, 0x3560, 0x02,
	0x6F12, 0x0120, 0x02,
	0x6F12, 0x01F0, 0x02,
	0x6F12, 0xA5F8, 0x02,
	0x6F12, 0x0121, 0x02,
	0x6F12, 0x5A20, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0x6CFD, 0x02,
	0x6F12, 0x0121, 0x02,
	0x6F12, 0x5C20, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0x68FD, 0x02,
	0x6F12, 0xDFF8, 0x02,
	0x6F12, 0x6881, 0x02,
	0x6F12, 0xDFF8, 0x02,
	0x6F12, 0x68C1, 0x02,
	0x6F12, 0x0022, 0x02,
	0x6F12, 0x4FF6, 0x02,
	0x6F12, 0xFF79, 0x02,
	0x6F12, 0xD007, 0x02,
	0x6F12, 0x01D0, 0x02,
	0x6F12, 0x6546, 0x02,
	0x6F12, 0x00E0, 0x02,
	0x6F12, 0x4546, 0x02,
	0x6F12, 0x02EB, 0x02,
	0x6F12, 0xD270, 0x02,
	0x6F12, 0x4110, 0x02,
	0x6F12, 0x09EA, 0x02,
	0x6F12, 0x4117, 0x02,
	0x6F12, 0x0020, 0x02,
	0x6F12, 0x8100, 0x02,
	0x6F12, 0x01EB, 0x02,
	0x6F12, 0x4211, 0x02,
	0x6F12, 0x04EB, 0x02,
	0x6F12, 0x4103, 0x02,
	0x6F12, 0x03F6, 0x02,
	0x6F12, 0x0403, 0x02,
	0x6F12, 0x07EB, 0x02,
	0x6F12, 0x8001, 0x02,
	0x6F12, 0xDE88, 0x02,
	0x6F12, 0x25F8, 0x02,
	0x6F12, 0x1160, 0x02,
	0x6F12, 0x05EB, 0x02,
	0x6F12, 0x4101, 0x02,
	0x6F12, 0x9E88, 0x02,
	0x6F12, 0x4E80, 0x02,
	0x6F12, 0x5E88, 0x02,
	0x6F12, 0x8E80, 0x02,
	0x6F12, 0x1B88, 0x02,
	0x6F12, 0x401C, 0x02,
	0x6F12, 0xCB80, 0x02,
	0x6F12, 0x0828, 0x02,
	0x6F12, 0xE8DB, 0x02,
	0x6F12, 0x521C, 0x02,
	0x6F12, 0x202A, 0x02,
	0x6F12, 0xDADB, 0x02,
	0x6F12, 0x0021, 0x02,
	0x6F12, 0x5A20, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0x38FD, 0x02,
	0x6F12, 0x0021, 0x02,
	0x6F12, 0x5C20, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0x34FD, 0x02,
	0x6F12, 0x07B0, 0x02,
	0x6F12, 0x0020, 0x02,
	0x6F12, 0xBDE8, 0x02,
	0x6F12, 0xF043, 0x02,
	0x6F12, 0x01F0, 0x02,
	0x6F12, 0x63B8, 0x02,
	0x6F12, 0x2DE9, 0x02,
	0x6F12, 0xF041, 0x02,
	0x6F12, 0x0E46, 0x02,
	0x6F12, 0x2DED, 0x02,
	0x6F12, 0x048B, 0x02,
	0x6F12, 0x0746, 0x02,
	0x6F12, 0x1446, 0x02,
	0x6F12, 0x9FED, 0x02,
	0x6F12, 0x3C9A, 0x02,
	0x6F12, 0x2C4D, 0x02,
	0x6F12, 0x3B49, 0x02,
	0x6F12, 0x0020, 0x02,
	0x6F12, 0x05EB, 0x02,
	0x6F12, 0x4002, 0x02,
	0x6F12, 0x04EB, 0x02,
	0x6F12, 0x4003, 0x02,
	0x6F12, 0xB2F8, 0x02,
	0x6F12, 0x2825, 0x02,
	0x6F12, 0xA3F8, 0x02,
	0x6F12, 0x4020, 0x02,
	0x6F12, 0x01EB, 0x02,
	0x6F12, 0x4002, 0x02,
	0x6F12, 0xB2F8, 0x02,
	0x6F12, 0x8C23, 0x02,
	0x6F12, 0x24F8, 0x02,
	0x6F12, 0x1020, 0x02,
	0x6F12, 0x401C, 0x02,
	0x6F12, 0x2028, 0x02,
	0x6F12, 0xEEDB, 0x02,
	0x6F12, 0x2146, 0x02,
	0x6F12, 0x3046, 0x02,
	0x6F12, 0x01F0, 0x02,
	0x6F12, 0x47F8, 0x02,
	0x6F12, 0xF0EE, 0x02,
	0x6F12, 0x408A, 0x02,
	0x6F12, 0x2146, 0x02,
	0x6F12, 0x3846, 0x02,
	0x6F12, 0x01F0, 0x02,
	0x6F12, 0x41F8, 0x02,
	0x6F12, 0xA888, 0x02,
	0x6F12, 0xB0EE, 0x02,
	0x6F12, 0x408A, 0x02,
	0x6F12, 0x08B3, 0x02,
	0x6F12, 0x00EE, 0x02,
	0x6F12, 0x106A, 0x02,
	0x6F12, 0x2046, 0x02,
	0x6F12, 0xB8EE, 0x02,
	0x6F12, 0x400A, 0x02,
	0x6F12, 0x01F0, 0x02,
	0x6F12, 0x3BF8, 0x02,
	0x6F12, 0xB4F8, 0x02,
	0x6F12, 0x8000, 0x02,
	0x6F12, 0x01EE, 0x02,
	0x6F12, 0x106A, 0x02,
	0x6F12, 0x00EE, 0x02,
	0x6F12, 0x900A, 0x02,
	0x6F12, 0xB8EE, 0x02,
	0x6F12, 0x411A, 0x02,
	0x6F12, 0xF8EE, 0x02,
	0x6F12, 0x600A, 0x02,
	0x6F12, 0x20EE, 0x02,
	0x6F12, 0x200A, 0x02,
	0x6F12, 0xDFED, 0x02,
	0x6F12, 0x220A, 0x02,
	0x6F12, 0x60EE, 0x02,
	0x6F12, 0x200A, 0x02,
	0x6F12, 0xB0EE, 0x02,
	0x6F12, 0x680A, 0x02,
	0x6F12, 0x01EE, 0x02,
	0x6F12, 0x600A, 0x02,
	0x6F12, 0x01EE, 0x02,
	0x6F12, 0x107A, 0x02,
	0x6F12, 0xB8EE, 0x02,
	0x6F12, 0x411A, 0x02,
	0x6F12, 0x01EE, 0x02,
	0x6F12, 0x200A, 0x02,
	0x6F12, 0x06E0, 0x02,
	0x6F12, 0x2246, 0x02,
	0x6F12, 0x3946, 0x02,
	0x6F12, 0xB0EE, 0x02,
	0x6F12, 0x680A, 0x02,
	0x6F12, 0x3046, 0x02,
	0x6F12, 0x01F0, 0x02,
	0x6F12, 0x1EF8, 0x02,
	0x6F12, 0xDFED, 0x02,
	0x6F12, 0x180A, 0x02,
	0x6F12, 0x78EE, 0x02,
	0x6F12, 0x200A, 0x02,
	0x6F12, 0xF4EE, 0x02,
	0x6F12, 0xC00A, 0x02,
	0x6F12, 0xF1EE, 0x02,
	0x6F12, 0x10FA, 0x02,
	0x6F12, 0x01D2, 0x02,
	0x6F12, 0xB0EE, 0x02,
	0x6F12, 0x600A, 0x02,
	0x6F12, 0x27E0, 0x02,
	0x6F12, 0x2001, 0x02,
	0x6F12, 0x2400, 0x02,
	0x6F12, 0x2001, 0x02,
	0x6F12, 0x2520, 0x02,
	0x6F12, 0x4000, 0x02,
	0x6F12, 0xF24C, 0x02,
	0x6F12, 0x2001, 0x02,
	0x6F12, 0x2480, 0x02,
	0x6F12, 0x3B80, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xA5F8, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0x8BA0, 0x02,
	0x6F12, 0x2001, 0x02,
	0x6F12, 0x2A38, 0x02,
	0x6F12, 0x3C00, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0x69F0, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xC43C, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0x7FB0, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xA498, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0x8E04, 0x02,
	0x6F12, 0x2008, 0x02,
	0x6F12, 0xE000, 0x02,
	0x6F12, 0x2008, 0x02,
	0x6F12, 0xE800, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0x7A00, 0x02,
	0x6F12, 0x3780, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x451C, 0x02,
	0x6F12, 0x4000, 0x02,
	0x6F12, 0xB5EE, 0x02,
	0x6F12, 0xC00A, 0x02,
	0x6F12, 0xF1EE, 0x02,
	0x6F12, 0x10FA, 0x02,
	0x6F12, 0x02D2, 0x02,
	0x6F12, 0xF0EE, 0x02,
	0x6F12, 0x490A, 0x02,
	0x6F12, 0x01E0, 0x02,
	0x6F12, 0xF0EE, 0x02,
	0x6F12, 0x400A, 0x02,
	0x6F12, 0x10EE, 0x02,
	0x6F12, 0x900A, 0x02,
	0x6F12, 0xB0F1, 0x02,
	0x6F12, 0x8B4F, 0x02,
	0x6F12, 0x07DA, 0x02,
	0x6F12, 0xB5EE, 0x02,
	0x6F12, 0xC00A, 0x02,
	0x6F12, 0xF1EE, 0x02,
	0x6F12, 0x10FA, 0x02,
	0x6F12, 0x04D2, 0x02,
	0x6F12, 0xB0EE, 0x02,
	0x6F12, 0x490A, 0x02,
	0x6F12, 0x01E0, 0x02,
	0x6F12, 0x9FED, 0x02,
	0x6F12, 0xFE0A, 0x02,
	0x6F12, 0xF6EE, 0x02,
	0x6F12, 0x000A, 0x02,
	0x6F12, 0x30EE, 0x02,
	0x6F12, 0x200A, 0x02,
	0x6F12, 0xBCEE, 0x02,
	0x6F12, 0xC00A, 0x02,
	0x6F12, 0x10EE, 0x02,
	0x6F12, 0x100A, 0x02,
	0x6F12, 0x80B2, 0x02,
	0x6F12, 0xBDEC, 0x02,
	0x6F12, 0x048B, 0x02,
	0x6F12, 0xBDE8, 0x02,
	0x6F12, 0xF081, 0x02,
	0x6F12, 0x2DE9, 0x02,
	0x6F12, 0xF047, 0x02,
	0x6F12, 0x0646, 0x02,
	0x6F12, 0xF648, 0x02,
	0x6F12, 0x0F46, 0x02,
	0x6F12, 0x2DED, 0x02,
	0x6F12, 0x028B, 0x02,
	0x6F12, 0x1546, 0x02,
	0x6F12, 0x806C, 0x02,
	0x6F12, 0x1C46, 0x02,
	0x6F12, 0x4FEA, 0x02,
	0x6F12, 0x1049, 0x02,
	0x6F12, 0x80B2, 0x02,
	0x6F12, 0x8046, 0x02,
	0x6F12, 0x0146, 0x02,
	0x6F12, 0x0022, 0x02,
	0x6F12, 0x4846, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0x27FC, 0x02,
	0x6F12, 0x2346, 0x02,
	0x6F12, 0x2A46, 0x02,
	0x6F12, 0x3946, 0x02,
	0x6F12, 0x3046, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0xB0FF, 0x02,
	0x6F12, 0xEC48, 0x02,
	0x6F12, 0xD0F8, 0x02,
	0x6F12, 0x7412, 0x02,
	0x6F12, 0xD0F8, 0x02,
	0x6F12, 0x6C02, 0x02,
	0x6F12, 0x4143, 0x02,
	0x6F12, 0x0A0B, 0x02,
	0x6F12, 0x1DB1, 0x02,
	0x6F12, 0x6A43, 0x02,
	0x6F12, 0x02F5, 0x02,
	0x6F12, 0x0062, 0x02,
	0x6F12, 0x120B, 0x02,
	0x6F12, 0x06F1, 0x02,
	0x6F12, 0x2001, 0x02,
	0x6F12, 0x04F1, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0x23FC, 0x02,
	0x6F12, 0x97F8, 0x02,
	0x6F12, 0x8002, 0x02,
	0x6F12, 0x0028, 0x02,
	0x6F12, 0x4ED0, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0x18FC, 0x02,
	0x6F12, 0x0228, 0x02,
	0x6F12, 0x4AD3, 0x02,
	0x6F12, 0x0026, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0x13FC, 0x02,
	0x6F12, 0x0328, 0x02,
	0x6F12, 0x46D0, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0x94FF, 0x02,
	0x6F12, 0x050A, 0x02,
	0x6F12, 0xB5FA, 0x02,
	0x6F12, 0x85F0, 0x02,
	0x6F12, 0xC0F1, 0x02,
	0x6F12, 0x2001, 0x02,
	0x6F12, 0x0929, 0x02,
	0x6F12, 0x00D2, 0x02,
	0x6F12, 0x0921, 0x02,
	0x6F12, 0xB6FA, 0x02,
	0x6F12, 0x86F0, 0x02,
	0x6F12, 0x0939, 0x02,
	0x6F12, 0xC0F1, 0x02,
	0x6F12, 0x2005, 0x02,
	0x6F12, 0x092D, 0x02,
	0x6F12, 0x00D2, 0x02,
	0x6F12, 0x0925, 0x02,
	0x6F12, 0x093D, 0x02,
	0x6F12, 0xD44F, 0x02,
	0x6F12, 0x0020, 0x02,
	0x6F12, 0x01EB, 0x02,
	0x6F12, 0x8106, 0x02,
	0x6F12, 0x00EB, 0x02,
	0x6F12, 0xC001, 0x02,
	0x6F12, 0x01EB, 0x02,
	0x6F12, 0x0011, 0x02,
	0x6F12, 0x07EB, 0x02,
	0x6F12, 0x4101, 0x02,
	0x6F12, 0x01EB, 0x02,
	0x6F12, 0x4601, 0x02,
	0x6F12, 0x01EB, 0x02,
	0x6F12, 0x4501, 0x02,
	0x6F12, 0x54F8, 0x02,
	0x6F12, 0x2020, 0x02,
	0x6F12, 0xB1F8, 0x02,
	0x6F12, 0x6012, 0x02,
	0x6F12, 0x4A43, 0x02,
	0x6F12, 0x110B, 0x02,
	0x6F12, 0x44F8, 0x02,
	0x6F12, 0x2010, 0x02,
	0x6F12, 0x401C, 0x02,
	0x6F12, 0x0428, 0x02,
	0x6F12, 0xEAD3, 0x02,
	0x6F12, 0x0020, 0x02,
	0x6F12, 0x00EB, 0x02,
	0x6F12, 0xC001, 0x02,
	0x6F12, 0x01EB, 0x02,
	0x6F12, 0x0011, 0x02,
	0x6F12, 0x07EB, 0x02,
	0x6F12, 0x4101, 0x02,
	0x6F12, 0x01EB, 0x02,
	0x6F12, 0x4601, 0x02,
	0x6F12, 0x01EB, 0x02,
	0x6F12, 0x4501, 0x02,
	0x6F12, 0xB1F8, 0x02,
	0x6F12, 0xF023, 0x02,
	0x6F12, 0x04EB, 0x02,
	0x6F12, 0x8001, 0x02,
	0x6F12, 0x401C, 0x02,
	0x6F12, 0x0B69, 0x02,
	0x6F12, 0x5343, 0x02,
	0x6F12, 0x1A0B, 0x02,
	0x6F12, 0x0A61, 0x02,
	0x6F12, 0x0428, 0x02,
	0x6F12, 0xEAD3, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0xCDFB, 0x02,
	0x6F12, 0x0328, 0x02,
	0x6F12, 0x20D0, 0x02,
	0x6F12, 0x35E0, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0x4DFF, 0x02,
	0x6F12, 0x00EE, 0x02,
	0x6F12, 0x100A, 0x02,
	0x6F12, 0xB8EE, 0x02,
	0x6F12, 0x400A, 0x02,
	0x6F12, 0x5FED, 0x02,
	0x6F12, 0x588A, 0x02,
	0x6F12, 0x20EE, 0x02,
	0x6F12, 0x288A, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0x48FF, 0x02,
	0x6F12, 0x00EE, 0x02,
	0x6F12, 0x100A, 0x02,
	0x6F12, 0xB8EE, 0x02,
	0x6F12, 0x400A, 0x02,
	0x6F12, 0x20EE, 0x02,
	0x6F12, 0x280A, 0x02,
	0x6F12, 0xC8EE, 0x02,
	0x6F12, 0x000A, 0x02,
	0x6F12, 0x9FED, 0x02,
	0x6F12, 0xB10A, 0x02,
	0x6F12, 0x20EE, 0x02,
	0x6F12, 0x800A, 0x02,
	0x6F12, 0xBCEE, 0x02,
	0x6F12, 0xC00A, 0x02,
	0x6F12, 0x10EE, 0x02,
	0x6F12, 0x105A, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0x36FF, 0x02,
	0x6F12, 0x060A, 0x02,
	0x6F12, 0x9BE7, 0x02,
	0x6F12, 0x0020, 0x02,
	0x6F12, 0x00EB, 0x02,
	0x6F12, 0xC001, 0x02,
	0x6F12, 0x01EB, 0x02,
	0x6F12, 0x0011, 0x02,
	0x6F12, 0x07EB, 0x02,
	0x6F12, 0x4101, 0x02,
	0x6F12, 0x01EB, 0x02,
	0x6F12, 0x4601, 0x02,
	0x6F12, 0x01EB, 0x02,
	0x6F12, 0x4501, 0x02,
	0x6F12, 0xB1F8, 0x02,
	0x6F12, 0x2823, 0x02,
	0x6F12, 0x04EB, 0x02,
	0x6F12, 0x8001, 0x02,
	0x6F12, 0x401C, 0x02,
	0x6F12, 0x0B6A, 0x02,
	0x6F12, 0x5343, 0x02,
	0x6F12, 0x1A0B, 0x02,
	0x6F12, 0x0A62, 0x02,
	0x6F12, 0x0428, 0x02,
	0x6F12, 0xEAD3, 0x02,
	0x6F12, 0xBDEC, 0x02,
	0x6F12, 0x028B, 0x02,
	0x6F12, 0x4146, 0x02,
	0x6F12, 0x4846, 0x02,
	0x6F12, 0xBDE8, 0x02,
	0x6F12, 0xF047, 0x02,
	0x6F12, 0x0122, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0x7CBB, 0x02,
	0x6F12, 0x2DE9, 0x02,
	0x6F12, 0xF04F, 0x02,
	0x6F12, 0x0446, 0x02,
	0x6F12, 0x9748, 0x02,
	0x6F12, 0x0D46, 0x02,
	0x6F12, 0x2DED, 0x02,
	0x6F12, 0x028B, 0x02,
	0x6F12, 0x87B0, 0x02,
	0x6F12, 0xC06C, 0x02,
	0x6F12, 0x9346, 0x02,
	0x6F12, 0x010C, 0x02,
	0x6F12, 0x80B2, 0x02,
	0x6F12, 0xCDE9, 0x02,
	0x6F12, 0x0401, 0x02,
	0x6F12, 0x0146, 0x02,
	0x6F12, 0x0022, 0x02,
	0x6F12, 0x0598, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0x69FB, 0x02,
	0x6F12, 0x5A46, 0x02,
	0x6F12, 0x2946, 0x02,
	0x6F12, 0x2046, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0x02FF, 0x02,
	0x6F12, 0x95F8, 0x02,
	0x6F12, 0x6501, 0x02,
	0x6F12, 0x0028, 0x02,
	0x6F12, 0x7DD0, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0x6DFB, 0x02,
	0x6F12, 0x0228, 0x02,
	0x6F12, 0x79D3, 0x02,
	0x6F12, 0x0026, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0x68FB, 0x02,
	0x6F12, 0x0328, 0x02,
	0x6F12, 0x2DD0, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0xE9FE, 0x02,
	0x6F12, 0x050A, 0x02,
	0x6F12, 0xB6FA, 0x02,
	0x6F12, 0x86F0, 0x02,
	0x6F12, 0xC0F1, 0x02,
	0x6F12, 0x2002, 0x02,
	0x6F12, 0x092A, 0x02,
	0x6F12, 0x00D2, 0x02,
	0x6F12, 0x0922, 0x02,
	0x6F12, 0x093A, 0x02,
	0x6F12, 0xD640, 0x02,
	0x6F12, 0xB5FA, 0x02,
	0x6F12, 0x85F0, 0x02,
	0x6F12, 0xC0F1, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xF7B2, 0x02,
	0x6F12, 0x0928, 0x02,
	0x6F12, 0x00D2, 0x02,
	0x6F12, 0x0920, 0x02,
	0x6F12, 0x0938, 0x02,
	0x6F12, 0xC540, 0x02,
	0x6F12, 0xDFF8, 0x02,
	0x6F12, 0xFC91, 0x02,
	0x6F12, 0xDFF8, 0x02,
	0x6F12, 0xF081, 0x02,
	0x6F12, 0x05F0, 0x02,
	0x6F12, 0xFF0A, 0x02,
	0x6F12, 0x0021, 0x02,
	0x6F12, 0xEE46, 0x02,
	0x6F12, 0x00EB, 0x02,
	0x6F12, 0x8003, 0x02,
	0x6F12, 0xA145, 0x02,
	0x6F12, 0x2BD1, 0x02,
	0x6F12, 0x01EB, 0x02,
	0x6F12, 0xC100, 0x02,
	0x6F12, 0x00EB, 0x02,
	0x6F12, 0x0110, 0x02,
	0x6F12, 0x08EB, 0x02,
	0x6F12, 0x4000, 0x02,
	0x6F12, 0x00EB, 0x02,
	0x6F12, 0x4300, 0x02,
	0x6F12, 0x00F1, 0x02,
	0x6F12, 0x0806, 0x02,
	0x6F12, 0x1230, 0x02,
	0x6F12, 0x32E0, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0xBBFE, 0x02,
	0x6F12, 0x00EE, 0x02,
	0x6F12, 0x100A, 0x02,
	0x6F12, 0xB8EE, 0x02,
	0x6F12, 0x400A, 0x02,
	0x6F12, 0x5FED, 0x02,
	0x6F12, 0xA18A, 0x02,
	0x6F12, 0x20EE, 0x02,
	0x6F12, 0x288A, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0xB6FE, 0x02,
	0x6F12, 0x00EE, 0x02,
	0x6F12, 0x100A, 0x02,
	0x6F12, 0xB8EE, 0x02,
	0x6F12, 0x400A, 0x02,
	0x6F12, 0x20EE, 0x02,
	0x6F12, 0x280A, 0x02,
	0x6F12, 0xC8EE, 0x02,
	0x6F12, 0x000A, 0x02,
	0x6F12, 0x9FED, 0x02,
	0x6F12, 0x680A, 0x02,
	0x6F12, 0x20EE, 0x02,
	0x6F12, 0x800A, 0x02,
	0x6F12, 0xBCEE, 0x02,
	0x6F12, 0xC00A, 0x02,
	0x6F12, 0x10EE, 0x02,
	0x6F12, 0x105A, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0xA4FE, 0x02,
	0x6F12, 0x060A, 0x02,
	0x6F12, 0xB4E7, 0x02,
	0x6F12, 0x6448, 0x02,
	0x6F12, 0xA042, 0x02,
	0x6F12, 0x01EB, 0x02,
	0x6F12, 0xC100, 0x02,
	0x6F12, 0x00EB, 0x02,
	0x6F12, 0x0110, 0x02,
	0x6F12, 0x08EB, 0x02,
	0x6F12, 0x4000, 0x02,
	0x6F12, 0x00EB, 0x02,
	0x6F12, 0x4300, 0x02,
	0x6F12, 0x04D1, 0x02,
	0x6F12, 0x00F5, 0x02,
	0x6F12, 0xCC76, 0x02,
	0x6F12, 0x00F5, 0x02,
	0x6F12, 0xD170, 0x02,
	0x6F12, 0x02E0, 0x02,
	0x6F12, 0x00F1, 0x02,
	0x6F12, 0xD006, 0x02,
	0x6F12, 0xDA30, 0x02,
	0x6F12, 0x551C, 0x02,
	0x6F12, 0x36F9, 0x02,
	0x6F12, 0x15C0, 0x02,
	0x6F12, 0x36F9, 0x02,
	0x6F12, 0x1260, 0x02,
	0x6F12, 0x30F9, 0x02,
	0x6F12, 0x1550, 0x02,
	0x6F12, 0xACEB, 0x02,
	0x6F12, 0x060C, 0x02,
	0x6F12, 0x30F9, 0x02,
	0x6F12, 0x1200, 0x02,
	0x6F12, 0x0CFB, 0x02,
	0x6F12, 0x07FC, 0x02,
	0x6F12, 0x0CF1, 0x02,
	0x6F12, 0x800C, 0x02,
	0x6F12, 0x2D1A, 0x02,
	0x6F12, 0x06EB, 0x02,
	0x6F12, 0x2C26, 0x02,
	0x6F12, 0x7D43, 0x02,
	0x6F12, 0x00E0, 0x02,
	0x6F12, 0x23E0, 0x02,
	0x6F12, 0x8035, 0x02,
	0x6F12, 0x00EB, 0x02,
	0x6F12, 0x2520, 0x02,
	0x6F12, 0x801B, 0x02,
	0x6F12, 0x00FB, 0x02,
	0x6F12, 0x0AF0, 0x02,
	0x6F12, 0x8030, 0x02,
	0x6F12, 0x06EB, 0x02,
	0x6F12, 0x2020, 0x02,
	0x6F12, 0x4EF8, 0x02,
	0x6F12, 0x2100, 0x02,
	0x6F12, 0x00FA, 0x02,
	0x6F12, 0x0BF0, 0x02,
	0x6F12, 0x4EF8, 0x02,
	0x6F12, 0x2100, 0x02,
	0x6F12, 0x491C, 0x02,
	0x6F12, 0x0329, 0x02,
	0x6F12, 0x97D9, 0x02,
	0x6F12, 0x04F5, 0x02,
	0x6F12, 0x8074, 0x02,
	0x6F12, 0x0099, 0x02,
	0x6F12, 0x2068, 0x02,
	0x6F12, 0x0844, 0x02,
	0x6F12, 0x2060, 0x02,
	0x6F12, 0x6068, 0x02,
	0x6F12, 0x0199, 0x02,
	0x6F12, 0x0844, 0x02,
	0x6F12, 0x6060, 0x02,
	0x6F12, 0xA068, 0x02,
	0x6F12, 0x0299, 0x02,
	0x6F12, 0x0844, 0x02,
	0x6F12, 0xA060, 0x02,
	0x6F12, 0xE068, 0x02,
	0x6F12, 0x0399, 0x02,
	0x6F12, 0x0844, 0x02,
	0x6F12, 0xE060, 0x02,
	0x6F12, 0xDDE9, 0x02,
	0x6F12, 0x0410, 0x02,
	0x6F12, 0x07B0, 0x02,
	0x6F12, 0x0122, 0x02,
	0x6F12, 0xBDEC, 0x02,
	0x6F12, 0x028B, 0x02,
	0x6F12, 0xBDE8, 0x02,
	0x6F12, 0xF04F, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0xB3BA, 0x02,
	0x6F12, 0x2DE9, 0x02,
	0x6F12, 0xF041, 0x02,
	0x6F12, 0x1646, 0x02,
	0x6F12, 0x2DED, 0x02,
	0x6F12, 0x048B, 0x02,
	0x6F12, 0x0F46, 0x02,
	0x6F12, 0x0446, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0xB9FA, 0x02,
	0x6F12, 0x304A, 0x02,
	0x6F12, 0x0146, 0x02,
	0x6F12, 0x41F6, 0x02,
	0x6F12, 0x6823, 0x02,
	0x6F12, 0x41F6, 0x02,
	0x6F12, 0x5820, 0x02,
	0x6F12, 0xD518, 0x02,
	0x6F12, 0x1044, 0x02,
	0x6F12, 0x0229, 0x02,
	0x6F12, 0x2BD3, 0x02,
	0x6F12, 0x90ED, 0x02,
	0x6F12, 0x000A, 0x02,
	0x6F12, 0xB8EE, 0x02,
	0x6F12, 0x400A, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0x12FB, 0x02,
	0x6F12, 0xF0EE, 0x02,
	0x6F12, 0x408A, 0x02,
	0x6F12, 0x95ED, 0x02,
	0x6F12, 0x000A, 0x02,
	0x6F12, 0xB8EE, 0x02,
	0x6F12, 0x400A, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0x0AFB, 0x02,
	0x6F12, 0x2948, 0x02,
	0x6F12, 0xB0EE, 0x02,
	0x6F12, 0x408A, 0x02,
	0x6F12, 0x90ED, 0x02,
	0x6F12, 0x000A, 0x02,
	0x6F12, 0xB8EE, 0x02,
	0x6F12, 0x400A, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0x01FB, 0x02,
	0x6F12, 0xB0EE, 0x02,
	0x6F12, 0x409A, 0x02,
	0x6F12, 0x0C20, 0x02,
	0x6F12, 0xB0EE, 0x02,
	0x6F12, 0x680A, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0x25FE, 0x02,
	0x6F12, 0x2060, 0x02,
	0x6F12, 0x0C20, 0x02,
	0x6F12, 0xB0EE, 0x02,
	0x6F12, 0x490A, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0x1FFE, 0x02,
	0x6F12, 0x3060, 0x02,
	0x6F12, 0x0C20, 0x02,
	0x6F12, 0xB0EE, 0x02,
	0x6F12, 0x480A, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0x19FE, 0x02,
	0x6F12, 0x3860, 0x02,
	0x6F12, 0x35E6, 0x02,
	0x6F12, 0x0068, 0x02,
	0x6F12, 0x2060, 0x02,
	0x6F12, 0x2868, 0x02,
	0x6F12, 0x3860, 0x02,
	0x6F12, 0x2068, 0x02,
	0x6F12, 0x3060, 0x02,
	0x6F12, 0x2EE6, 0x02,
	0x6F12, 0x2DE9, 0x02,
	0x6F12, 0xF04F, 0x02,
	0x6F12, 0x8146, 0x02,
	0x6F12, 0x0F48, 0x02,
	0x6F12, 0x0D46, 0x02,
	0x6F12, 0x2DED, 0x02,
	0x6F12, 0x028B, 0x02,
	0x6F12, 0xA9B0, 0x02,
	0x6F12, 0x406E, 0x02,
	0x6F12, 0x0022, 0x02,
	0x6F12, 0x87B2, 0x02,
	0x6F12, 0x040C, 0x02,
	0x6F12, 0x3946, 0x02,
	0x6F12, 0x2046, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0x5DFA, 0x02,
	0x6F12, 0x0120, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0x02FE, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0xD0FA, 0x02,
	0x6F12, 0xB0EE, 0x02,
	0x6F12, 0x408A, 0x02,
	0x6F12, 0x0020, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0xFBFD, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0xC9FA, 0x02,
	0x6F12, 0xF0EE, 0x02,
	0x6F12, 0x408A, 0x02,
	0x6F12, 0x06AA, 0x02,
	0x6F12, 0x07A9, 0x02,
	0x6F12, 0x0FE0, 0x02,
	0x6F12, 0x4580, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x2001, 0x02,
	0x6F12, 0x2400, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0x8BA0, 0x02,
	0x6F12, 0x2001, 0x02,
	0x6F12, 0x2520, 0x02,
	0x6F12, 0x4380, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xC0DC, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xC1FC, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xA618, 0x02,
	0x6F12, 0x05A8, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0x86FF, 0x02,
	0x6F12, 0xDDED, 0x02,
	0x6F12, 0x060A, 0x02,
	0x6F12, 0x9DED, 0x02,
	0x6F12, 0x051A, 0x02,
	0x6F12, 0xF8EE, 0x02,
	0x6F12, 0x601A, 0x02,
	0x6F12, 0xB8EE, 0x02,
	0x6F12, 0x412A, 0x02,
	0x6F12, 0xB8EE, 0x02,
	0x6F12, 0x411A, 0x02,
	0x6F12, 0x81EE, 0x02,
	0x6F12, 0x820A, 0x02,
	0x6F12, 0xF8EE, 0x02,
	0x6F12, 0x601A, 0x02,
	0x6F12, 0xC1EE, 0x02,
	0x6F12, 0x810A, 0x02,
	0x6F12, 0xFE48, 0x02,
	0x6F12, 0x0026, 0x02,
	0x6F12, 0xB6EE, 0x02,
	0x6F12, 0x001A, 0x02,
	0x6F12, 0xB0F8, 0x02,
	0x6F12, 0xC004, 0x02,
	0x6F12, 0x0328, 0x02,
	0x6F12, 0x21D1, 0x02,
	0x6F12, 0xFB48, 0x02,
	0x6F12, 0xB0F8, 0x02,
	0x6F12, 0xAC05, 0x02,
	0x6F12, 0xE8B1, 0x02,
	0x6F12, 0x68EE, 0x02,
	0x6F12, 0xA00A, 0x02,
	0x6F12, 0x68EE, 0x02,
	0x6F12, 0x001A, 0x02,
	0x6F12, 0x60EE, 0x02,
	0x6F12, 0x810A, 0x02,
	0x6F12, 0xF4EE, 0x02,
	0x6F12, 0xE01A, 0x02,
	0x6F12, 0xF1EE, 0x02,
	0x6F12, 0x10FA, 0x02,
	0x6F12, 0x12DD, 0x02,
	0x6F12, 0xF348, 0x02,
	0x6F12, 0xF349, 0x02,
	0x6F12, 0xB0F8, 0x02,
	0x6F12, 0x8605, 0x02,
	0x6F12, 0xB1F8, 0x02,
	0x6F12, 0x8815, 0x02,
	0x6F12, 0x4218, 0x02,
	0x6F12, 0xEF48, 0x02,
	0x6F12, 0x00F5, 0x02,
	0x6F12, 0x9761, 0x02,
	0x6F12, 0xA0F8, 0x02,
	0x6F12, 0xB824, 0x02,
	0x6F12, 0xA0F8, 0x02,
	0x6F12, 0xBA64, 0x02,
	0x6F12, 0x0888, 0x02,
	0x6F12, 0xED49, 0x02,
	0x6F12, 0x0880, 0x02,
	0x6F12, 0x881C, 0x02,
	0x6F12, 0x0680, 0x02,
	0x6F12, 0xEA48, 0x02,
	0x6F12, 0xB0F8, 0x02,
	0x6F12, 0x8C05, 0x02,
	0x6F12, 0x0128, 0x02,
	0x6F12, 0x28D1, 0x02,
	0x6F12, 0xE848, 0x02,
	0x6F12, 0x001D, 0x02,
	0x6F12, 0x0188, 0x02,
	0x6F12, 0xDFED, 0x02,
	0x6F12, 0xE80A, 0x02,
	0x6F12, 0x68EE, 0x02,
	0x6F12, 0x200A, 0x02,
	0x6F12, 0x00EE, 0x02,
	0x6F12, 0x801A, 0x02,
	0x6F12, 0xBCEE, 0x02,
	0x6F12, 0xC10A, 0x02,
	0x6F12, 0x10EE, 0x02,
	0x6F12, 0x100A, 0x02,
	0x6F12, 0x11FA, 0x02,
	0x6F12, 0x80F0, 0x02,
	0x6F12, 0xB0F5, 0x02,
	0x6F12, 0x805F, 0x02,
	0x6F12, 0x01DB, 0x02,
	0x6F12, 0x40F6, 0x02,
	0x6F12, 0xFF70, 0x02,
	0x6F12, 0xADF8, 0x02,
	0x6F12, 0x0C00, 0x02,
	0x6F12, 0xADF8, 0x02,
	0x6F12, 0x0E10, 0x02,
	0x6F12, 0x0C21, 0x02,
	0x6F12, 0x6846, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0x8BFD, 0x02,
	0x6F12, 0xDA49, 0x02,
	0x6F12, 0xBDF8, 0x02,
	0x6F12, 0x0C00, 0x02,
	0x6F12, 0x891D, 0x02,
	0x6F12, 0x0880, 0x02,
	0x6F12, 0x891C, 0x02,
	0x6F12, 0xBDF8, 0x02,
	0x6F12, 0x1200, 0x02,
	0x6F12, 0x0880, 0x02,
	0x6F12, 0x891C, 0x02,
	0x6F12, 0xBDF8, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x0880, 0x02,
	0x6F12, 0x2946, 0x02,
	0x6F12, 0x4846, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0x7FFD, 0x02,
	0x6F12, 0x0122, 0x02,
	0x6F12, 0x3946, 0x02,
	0x6F12, 0x2046, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0xC8F9, 0x02,
	0x6F12, 0xE889, 0x02,
	0x6F12, 0x0328, 0x02,
	0x6F12, 0x5AD1, 0x02,
	0x6F12, 0xB5F9, 0x02,
	0x6F12, 0x0640, 0x02,
	0x6F12, 0xB5F9, 0x02,
	0x6F12, 0x0810, 0x02,
	0x6F12, 0xCB4A, 0x02,
	0x6F12, 0x0591, 0x02,
	0x6F12, 0x671A, 0x02,
	0x6F12, 0xB2F9, 0x02,
	0x6F12, 0xB834, 0x02,
	0x6F12, 0x02F5, 0x02,
	0x6F12, 0x9768, 0x02,
	0x6F12, 0x7B43, 0x02,
	0x6F12, 0x4FF0, 0x02,
	0x6F12, 0x640A, 0x02,
	0x6F12, 0x93FB, 0x02,
	0x6F12, 0xFAF2, 0x02,
	0x6F12, 0x6018, 0x02,
	0x6F12, 0x00EB, 0x02,
	0x6F12, 0xD075, 0x02,
	0x6F12, 0xC2EB, 0x02,
	0x6F12, 0x6500, 0x02,
	0x6F12, 0x491C, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0x64FD, 0x02,
	0x6F12, 0x0790, 0x02,
	0x6F12, 0xB8F9, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x611E, 0x02,
	0x6F12, 0x7843, 0x02,
	0x6F12, 0x90FB, 0x02,
	0x6F12, 0xFAF0, 0x02,
	0x6F12, 0x00EB, 0x02,
	0x6F12, 0x6500, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0x5EFD, 0x02,
	0x6F12, 0x0990, 0x02,
	0x6F12, 0xBB48, 0x02,
	0x6F12, 0x0B94, 0x02,
	0x6F12, 0x0124, 0x02,
	0x6F12, 0xB0F9, 0x02,
	0x6F12, 0xB604, 0x02,
	0x6F12, 0x0690, 0x02,
	0x6F12, 0xB848, 0x02,
	0x6F12, 0x0DF1, 0x02,
	0x6F12, 0x0C08, 0x02,
	0x6F12, 0x27AF, 0x02,
	0x6F12, 0xB0F9, 0x02,
	0x6F12, 0xBE04, 0x02,
	0x6F12, 0x0890, 0x02,
	0x6F12, 0x0A90, 0x02,
	0x6F12, 0xB448, 0x02,
	0x6F12, 0x0DF1, 0x02,
	0x6F12, 0x980A, 0x02,
	0x6F12, 0x0DF1, 0x02,
	0x6F12, 0x940B, 0x02,
	0x6F12, 0xB0F9, 0x02,
	0x6F12, 0xBC04, 0x02,
	0x6F12, 0x2596, 0x02,
	0x6F12, 0x2796, 0x02,
	0x6F12, 0x2696, 0x02,
	0x6F12, 0x0C90, 0x02,
	0x6F12, 0x08EB, 0x02,
	0x6F12, 0xC402, 0x02,
	0x6F12, 0x58F8, 0x02,
	0x6F12, 0x3400, 0x02,
	0x6F12, 0xD2E9, 0x02,
	0x6F12, 0x0115, 0x02,
	0x6F12, 0xD368, 0x02,
	0x6F12, 0xCDF8, 0x02,
	0x6F12, 0x00B0, 0x02,
	0x6F12, 0xCDE9, 0x02,
	0x6F12, 0x01A7, 0x02,
	0x6F12, 0x2A46, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0x3BFD, 0x02,
	0x6F12, 0x09EB, 0x02,
	0x6F12, 0x4400, 0x02,
	0x6F12, 0x2799, 0x02,
	0x6F12, 0x4184, 0x02,
	0x6F12, 0x2599, 0x02,
	0x6F12, 0x4180, 0x02,
	0x6F12, 0x2699, 0x02,
	0x6F12, 0xA0F8, 0x02,
	0x6F12, 0x4210, 0x02,
	0x6F12, 0x0F2C, 0x02,
	0x6F12, 0x01DA, 0x02,
	0x6F12, 0xA0F8, 0x02,
	0x6F12, 0x6250, 0x02,
	0x6F12, 0x641C, 0x02,
	0x6F12, 0x042C, 0x02,
	0x6F12, 0xE1DB, 0x02,
	0x6F12, 0xA248, 0x02,
	0x6F12, 0xB0F8, 0x02,
	0x6F12, 0xBA14, 0x02,
	0x6F12, 0x9E48, 0x02,
	0x6F12, 0x0129, 0x02,
	0x6F12, 0xA049, 0x02,
	0x6F12, 0x7ED0, 0x02,
	0x6F12, 0x0246, 0x02,
	0x6F12, 0xB1F8, 0x02,
	0x6F12, 0xC214, 0x02,
	0x6F12, 0xA2F8, 0x02,
	0x6F12, 0x7215, 0x02,
	0x6F12, 0x9C49, 0x02,
	0x6F12, 0xB1F8, 0x02,
	0x6F12, 0xBE14, 0x02,
	0x6F12, 0xA0F8, 0x02,
	0x6F12, 0x1612, 0x02,
	0x6F12, 0x9748, 0x02,
	0x6F12, 0x9649, 0x02,
	0x6F12, 0x964A, 0x02,
	0x6F12, 0xB0F8, 0x02,
	0x6F12, 0x8405, 0x02,
	0x6F12, 0xB1F8, 0x02,
	0x6F12, 0x8C15, 0x02,
	0x6F12, 0xB2F8, 0x02,
	0x6F12, 0x9025, 0x02,
	0x6F12, 0x40EA, 0x02,
	0x6F12, 0x4100, 0x02,
	0x6F12, 0x9149, 0x02,
	0x6F12, 0xB1F8, 0x02,
	0x6F12, 0x7215, 0x02,
	0x6F12, 0xC900, 0x02,
	0x6F12, 0x41EA, 0x02,
	0x6F12, 0x8201, 0x02,
	0x6F12, 0x0843, 0x02,
	0x6F12, 0x8F49, 0x02,
	0x6F12, 0x0C39, 0x02,
	0x6F12, 0x0880, 0x02,
	0x6F12, 0x8F48, 0x02,
	0x6F12, 0x8C49, 0x02,
	0x6F12, 0xB0F8, 0x02,
	0x6F12, 0xC404, 0x02,
	0x6F12, 0xA1F8, 0x02,
	0x6F12, 0x6604, 0x02,
	0x6F12, 0x8C48, 0x02,
	0x6F12, 0x8D49, 0x02,
	0x6F12, 0xB0F8, 0x02,
	0x6F12, 0x7205, 0x02,
	0x6F12, 0x0880, 0x02,
	0x6F12, 0x8748, 0x02,
	0x6F12, 0x8B49, 0x02,
	0x6F12, 0xB0F8, 0x02,
	0x6F12, 0xC804, 0x02,
	0x6F12, 0x0880, 0x02,
	0x6F12, 0x8748, 0x02,
	0x6F12, 0x8549, 0x02,
	0x6F12, 0x0C31, 0x02,
	0x6F12, 0xB0F8, 0x02,
	0x6F12, 0x8005, 0x02,
	0x6F12, 0x0880, 0x02,
	0x6F12, 0x8448, 0x02,
	0x6F12, 0x891C, 0x02,
	0x6F12, 0xB0F8, 0x02,
	0x6F12, 0x7C05, 0x02,
	0x6F12, 0x0880, 0x02,
	0x6F12, 0x8248, 0x02,
	0x6F12, 0x891C, 0x02,
	0x6F12, 0xB0F8, 0x02,
	0x6F12, 0x7805, 0x02,
	0x6F12, 0x0880, 0x02,
	0x6F12, 0x7F48, 0x02,
	0x6F12, 0x891C, 0x02,
	0x6F12, 0xB0F8, 0x02,
	0x6F12, 0x7405, 0x02,
	0x6F12, 0x0880, 0x02,
	0x6F12, 0x7D48, 0x02,
	0x6F12, 0x891C, 0x02,
	0x6F12, 0xB0F8, 0x02,
	0x6F12, 0x8205, 0x02,
	0x6F12, 0x0880, 0x02,
	0x6F12, 0x7A48, 0x02,
	0x6F12, 0x891C, 0x02,
	0x6F12, 0xB0F8, 0x02,
	0x6F12, 0x7E05, 0x02,
	0x6F12, 0x0880, 0x02,
	0x6F12, 0x7848, 0x02,
	0x6F12, 0x891C, 0x02,
	0x6F12, 0xB0F8, 0x02,
	0x6F12, 0x7A05, 0x02,
	0x6F12, 0x0880, 0x02,
	0x6F12, 0x7548, 0x02,
	0x6F12, 0x891C, 0x02,
	0x6F12, 0xB0F8, 0x02,
	0x6F12, 0x7605, 0x02,
	0x6F12, 0x0880, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0xCFFC, 0x02,
	0x6F12, 0x0028, 0x02,
	0x6F12, 0x26D0, 0x02,
	0x6F12, 0x6E48, 0x02,
	0x6F12, 0xB0F9, 0x02,
	0x6F12, 0x6414, 0x02,
	0x6F12, 0x19B1, 0x02,
	0x6F12, 0xB0F8, 0x02,
	0x6F12, 0xB404, 0x02,
	0x6F12, 0x0028, 0x02,
	0x6F12, 0x1ED1, 0x02,
	0x6F12, 0x694B, 0x02,
	0x6F12, 0x6F4A, 0x02,
	0x6F12, 0x9879, 0x02,
	0x6F12, 0x1268, 0x02,
	0x6F12, 0xC0F1, 0x02,
	0x6F12, 0x0C00, 0x02,
	0x6F12, 0xC240, 0x02,
	0x6F12, 0xB3F8, 0x02,
	0x6F12, 0x6402, 0x02,
	0x6F12, 0x6A4B, 0x02,
	0x6F12, 0x5043, 0x02,
	0x6F12, 0x674A, 0x02,
	0x6F12, 0x000A, 0x02,
	0x6F12, 0x923B, 0x02,
	0x6F12, 0xB2F8, 0x02,
	0x6F12, 0x8645, 0x02,
	0x6F12, 0x694A, 0x02,
	0x6F12, 0x8442, 0x02,
	0x6F12, 0x19D8, 0x02,
	0x6F12, 0x634C, 0x02,
	0x6F12, 0xB4F8, 0x02,
	0x6F12, 0x8A45, 0x02,
	0x6F12, 0xACB1, 0x02,
	0x6F12, 0x6148, 0x02,
	0x6F12, 0xA0F8, 0x02,
	0x6F12, 0x8A65, 0x02,
	0x6F12, 0x00E0, 0x02,
	0x6F12, 0x07E0, 0x02,
	0x6F12, 0xA2F8, 0x02,
	0x6F12, 0x0063, 0x02,
	0x6F12, 0x1E80, 0x02,
	0x6F12, 0x29B0, 0x02,
	0x6F12, 0xBDEC, 0x02,
	0x6F12, 0x028B, 0x02,
	0x6F12, 0xBDE8, 0x02,
	0x6F12, 0xF08F, 0x02,
	0x6F12, 0x584A, 0x02,
	0x6F12, 0xB1F8, 0x02,
	0x6F12, 0xC014, 0x02,
	0x6F12, 0xA2F8, 0x02,
	0x6F12, 0x7215, 0x02,
	0x6F12, 0x5849, 0x02,
	0x6F12, 0xB1F8, 0x02,
	0x6F12, 0xBC14, 0x02,
	0x6F12, 0x76E7, 0x02,
	0x6F12, 0x564C, 0x02,
	0x6F12, 0xB4F8, 0x02,
	0x6F12, 0x8845, 0x02,
	0x6F12, 0x8442, 0x02,
	0x6F12, 0xECD3, 0x02,
	0x6F12, 0x5448, 0x02,
	0x6F12, 0xB0F8, 0x02,
	0x6F12, 0x8A05, 0x02,
	0x6F12, 0x0028, 0x02,
	0x6F12, 0xE7D1, 0x02,
	0x6F12, 0x514C, 0x02,
	0x6F12, 0x0120, 0x02,
	0x6F12, 0xA4F8, 0x02,
	0x6F12, 0x8A05, 0x02,
	0x6F12, 0x09B9, 0x02,
	0x6F12, 0xA2F8, 0x02,
	0x6F12, 0x0003, 0x02,
	0x6F12, 0x4A49, 0x02,
	0x6F12, 0x91F8, 0x02,
	0x6F12, 0x3214, 0x02,
	0x6F12, 0x0029, 0x02,
	0x6F12, 0xDBD1, 0x02,
	0x6F12, 0x1880, 0x02,
	0x6F12, 0xD9E7, 0x02,
	0x6F12, 0x10B5, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0x82FC, 0x02,
	0x6F12, 0x4E48, 0x02,
	0x6F12, 0xB0F8, 0x02,
	0x6F12, 0x3E07, 0x02,
	0x6F12, 0x0428, 0x02,
	0x6F12, 0x13D1, 0x02,
	0x6F12, 0x0020, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0x7FFC, 0x02,
	0x6F12, 0x0228, 0x02,
	0x6F12, 0x0ED0, 0x02,
	0x6F12, 0x0121, 0x02,
	0x6F12, 0x0846, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0x7EFC, 0x02,
	0x6F12, 0x4749, 0x02,
	0x6F12, 0x4748, 0x02,
	0x6F12, 0xB1F8, 0x02,
	0x6F12, 0x8E17, 0x02,
	0x6F12, 0x0180, 0x02,
	0x6F12, 0x0121, 0x02,
	0x6F12, 0xBDE8, 0x02,
	0x6F12, 0x1040, 0x02,
	0x6F12, 0x0020, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0x73BC, 0x02,
	0x6F12, 0x10BD, 0x02,
	0x6F12, 0x2DE9, 0x02,
	0x6F12, 0xF04F, 0x02,
	0x6F12, 0x424B, 0x02,
	0x6F12, 0x3A4A, 0x02,
	0x6F12, 0x9A46, 0x02,
	0x6F12, 0xB3F8, 0x02,
	0x6F12, 0xE000, 0x02,
	0x6F12, 0xB3F8, 0x02,
	0x6F12, 0xE850, 0x02,
	0x6F12, 0x02F5, 0x02,
	0x6F12, 0xAE64, 0x02,
	0x6F12, 0x0021, 0x02,
	0x6F12, 0xA31E, 0x02,
	0x6F12, 0x85B0, 0x02,
	0x6F12, 0x9A1E, 0x02,
	0x6F12, 0x344E, 0x02,
	0x6F12, 0x00F1, 0x02,
	0x6F12, 0x8040, 0x02,
	0x6F12, 0xB6F8, 0x02,
	0x6F12, 0x6C65, 0x02,
	0x6F12, 0x4682, 0x02,
	0x6F12, 0x1688, 0x02,
	0x6F12, 0x8682, 0x02,
	0x6F12, 0x1688, 0x02,
	0x6F12, 0xC682, 0x02,
	0x6F12, 0x2F4E, 0x02,
	0x6F12, 0xB6F8, 0x02,
	0x6F12, 0x6E65, 0x02,
	0x6F12, 0x0683, 0x02,
	0x6F12, 0x1E88, 0x02,
	0x6F12, 0x4683, 0x02,
	0x6F12, 0x1E88, 0x02,
	0x6F12, 0x8683, 0x02,
	0x6F12, 0x2B4E, 0x02,
	0x6F12, 0xB6F8, 0x02,
	0x6F12, 0x7065, 0x02,
	0x6F12, 0xC683, 0x02,
	0x6F12, 0x2688, 0x02,
	0x6F12, 0x0684, 0x02,
	0x6F12, 0x2688, 0x02,
	0x6F12, 0x4684, 0x02,
	0x6F12, 0x274E, 0x02,
	0x6F12, 0xB6F8, 0x02,
	0x6F12, 0x6A65, 0x02,
	0x6F12, 0x0682, 0x02,
	0x6F12, 0x491C, 0x02,
	0x6F12, 0x2846, 0x02,
	0x6F12, 0x0229, 0x02,
	0x6F12, 0xDDD3, 0x02,
	0x6F12, 0x2B49, 0x02,
	0x6F12, 0x4FF0, 0x02,
	0x6F12, 0x0108, 0x02,
	0x6F12, 0x81F8, 0x02,
	0x6F12, 0x3380, 0x02,
	0x6F12, 0x2A49, 0x02,
	0x6F12, 0x0888, 0x02,
	0x6F12, 0x204A, 0x02,
	0x6F12, 0x92F8, 0x02,
	0x6F12, 0x1625, 0x02,
	0x6F12, 0x40EA, 0x02,
	0x6F12, 0x8200, 0x02,
	0x6F12, 0x0880, 0x02,
	0x6F12, 0x2748, 0x02,
	0x6F12, 0x4FF0, 0x02,
	0x6F12, 0x0009, 0x02,
	0x6F12, 0x0088, 0x02,
	0x6F12, 0x58B3, 0x02,
	0x6F12, 0x0914, 0x02,
	0x6F12, 0xB1FB, 0x02,
	0x6F12, 0xF0F0, 0x02,
	0x6F12, 0xC0B2, 0x02,
	0x6F12, 0x0490, 0x02,
	0x6F12, 0x2348, 0x02,
	0x6F12, 0x234D, 0x02,
	0x6F12, 0x49F6, 0x02,
	0x6F12, 0x0A04, 0x02,
	0x6F12, 0xD0E9, 0x02,
	0x6F12, 0x0010, 0x02,
	0x6F12, 0xCDF8, 0x02,
	0x6F12, 0x0C90, 0x02,
	0x6F12, 0xCDE9, 0x02,
	0x6F12, 0x0110, 0x02,
	0x6F12, 0x0227, 0x02,
	0x6F12, 0x49F6, 0x02,
	0x6F12, 0x1C06, 0x02,
	0x6F12, 0x0DF1, 0x02,
	0x6F12, 0x0C0B, 0x02,
	0x6F12, 0x0823, 0x02,
	0x6F12, 0xCDF8, 0x02,
	0x6F12, 0x00B0, 0x02,
	0x6F12, 0x0421, 0x02,
	0x6F12, 0x01A8, 0x02,
	0x6F12, 0x049A, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0xA7F8, 0x02,
	0x6F12, 0x0328, 0x02,
	0x6F12, 0x7DD0, 0x02,
	0x6F12, 0x05EB, 0x02,
	0x6F12, 0x4001, 0x02,
	0x6F12, 0x35F8, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x4988, 0x02,
	0x6F12, 0x039A, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0x98F8, 0x02,
	0x6F12, 0x4FF0, 0x02,
	0x6F12, 0x8041, 0x02,
	0x6F12, 0x0853, 0x02,
	0x6F12, 0x0835, 0x02,
	0x6F12, 0xE019, 0x02,
	0x6F12, 0x23E0, 0x02,
	0x6F12, 0x25E0, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0x8BA0, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0x7A00, 0x02,
	0x6F12, 0x4000, 0x02,
	0x6F12, 0xA40E, 0x02,
	0x6F12, 0x42C8, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x2001, 0x02,
	0x6F12, 0x2520, 0x02,
	0x6F12, 0x4000, 0x02,
	0x6F12, 0xA808, 0x02,
	0x6F12, 0x4000, 0x02,
	0x6F12, 0xA692, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xA608, 0x02,
	0x6F12, 0x4000, 0x02,
	0x6F12, 0xA000, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0x6210, 0x02,
	0x6F12, 0x200B, 0x02,
	0x6F12, 0x8442, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0x7FB0, 0x02,
	0x6F12, 0x2001, 0x02,
	0x6F12, 0x2480, 0x02,
	0x6F12, 0x4000, 0x02,
	0x6F12, 0x9802, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0x8520, 0x02,
	0x6F12, 0x2001, 0x02,
	0x6F12, 0x00C0, 0x02,
	0x6F12, 0x2001, 0x02,
	0x6F12, 0x29E6, 0x02,
	0x6F12, 0x84B2, 0x02,
	0x6F12, 0xB442, 0x02,
	0x6F12, 0xC0D9, 0x02,
	0x6F12, 0xFA4C, 0x02,
	0x6F12, 0xFA48, 0x02,
	0x6F12, 0xA179, 0x02,
	0x6F12, 0x0180, 0x02,
	0x6F12, 0xE179, 0x02,
	0x6F12, 0x4186, 0x02,
	0x6F12, 0x4FF2, 0x02,
	0x6F12, 0x1800, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0xD7FB, 0x02,
	0x6F12, 0x0546, 0x02,
	0x6F12, 0x4FF2, 0x02,
	0x6F12, 0x1C00, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0xD2FB, 0x02,
	0x6F12, 0x2188, 0x02,
	0x6F12, 0x6288, 0x02,
	0x6F12, 0x2944, 0x02,
	0x6F12, 0x841A, 0x02,
	0x6F12, 0x4FF2, 0x02,
	0x6F12, 0x2800, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0xCFFB, 0x02,
	0x6F12, 0x2146, 0x02,
	0x6F12, 0x4FF2, 0x02,
	0x6F12, 0x2C00, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0xCAFB, 0x02,
	0x6F12, 0xEE48, 0x02,
	0x6F12, 0x0224, 0x02,
	0x6F12, 0xED4D, 0x02,
	0x6F12, 0x90F8, 0x02,
	0x6F12, 0xF705, 0x02,
	0x6F12, 0x0028, 0x02,
	0x6F12, 0x7DD0, 0x02,
	0x6F12, 0x0021, 0x02,
	0x6F12, 0x0120, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0xB5FB, 0x02,
	0x6F12, 0xEA4A, 0x02,
	0x6F12, 0xE949, 0x02,
	0x6F12, 0x5068, 0x02,
	0x6F12, 0x91F8, 0x02,
	0x6F12, 0xF932, 0x02,
	0x6F12, 0x80F8, 0x02,
	0x6F12, 0xD630, 0x02,
	0x6F12, 0x91F8, 0x02,
	0x6F12, 0xF632, 0x02,
	0x6F12, 0x80F8, 0x02,
	0x6F12, 0xD030, 0x02,
	0x6F12, 0xE64B, 0x02,
	0x6F12, 0x93F8, 0x02,
	0x6F12, 0xC060, 0x02,
	0x6F12, 0x86B1, 0x02,
	0x6F12, 0x93F8, 0x02,
	0x6F12, 0xBF30, 0x02,
	0x6F12, 0x6BB1, 0x02,
	0x6F12, 0x01F5, 0x02,
	0x6F12, 0xB871, 0x02,
	0x6F12, 0x0B88, 0x02,
	0x6F12, 0x1B0A, 0x02,
	0x6F12, 0x00F8, 0x02,
	0x6F12, 0x763F, 0x02,
	0x6F12, 0x4B78, 0x02,
	0x6F12, 0x8370, 0x02,
	0x6F12, 0x4B88, 0x02,
	0x6F12, 0x1B0A, 0x02,
	0x6F12, 0x0371, 0x02,
	0x6F12, 0xC978, 0x02,
	0x6F12, 0x8171, 0x02,
	0x6F12, 0x7638, 0x02,
	0x6F12, 0xAA26, 0x02,
	0x6F12, 0x00E0, 0x02,
	0x6F12, 0x8CE0, 0x02,
	0x6F12, 0x00F8, 0x02,
	0x6F12, 0xDF6F, 0x02,
	0x6F12, 0x0C21, 0x02,
	0x6F12, 0x4170, 0x02,
	0x6F12, 0xA523, 0x02,
	0x6F12, 0x8370, 0x02,
	0x6F12, 0x8021, 0x02,
	0x6F12, 0xC170, 0x02,
	0x6F12, 0xD449, 0x02,
	0x6F12, 0x91F8, 0x02,
	0x6F12, 0x4014, 0x02,
	0x6F12, 0x4171, 0x02,
	0x6F12, 0xD249, 0x02,
	0x6F12, 0x91F8, 0x02,
	0x6F12, 0x4114, 0x02,
	0x6F12, 0xC171, 0x02,
	0x6F12, 0xD049, 0x02,
	0x6F12, 0x91F8, 0x02,
	0x6F12, 0x4214, 0x02,
	0x6F12, 0x4172, 0x02,
	0x6F12, 0xCE49, 0x02,
	0x6F12, 0x91F8, 0x02,
	0x6F12, 0x4314, 0x02,
	0x6F12, 0xC172, 0x02,
	0x6F12, 0xCC49, 0x02,
	0x6F12, 0x01F2, 0x02,
	0x6F12, 0x4447, 0x02,
	0x6F12, 0xB1F8, 0x02,
	0x6F12, 0x4414, 0x02,
	0x6F12, 0x090A, 0x02,
	0x6F12, 0x4173, 0x02,
	0x6F12, 0x7978, 0x02,
	0x6F12, 0xC173, 0x02,
	0x6F12, 0xCA4F, 0x02,
	0x6F12, 0x95F8, 0x02,
	0x6F12, 0xBD10, 0x02,
	0x6F12, 0x1268, 0x02,
	0x6F12, 0xD7F8, 0x02,
	0x6F12, 0x00C0, 0x02,
	0x6F12, 0xC1F1, 0x02,
	0x6F12, 0x0C01, 0x02,
	0x6F12, 0x2CFA, 0x02,
	0x6F12, 0x01FC, 0x02,
	0x6F12, 0x4FEA, 0x02,
	0x6F12, 0x1C2C, 0x02,
	0x6F12, 0x02F8, 0x02,
	0x6F12, 0xDCCF, 0x02,
	0x6F12, 0x3F68, 0x02,
	0x6F12, 0xCF40, 0x02,
	0x6F12, 0x9770, 0x02,
	0x6F12, 0xC24F, 0x02,
	0x6F12, 0x3F1D, 0x02,
	0x6F12, 0xD7F8, 0x02,
	0x6F12, 0x00C0, 0x02,
	0x6F12, 0x2CFA, 0x02,
	0x6F12, 0x01FC, 0x02,
	0x6F12, 0x4FEA, 0x02,
	0x6F12, 0x1C2C, 0x02,
	0x6F12, 0x82F8, 0x02,
	0x6F12, 0x04C0, 0x02,
	0x6F12, 0x3F68, 0x02,
	0x6F12, 0xCF40, 0x02,
	0x6F12, 0x9771, 0x02,
	0x6F12, 0xBB4F, 0x02,
	0x6F12, 0x0837, 0x02,
	0x6F12, 0xD7F8, 0x02,
	0x6F12, 0x00C0, 0x02,
	0x6F12, 0x2CFA, 0x02,
	0x6F12, 0x01FC, 0x02,
	0x6F12, 0x4FEA, 0x02,
	0x6F12, 0x1C2C, 0x02,
	0x6F12, 0x82F8, 0x02,
	0x6F12, 0x08C0, 0x02,
	0x6F12, 0x3F68, 0x02,
	0x6F12, 0xCF40, 0x02,
	0x6F12, 0x9772, 0x02,
	0x6F12, 0xB54F, 0x02,
	0x6F12, 0x0C37, 0x02,
	0x6F12, 0xD7F8, 0x02,
	0x6F12, 0x00C0, 0x02,
	0x6F12, 0x2CFA, 0x02,
	0x6F12, 0x01FC, 0x02,
	0x6F12, 0x4FEA, 0x02,
	0x6F12, 0x1C2C, 0x02,
	0x6F12, 0x82F8, 0x02,
	0x6F12, 0x0CC0, 0x02,
	0x6F12, 0x00E0, 0x02,
	0x6F12, 0x2CE0, 0x02,
	0x6F12, 0x3F68, 0x02,
	0x6F12, 0xCF40, 0x02,
	0x6F12, 0x9773, 0x02,
	0x6F12, 0xAD4A, 0x02,
	0x6F12, 0x1032, 0x02,
	0x6F12, 0x1768, 0x02,
	0x6F12, 0xCF40, 0x02,
	0x6F12, 0x3F0A, 0x02,
	0x6F12, 0x00F8, 0x02,
	0x6F12, 0xA77C, 0x02,
	0x6F12, 0x1268, 0x02,
	0x6F12, 0xCA40, 0x02,
	0x6F12, 0x00F8, 0x02,
	0x6F12, 0xA52C, 0x02,
	0x6F12, 0xA84A, 0x02,
	0x6F12, 0x1432, 0x02,
	0x6F12, 0x1768, 0x02,
	0x6F12, 0xCF40, 0x02,
	0x6F12, 0x3F0A, 0x02,
	0x6F12, 0x00F8, 0x02,
	0x6F12, 0xA37C, 0x02,
	0x6F12, 0x1268, 0x02,
	0x6F12, 0xCA40, 0x02,
	0x6F12, 0x00F8, 0x02,
	0x6F12, 0xA12C, 0x02,
	0x6F12, 0xA24A, 0x02,
	0x6F12, 0x2032, 0x02,
	0x6F12, 0x1268, 0x02,
	0x6F12, 0x00F8, 0x02,
	0x6F12, 0xA06D, 0x02,
	0x6F12, 0xCA40, 0x02,
	0x6F12, 0x4470, 0x02,
	0x6F12, 0x8370, 0x02,
	0x6F12, 0x3821, 0x02,
	0x6F12, 0xC170, 0x02,
	0x6F12, 0x5A21, 0x02,
	0x6F12, 0x0171, 0x02,
	0x6F12, 0x130A, 0x02,
	0x6F12, 0x4371, 0x02,
	0x6F12, 0x8171, 0x02,
	0x6F12, 0xC271, 0x02,
	0x6F12, 0x0021, 0x02,
	0x6F12, 0x0846, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0x0DFB, 0x02,
	0x6F12, 0x0121, 0x02,
	0x6F12, 0x0846, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0x09FB, 0x02,
	0x6F12, 0x9748, 0x02,
	0x6F12, 0x0088, 0x02,
	0x6F12, 0x28B1, 0x02,
	0x6F12, 0x2FE0, 0x02,
	0x6F12, 0x4FF0, 0x02,
	0x6F12, 0x8040, 0x02,
	0x6F12, 0xE988, 0x02,
	0x6F12, 0x0153, 0x02,
	0x6F12, 0xF9E6, 0x02,
	0x6F12, 0xFEF7, 0x02,
	0x6F12, 0x38FF, 0x02,
	0x6F12, 0x0746, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0x0AFB, 0x02,
	0x6F12, 0x9248, 0x02,
	0x6F12, 0x914E, 0x02,
	0x6F12, 0x0088, 0x02,
	0x6F12, 0xB188, 0x02,
	0x6F12, 0x401A, 0x02,
	0x6F12, 0x401C, 0x02,
	0x6F12, 0x3081, 0x02,
	0x6F12, 0x4FF4, 0x02,
	0x6F12, 0x0060, 0x02,
	0x6F12, 0x3082, 0x02,
	0x6F12, 0x0420, 0x02,
	0x6F12, 0x3070, 0x02,
	0x6F12, 0x7070, 0x02,
	0x6F12, 0xB070, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0xFAFA, 0x02,
	0x6F12, 0x28B1, 0x02,
	0x6F12, 0x032F, 0x02,
	0x6F12, 0x06D0, 0x02,
	0x6F12, 0x022F, 0x02,
	0x6F12, 0x0AD0, 0x02,
	0x6F12, 0x6FB1, 0x02,
	0x6F12, 0x0EE0, 0x02,
	0x6F12, 0x0320, 0x02,
	0x6F12, 0x3070, 0x02,
	0x6F12, 0x0BE0, 0x02,
	0x6F12, 0x86F8, 0x02,
	0x6F12, 0x0090, 0x02,
	0x6F12, 0x86F8, 0x02,
	0x6F12, 0x0180, 0x02,
	0x6F12, 0xB470, 0x02,
	0x6F12, 0x05E0, 0x02,
	0x6F12, 0x86F8, 0x02,
	0x6F12, 0x0090, 0x02,
	0x6F12, 0x7470, 0x02,
	0x6F12, 0x01E0, 0x02,
	0x6F12, 0x86F8, 0x02,
	0x6F12, 0x0080, 0x02,
	0x6F12, 0x804E, 0x02,
	0x6F12, 0x7088, 0x02,
	0x6F12, 0x98B9, 0x02,
	0x6F12, 0x7D4F, 0x02,
	0x6F12, 0x403F, 0x02,
	0x6F12, 0x3846, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0xE1FA, 0x02,
	0x6F12, 0xB884, 0x02,
	0x6F12, 0x7A48, 0x02,
	0x6F12, 0x3838, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0xDCFA, 0x02,
	0x6F12, 0xB98C, 0x02,
	0x6F12, 0x0844, 0x02,
	0x6F12, 0xF884, 0x02,
	0x6F12, 0x7648, 0x02,
	0x6F12, 0x3038, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0xD5FA, 0x02,
	0x6F12, 0xF98C, 0x02,
	0x6F12, 0x0844, 0x02,
	0x6F12, 0x3885, 0x02,
	0x6F12, 0x0121, 0x02,
	0x6F12, 0x0020, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0xBAFA, 0x02,
	0x6F12, 0x7448, 0x02,
	0x6F12, 0x90F8, 0x02,
	0x6F12, 0x2E04, 0x02,
	0x6F12, 0x90B9, 0x02,
	0x6F12, 0x0120, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0xCCFA, 0x02,
	0x6F12, 0x7249, 0x02,
	0x6F12, 0x7148, 0x02,
	0x6F12, 0x724A, 0x02,
	0x6F12, 0x0A60, 0x02,
	0x6F12, 0x7249, 0x02,
	0x6F12, 0x0160, 0x02,
	0x6F12, 0x011D, 0x02,
	0x6F12, 0x6F48, 0x02,
	0x6F12, 0x714A, 0x02,
	0x6F12, 0x001D, 0x02,
	0x6F12, 0x0260, 0x02,
	0x6F12, 0x7148, 0x02,
	0x6F12, 0x0860, 0x02,
	0x6F12, 0x0020, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0xBCFA, 0x02,
	0x6F12, 0x3088, 0x02,
	0x6F12, 0x30BB, 0x02,
	0x6F12, 0xBAF8, 0x02,
	0x6F12, 0xF010, 0x02,
	0x6F12, 0x36F8, 0x02,
	0x6F12, 0x9C2F, 0x02,
	0x6F12, 0x01F1, 0x02,
	0x6F12, 0x1400, 0x02,
	0x6F12, 0x01F1, 0x02,
	0x6F12, 0x8041, 0x02,
	0x6F12, 0x1431, 0x02,
	0x6F12, 0x0A80, 0x02,
	0x6F12, 0x4FF0, 0x02,
	0x6F12, 0x8041, 0x02,
	0x6F12, 0x801C, 0x02,
	0x6F12, 0x7288, 0x02,
	0x6F12, 0x0A52, 0x02,
	0x6F12, 0x801C, 0x02,
	0x6F12, 0xB288, 0x02,
	0x6F12, 0x0A52, 0x02,
	0x6F12, 0x801C, 0x02,
	0x6F12, 0xF289, 0x02,
	0x6F12, 0x0A52, 0x02,
	0x6F12, 0x801C, 0x02,
	0x6F12, 0x328A, 0x02,
	0x6F12, 0x0A52, 0x02,
	0x6F12, 0x801C, 0x02,
	0x6F12, 0x728A, 0x02,
	0x6F12, 0x0A52, 0x02,
	0x6F12, 0x801C, 0x02,
	0x6F12, 0xB28B, 0x02,
	0x6F12, 0x0A52, 0x02,
	0x6F12, 0x801C, 0x02,
	0x6F12, 0xF28B, 0x02,
	0x6F12, 0x0A52, 0x02,
	0x6F12, 0x318C, 0x02,
	0x6F12, 0x9C3E, 0x02,
	0x6F12, 0x00F1, 0x02,
	0x6F12, 0x8040, 0x02,
	0x6F12, 0x801C, 0x02,
	0x6F12, 0x0180, 0x02,
	0x6F12, 0x7088, 0x02,
	0x6F12, 0x78B9, 0x02,
	0x6F12, 0xBAF8, 0x02,
	0x6F12, 0xF800, 0x02,
	0x6F12, 0x4FF0, 0x02,
	0x6F12, 0x8041, 0x02,
	0x6F12, 0x2030, 0x02,
	0x6F12, 0x36F8, 0x02,
	0x6F12, 0xC62F, 0x02,
	0x6F12, 0x0A52, 0x02,
	0x6F12, 0x801C, 0x02,
	0x6F12, 0x7288, 0x02,
	0x6F12, 0x0A52, 0x02,
	0x6F12, 0x00F1, 0x02,
	0x6F12, 0x8040, 0x02,
	0x6F12, 0x801C, 0x02,
	0x6F12, 0xB188, 0x02,
	0x6F12, 0x0180, 0x02,
	0x6F12, 0x95F8, 0x02,
	0x6F12, 0xDA00, 0x02,
	0x6F12, 0x40B9, 0x02,
	0x6F12, 0x95F8, 0x02,
	0x6F12, 0x8D02, 0x02,
	0x6F12, 0x28B1, 0x02,
	0x6F12, 0x95F8, 0x02,
	0x6F12, 0x8E02, 0x02,
	0x6F12, 0x10B1, 0x02,
	0x6F12, 0x4D49, 0x02,
	0x6F12, 0xA1F8, 0x02,
	0x6F12, 0x0090, 0x02,
	0x6F12, 0x3A48, 0x02,
	0x6F12, 0x90F8, 0x02,
	0x6F12, 0x8405, 0x02,
	0x6F12, 0x0028, 0x02,
	0x6F12, 0x04D0, 0x02,
	0x6F12, 0x4A48, 0x02,
	0x6F12, 0x0480, 0x02,
	0x6F12, 0x811F, 0x02,
	0x6F12, 0xA1F8, 0x02,
	0x6F12, 0x0080, 0x02,
	0x6F12, 0x05B0, 0x02,
	0x6F12, 0x9DE5, 0x02,
	0x6F12, 0x70B5, 0x02,
	0x6F12, 0x3E4D, 0x02,
	0x6F12, 0x95F8, 0x02,
	0x6F12, 0xA802, 0x02,
	0x6F12, 0x85F8, 0x02,
	0x6F12, 0xBC02, 0x02,
	0x6F12, 0x2846, 0x02,
	0x6F12, 0x90F8, 0x02,
	0x6F12, 0x2E04, 0x02,
	0x6F12, 0x80B9, 0x02,
	0x6F12, 0x4FF4, 0x02,
	0x6F12, 0x7041, 0x02,
	0x6F12, 0x0A46, 0x02,
	0x6F12, 0x47F6, 0x02,
	0x6F12, 0x1A40, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0x5DFA, 0x02,
	0x6F12, 0x3F49, 0x02,
	0x6F12, 0x4FF6, 0x02,
	0x6F12, 0xFF70, 0x02,
	0x6F12, 0x0880, 0x02,
	0x6F12, 0x0F22, 0x02,
	0x6F12, 0x1146, 0x02,
	0x6F12, 0x47F6, 0x02,
	0x6F12, 0x1E40, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0x53FA, 0x02,
	0x6F12, 0x2D4E, 0x02,
	0x6F12, 0x2648, 0x02,
	0x6F12, 0x7488, 0x02,
	0x6F12, 0x90F8, 0x02,
	0x6F12, 0xB804, 0x02,
	0x6F12, 0x08B1, 0x02,
	0x6F12, 0x6008, 0x02,
	0x6F12, 0x7080, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0x4EFA, 0x02,
	0x6F12, 0x2A49, 0x02,
	0x6F12, 0xA0B2, 0x02,
	0x6F12, 0x891F, 0x02,
	0x6F12, 0x7080, 0x02,
	0x6F12, 0x0880, 0x02,
	0x6F12, 0xB5F8, 0x02,
	0x6F12, 0xDE12, 0x02,
	0x6F12, 0xB1FB, 0x02,
	0x6F12, 0xF0F0, 0x02,
	0x6F12, 0x2649, 0x02,
	0x6F12, 0x0839, 0x02,
	0x6F12, 0x0880, 0x02,
	0x6F12, 0x0021, 0x02,
	0x6F12, 0x0846, 0x02,
	0x6F12, 0xFDF7, 0x02,
	0x6F12, 0xA0F8, 0x02,
	0x6F12, 0x2E4B, 0x02,
	0x6F12, 0x4FF4, 0x02,
	0x6F12, 0xF071, 0x02,
	0x6F12, 0x1888, 0x02,
	0x6F12, 0x00F1, 0x02,
	0x6F12, 0x8040, 0x02,
	0x6F12, 0x4181, 0x02,
	0x6F12, 0x2022, 0x02,
	0x6F12, 0x8281, 0x02,
	0x6F12, 0x5888, 0x02,
	0x6F12, 0x00F1, 0x02,
	0x6F12, 0x8040, 0x02,
	0x6F12, 0x4181, 0x02,
	0x6F12, 0x8281, 0x02,
	0x6F12, 0x2849, 0x02,
	0x6F12, 0x0020, 0x02,
	0x6F12, 0x0880, 0x02,
	0x6F12, 0x891C, 0x02,
	0x6F12, 0x0880, 0x02,
	0x6F12, 0x1048, 0x02,
	0x6F12, 0x224A, 0x02,
	0x6F12, 0x0121, 0x02,
	0x6F12, 0x90F8, 0x02,
	0x6F12, 0x8405, 0x02,
	0x6F12, 0x4232, 0x02,
	0x6F12, 0x00B1, 0x02,
	0x6F12, 0x1180, 0x02,
	0x6F12, 0x0E48, 0x02,
	0x6F12, 0x90F8, 0x02,
	0x6F12, 0xDA30, 0x02,
	0x6F12, 0x022B, 0x02,
	0x6F12, 0x04D3, 0x02,
	0x6F12, 0x1180, 0x02,
	0x6F12, 0x1C48, 0x02,
	0x6F12, 0x4630, 0x02,
	0x6F12, 0x0180, 0x02,
	0x6F12, 0x70BD, 0x02,
	0x6F12, 0x90F8, 0x02,
	0x6F12, 0x8D12, 0x02,
	0x6F12, 0x0029, 0x02,
	0x6F12, 0xFAD0, 0x02,
	0x6F12, 0x90F8, 0x02,
	0x6F12, 0x8E02, 0x02,
	0x6F12, 0x0028, 0x02,
	0x6F12, 0xF6D0, 0x02,
	0x6F12, 0x1549, 0x02,
	0x6F12, 0x0320, 0x02,
	0x6F12, 0x4831, 0x02,
	0x6F12, 0x0880, 0x02,
	0x6F12, 0x091D, 0x02,
	0x6F12, 0x0880, 0x02,
	0x6F12, 0x70BD, 0x02,
	0x6F12, 0x2001, 0x02,
	0x6F12, 0x2520, 0x02,
	0x6F12, 0x4000, 0x02,
	0x6F12, 0x83A6, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0x6210, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0x6A80, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0x8580, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0xA5F8, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0x4DE0, 0x02,
	0x6F12, 0x200B, 0x02,
	0x6F12, 0x8548, 0x02,
	0x6F12, 0x4000, 0x02,
	0x6F12, 0x960A, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0x4B00, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0x8BA0, 0x02,
	0x6F12, 0x2007, 0x02,
	0x6F12, 0x03F8, 0x02,
	0x6F12, 0x2008, 0x02,
	0x6F12, 0x03F8, 0x02,
	0x6F12, 0x0FF4, 0x02,
	0x6F12, 0x0FF2, 0x02,
	0x6F12, 0x0FF8, 0x02,
	0x6F12, 0x0FF6, 0x02,
	0x6F12, 0x0FFC, 0x02,
	0x6F12, 0x0FFA, 0x02,
	0x6F12, 0x0FFF, 0x02,
	0x6F12, 0x0FFE, 0x02,
	0x6F12, 0x4000, 0x02,
	0x6F12, 0xDE02, 0x02,
	0x6F12, 0x4000, 0x02,
	0x6F12, 0xBA08, 0x02,
	0x6F12, 0x4000, 0x02,
	0x6F12, 0x7C1C, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0x8044, 0x02,
	0x6F12, 0x4000, 0x02,
	0x6F12, 0x970C, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x30B5, 0x02,
	0x6F12, 0x0228, 0x02,
	0x6F12, 0x1ED0, 0x02,
	0x6F12, 0x0023, 0x02,
	0x6F12, 0x9C4C, 0x02,
	0x6F12, 0x6288, 0x02,
	0x6F12, 0x6568, 0x02,
	0x6F12, 0xC2F1, 0x02,
	0x6F12, 0x1F02, 0x02,
	0x6F12, 0x9340, 0x02,
	0x6F12, 0x1D43, 0x02,
	0x6F12, 0x6560, 0x02,
	0x6F12, 0x0328, 0x02,
	0x6F12, 0x15D0, 0x02,
	0x6F12, 0x0023, 0x02,
	0x6F12, 0xA568, 0x02,
	0x6F12, 0x9340, 0x02,
	0x6F12, 0x1D43, 0x02,
	0x6F12, 0xA560, 0x02,
	0x6F12, 0x0428, 0x02,
	0x6F12, 0x0BD1, 0x02,
	0x6F12, 0x01F0, 0x02,
	0x6F12, 0x0100, 0x02,
	0x6F12, 0x2369, 0x02,
	0x6F12, 0x9040, 0x02,
	0x6F12, 0x0343, 0x02,
	0x6F12, 0xC1F3, 0x02,
	0x6F12, 0x4000, 0x02,
	0x6F12, 0x2361, 0x02,
	0x6F12, 0xE168, 0x02,
	0x6F12, 0x9040, 0x02,
	0x6F12, 0x0143, 0x02,
	0x6F12, 0xE160, 0x02,
	0x6F12, 0x30BD, 0x02,
	0x6F12, 0x0123, 0x02,
	0x6F12, 0xDFE7, 0x02,
	0x6F12, 0x0123, 0x02,
	0x6F12, 0xE8E7, 0x02,
	0x6F12, 0x8B48, 0x02,
	0x6F12, 0x47F6, 0x02,
	0x6F12, 0x5C51, 0x02,
	0x6F12, 0x0180, 0x02,
	0x6F12, 0x0021, 0x02,
	0x6F12, 0x4160, 0x02,
	0x6F12, 0x8160, 0x02,
	0x6F12, 0xC160, 0x02,
	0x6F12, 0x0161, 0x02,
	0x6F12, 0x0121, 0x02,
	0x6F12, 0x4180, 0x02,
	0x6F12, 0x7047, 0x02,
	0x6F12, 0x10B5, 0x02,
	0x6F12, 0x854C, 0x02,
	0x6F12, 0x6FF0, 0x02,
	0x6F12, 0x0F01, 0x02,
	0x6F12, 0x6088, 0x02,
	0x6F12, 0x01EB, 0x02,
	0x6F12, 0x0010, 0x02,
	0x6F12, 0x80B2, 0x02,
	0x6F12, 0x40F0, 0x02,
	0x6F12, 0x0102, 0x02,
	0x6F12, 0xC90D, 0x02,
	0x6F12, 0x47F6, 0x02,
	0x6F12, 0x5450, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0x90F9, 0x02,
	0x6F12, 0x6068, 0x02,
	0x6F12, 0x7E4A, 0x02,
	0x6F12, 0x010C, 0x02,
	0x6F12, 0x1180, 0x02,
	0x6F12, 0x911E, 0x02,
	0x6F12, 0x0880, 0x02,
	0x6F12, 0xA068, 0x02,
	0x6F12, 0x010C, 0x02,
	0x6F12, 0x121D, 0x02,
	0x6F12, 0x1180, 0x02,
	0x6F12, 0x911E, 0x02,
	0x6F12, 0x0880, 0x02,
	0x6F12, 0xE068, 0x02,
	0x6F12, 0x784A, 0x02,
	0x6F12, 0x010C, 0x02,
	0x6F12, 0x0C32, 0x02,
	0x6F12, 0x1180, 0x02,
	0x6F12, 0x911E, 0x02,
	0x6F12, 0x0880, 0x02,
	0x6F12, 0x2069, 0x02,
	0x6F12, 0x010C, 0x02,
	0x6F12, 0x121F, 0x02,
	0x6F12, 0x1180, 0x02,
	0x6F12, 0x911E, 0x02,
	0x6F12, 0x0880, 0x02,
	0x6F12, 0x10BD, 0x02,
	0x6F12, 0x70B5, 0x02,
	0x6F12, 0x1D46, 0x02,
	0x6F12, 0x704C, 0x02,
	0x6F12, 0x1646, 0x02,
	0x6F12, 0x0346, 0x02,
	0x6F12, 0x0A46, 0x02,
	0x6F12, 0x2088, 0x02,
	0x6F12, 0x1946, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0x76F9, 0x02,
	0x6F12, 0x2946, 0x02,
	0x6F12, 0x3046, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0x97FF, 0x02,
	0x6F12, 0x2088, 0x02,
	0x6F12, 0x801D, 0x02,
	0x6F12, 0x2080, 0x02,
	0x6F12, 0x6088, 0x02,
	0x6F12, 0x401C, 0x02,
	0x6F12, 0x6080, 0x02,
	0x6F12, 0x70BD, 0x02,
	0x6F12, 0x40EA, 0x02,
	0x6F12, 0x0220, 0x02,
	0x6F12, 0x0023, 0x02,
	0x6F12, 0x80B2, 0x02,
	0x6F12, 0x0222, 0x02,
	0x6F12, 0xE4E7, 0x02,
	0x6F12, 0x0023, 0x02,
	0x6F12, 0x40F4, 0x02,
	0x6F12, 0x0060, 0x02,
	0x6F12, 0x0222, 0x02,
	0x6F12, 0xDFE7, 0x02,
	0x6F12, 0x0023, 0x02,
	0x6F12, 0x40F4, 0x02,
	0x6F12, 0x0060, 0x02,
	0x6F12, 0x0322, 0x02,
	0x6F12, 0x1946, 0x02,
	0x6F12, 0xD9E7, 0x02,
	0x6F12, 0x1346, 0x02,
	0x6F12, 0x40F4, 0x02,
	0x6F12, 0x0060, 0x02,
	0x6F12, 0x0422, 0x02,
	0x6F12, 0xD4E7, 0x02,
	0x6F12, 0x2DE9, 0x02,
	0x6F12, 0xF047, 0x02,
	0x6F12, 0x5C4F, 0x02,
	0x6F12, 0x5D4B, 0x02,
	0x6F12, 0xD7F8, 0x02,
	0x6F12, 0x8800, 0x02,
	0x6F12, 0x20F0, 0x02,
	0x6F12, 0x2005, 0x02,
	0x6F12, 0x93F8, 0x02,
	0x6F12, 0x2900, 0x02,
	0x6F12, 0x08B1, 0x02,
	0x6F12, 0x0424, 0x02,
	0x6F12, 0x00E0, 0x02,
	0x6F12, 0x0624, 0x02,
	0x6F12, 0x584E, 0x02,
	0x6F12, 0x96F8, 0x02,
	0x6F12, 0x9100, 0x02,
	0x6F12, 0x00B1, 0x02,
	0x6F12, 0x6400, 0x02,
	0x6F12, 0x93F8, 0x02,
	0x6F12, 0x2B00, 0x02,
	0x6F12, 0x4FF4, 0x02,
	0x6F12, 0x407C, 0x02,
	0x6F12, 0xC0F1, 0x02,
	0x6F12, 0x0101, 0x02,
	0x6F12, 0x4942, 0x02,
	0x6F12, 0x0CEB, 0x02,
	0x6F12, 0x8101, 0x02,
	0x6F12, 0x04EB, 0x02,
	0x6F12, 0x8002, 0x02,
	0x6F12, 0xB1FB, 0x02,
	0x6F12, 0xF2F1, 0x02,
	0x6F12, 0xDFF8, 0x02,
	0x6F12, 0x4081, 0x02,
	0x6F12, 0x88F8, 0x02,
	0x6F12, 0x6717, 0x02,
	0x6F12, 0x93F8, 0x02,
	0x6F12, 0x2A10, 0x02,
	0x6F12, 0x09B1, 0x02,
	0x6F12, 0x88F8, 0x02,
	0x6F12, 0x6717, 0x02,
	0x6F12, 0x4C49, 0x02,
	0x6F12, 0x9946, 0x02,
	0x6F12, 0x91F8, 0x02,
	0x6F12, 0x6717, 0x02,
	0x6F12, 0x5143, 0x02,
	0x6F12, 0x00B9, 0x02,
	0x6F12, 0x091D, 0x02,
	0x6F12, 0xC81C, 0x02,
	0x6F12, 0x4FF6, 0x02,
	0x6F12, 0xFC72, 0x02,
	0x6F12, 0x1040, 0x02,
	0x6F12, 0x464A, 0x02,
	0x6F12, 0xE046, 0x02,
	0x6F12, 0x4145, 0x02,
	0x6F12, 0xA2F8, 0x02,
	0x6F12, 0x8A07, 0x02,
	0x6F12, 0x06D9, 0x02,
	0x6F12, 0x4FF4, 0x02,
	0x6F12, 0x2C60, 0x02,
	0x6F12, 0xFEF7, 0x02,
	0x6F12, 0x74FD, 0x02,
	0x6F12, 0x4149, 0x02,
	0x6F12, 0xA1F8, 0x02,
	0x6F12, 0x8A87, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0x5DFF, 0x02,
	0x6F12, 0x0121, 0x02,
	0x6F12, 0x0C20, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0xA9FF, 0x02,
	0x6F12, 0x0020, 0x02,
	0x6F12, 0xD7F8, 0x02,
	0x6F12, 0x8810, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0xA4FF, 0x02,
	0x6F12, 0x0021, 0x02,
	0x6F12, 0x2020, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0xA0FF, 0x02,
	0x6F12, 0x97F8, 0x02,
	0x6F12, 0xAA20, 0x02,
	0x6F12, 0x45F0, 0x02,
	0x6F12, 0x0101, 0x02,
	0x6F12, 0x0020, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0x93FF, 0x02,
	0x6F12, 0x3548, 0x02,
	0x6F12, 0xB0F8, 0x02,
	0x6F12, 0x5E07, 0x02,
	0x6F12, 0x40F0, 0x02,
	0x6F12, 0x8001, 0x02,
	0x6F12, 0x1820, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0x91FF, 0x02,
	0x6F12, 0x0021, 0x02,
	0x6F12, 0x0C20, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0x8DFF, 0x02,
	0x6F12, 0x0022, 0x02,
	0x6F12, 0x1146, 0x02,
	0x6F12, 0x1420, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0x93FF, 0x02,
	0x6F12, 0x4FF4, 0x02,
	0x6F12, 0x0061, 0x02,
	0x6F12, 0x0820, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0x83FF, 0x02,
	0x6F12, 0xA0B2, 0x02,
	0x6F12, 0x40F4, 0x02,
	0x6F12, 0x8031, 0x02,
	0x6F12, 0x2020, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0x7DFF, 0x02,
	0x6F12, 0x45F0, 0x02,
	0x6F12, 0x0201, 0x02,
	0x6F12, 0x0020, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0x78FF, 0x02,
	0x6F12, 0x99F8, 0x02,
	0x6F12, 0x2900, 0x02,
	0x6F12, 0x4F46, 0x02,
	0x6F12, 0x08B1, 0x02,
	0x6F12, 0x0122, 0x02,
	0x6F12, 0x00E0, 0x02,
	0x6F12, 0x0322, 0x02,
	0x6F12, 0x0221, 0x02,
	0x6F12, 0x1420, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0x78FF, 0x02,
	0x6F12, 0x0025, 0x02,
	0x6F12, 0x03E0, 0x02,
	0x6F12, 0x1C20, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0x6DFF, 0x02,
	0x6F12, 0x6D1C, 0x02,
	0x6F12, 0xA542, 0x02,
	0x6F12, 0xF9D3, 0x02,
	0x6F12, 0x97F8, 0x02,
	0x6F12, 0x2900, 0x02,
	0x6F12, 0x60B9, 0x02,
	0x6F12, 0x1548, 0x02,
	0x6F12, 0x96F8, 0x02,
	0x6F12, 0x9110, 0x02,
	0x6F12, 0x1430, 0x02,
	0x6F12, 0x09B1, 0x02,
	0x6F12, 0x0621, 0x02,
	0x6F12, 0x00E0, 0x02,
	0x6F12, 0x0321, 0x02,
	0x6F12, 0x0180, 0x02,
	0x6F12, 0x1149, 0x02,
	0x6F12, 0x0220, 0x02,
	0x6F12, 0x1231, 0x02,
	0x6F12, 0x0880, 0x02,
	0x6F12, 0x0121, 0x02,
	0x6F12, 0x0C20, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0x51FF, 0x02,
	0x6F12, 0xFFF7, 0x02,
	0x6F12, 0x0BFF, 0x02,
	0x6F12, 0x1048, 0x02,
	0x6F12, 0x96F8, 0x02,
	0x6F12, 0x9110, 0x02,
	0x6F12, 0x90F8, 0x02,
	0x6F12, 0x6707, 0x02,
	0x6F12, 0x0001, 0x02,
	0x6F12, 0x19B1, 0x02,
	0x6F12, 0x4FF6, 0x02,
	0x6F12, 0xFF71, 0x02,
	0x6F12, 0x01EA, 0x02,
	0x6F12, 0x4000, 0x02,
	0x6F12, 0x97F8, 0x02,
	0x6F12, 0x2E10, 0x02,
	0x6F12, 0xBDE8, 0x02,
	0x6F12, 0xF047, 0x02,
	0x6F12, 0x40EA, 0x02,
	0x6F12, 0x8102, 0x02,
	0x6F12, 0x40F6, 0x02,
	0x6F12, 0xFC71, 0x02,
	0x6F12, 0x47F6, 0x02,
	0x6F12, 0x2850, 0x02,
	0x6F12, 0x00F0, 0x02,
	0x6F12, 0x93B8, 0x02,
	0x6F12, 0x2001, 0x02,
	0x6F12, 0x00D0, 0x02,
	0x6F12, 0x4000, 0x02,
	0x6F12, 0x7D3C, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0x10D0, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0x4730, 0x02,
	0x6F12, 0x2001, 0x02,
	0x6F12, 0x2480, 0x02,
	0x6F12, 0x2000, 0x02,
	0x6F12, 0x6210, 0x02,
	0x6F12, 0x41F6, 0x02,
	0x6F12, 0x210C, 0x02,
	0x6F12, 0xC0F2, 0x02,
	0x6F12, 0x020C, 0x02,
	0x6F12, 0x6047, 0x02,
	0x6F12, 0x4DF2, 0x02,
	0x6F12, 0xF54C, 0x02,
	0x6F12, 0xC0F2, 0x02,
	0x6F12, 0x010C, 0x02,
	0x6F12, 0x6047, 0x02,
	0x6F12, 0x4DF2, 0x02,
	0x6F12, 0x215C, 0x02,
	0x6F12, 0xC0F2, 0x02,
	0x6F12, 0x010C, 0x02,
	0x6F12, 0x6047, 0x02,
	0x6F12, 0x4DF6, 0x02,
	0x6F12, 0x2D2C, 0x02,
	0x6F12, 0xC0F2, 0x02,
	0x6F12, 0x010C, 0x02,
	0x6F12, 0x6047, 0x02,
	0x6F12, 0x4CF2, 0x02,
	0x6F12, 0xA31C, 0x02,
	0x6F12, 0xC0F2, 0x02,
	0x6F12, 0x010C, 0x02,
	0x6F12, 0x6047, 0x02,
	0x6F12, 0x4CF2, 0x02,
	0x6F12, 0xED1C, 0x02,
	0x6F12, 0xC0F2, 0x02,
	0x6F12, 0x010C, 0x02,
	0x6F12, 0x6047, 0x02,
	0x6F12, 0x4CF2, 0x02,
	0x6F12, 0x472C, 0x02,
	0x6F12, 0xC0F2, 0x02,
	0x6F12, 0x010C, 0x02,
	0x6F12, 0x6047, 0x02,
	0x6F12, 0x47F2, 0x02,
	0x6F12, 0x895C, 0x02,
	0x6F12, 0xC0F2, 0x02,
	0x6F12, 0x010C, 0x02,
	0x6F12, 0x6047, 0x02,
	0x6F12, 0x4FF6, 0x02,
	0x6F12, 0x656C, 0x02,
	0x6F12, 0xC0F2, 0x02,
	0x6F12, 0x010C, 0x02,
	0x6F12, 0x6047, 0x02,
	0x6F12, 0x4FF6, 0x02,
	0x6F12, 0x6D6C, 0x02,
	0x6F12, 0xC0F2, 0x02,
	0x6F12, 0x010C, 0x02,
	0x6F12, 0x6047, 0x02,
	0x6F12, 0x47F2, 0x02,
	0x6F12, 0x0F7C, 0x02,
	0x6F12, 0xC0F2, 0x02,
	0x6F12, 0x010C, 0x02,
	0x6028, 0x2001, 0x02,
	0x602A, 0x0000, 0x02,
	0x6F12, 0x6047, 0x02,
	0x6F12, 0x48F2, 0x02,
	0x6F12, 0x834C, 0x02,
	0x6F12, 0xC0F2, 0x02,
	0x6F12, 0x000C, 0x02,
	0x6F12, 0x6047, 0x02,
	0x6F12, 0x4FF6, 0x02,
	0x6F12, 0x756C, 0x02,
	0x6F12, 0xC0F2, 0x02,
	0x6F12, 0x010C, 0x02,
	0x6F12, 0x6047, 0x02,
	0x6F12, 0x4BF6, 0x02,
	0x6F12, 0x550C, 0x02,
	0x6F12, 0xC0F2, 0x02,
	0x6F12, 0x010C, 0x02,
	0x6F12, 0x6047, 0x02,
	0x6F12, 0x4DF2, 0x02,
	0x6F12, 0xE55C, 0x02,
	0x6F12, 0xC0F2, 0x02,
	0x6F12, 0x010C, 0x02,
	0x6F12, 0x6047, 0x02,
	0x6F12, 0x47F6, 0x02,
	0x6F12, 0xC37C, 0x02,
	0x6F12, 0xC0F2, 0x02,
	0x6F12, 0x000C, 0x02,
	0x6F12, 0x6047, 0x02,
	0x6F12, 0x47F6, 0x02,
	0x6F12, 0xCB7C, 0x02,
	0x6F12, 0xC0F2, 0x02,
	0x6F12, 0x000C, 0x02,
	0x6F12, 0x6047, 0x02,
	0x6F12, 0x4DF2, 0x02,
	0x6F12, 0x855C, 0x02,
	0x6F12, 0xC0F2, 0x02,
	0x6F12, 0x010C, 0x02,
	0x6F12, 0x6047, 0x02,
	0x6F12, 0x40F6, 0x02,
	0x6F12, 0x0D7C, 0x02,
	0x6F12, 0xC0F2, 0x02,
	0x6F12, 0x000C, 0x02,
	0x6F12, 0x6047, 0x02,
	0x6F12, 0x49F2, 0x02,
	0x6F12, 0xC92C, 0x02,
	0x6F12, 0xC0F2, 0x02,
	0x6F12, 0x000C, 0x02,
	0x6F12, 0x6047, 0x02,
	0x6F12, 0x45F6, 0x02,
	0x6F12, 0x834C, 0x02,
	0x6F12, 0xC0F2, 0x02,
	0x6F12, 0x010C, 0x02,
	0x6F12, 0x6047, 0x02,
	0x6F12, 0x4DF6, 0x02,
	0x6F12, 0x792C, 0x02,
	0x6F12, 0xC0F2, 0x02,
	0x6F12, 0x010C, 0x02,
	0x6F12, 0x6047, 0x02,
	0x6F12, 0x48F2, 0x02,
	0x6F12, 0xDB1C, 0x02,
	0x6F12, 0xC0F2, 0x02,
	0x6F12, 0x000C, 0x02,
	0x6F12, 0x6047, 0x02,
	0x6F12, 0x48F2, 0x02,
	0x6F12, 0x0F2C, 0x02,
	0x6F12, 0xC0F2, 0x02,
	0x6F12, 0x000C, 0x02,
	0x6F12, 0x6047, 0x02,
	0x6F12, 0x43F2, 0x02,
	0x6F12, 0x551C, 0x02,
	0x6F12, 0xC0F2, 0x02,
	0x6F12, 0x010C, 0x02,
	0x6F12, 0x6047, 0x02,
	0x6F12, 0x4AF6, 0x02,
	0x6F12, 0x4D0C, 0x02,
	0x6F12, 0xC0F2, 0x02,
	0x6F12, 0x010C, 0x02,
	0x6F12, 0x6047, 0x02,
	0x6F12, 0x4DF6, 0x02,
	0x6F12, 0xB12C, 0x02,
	0x6F12, 0xC0F2, 0x02,
	0x6F12, 0x010C, 0x02,
	0x6F12, 0x6047, 0x02,
	0x6F12, 0x48F2, 0x02,
	0x6F12, 0xCB1C, 0x02,
	0x6F12, 0xC0F2, 0x02,
	0x6F12, 0x000C, 0x02,
	0x6F12, 0x6047, 0x02,
	0x6F12, 0x42F2, 0x02,
	0x6F12, 0x2D4C, 0x02,
	0x6F12, 0xC0F2, 0x02,
	0x6F12, 0x010C, 0x02,
	0x6F12, 0x6047, 0x02,
	0x6F12, 0x43F6, 0x02,
	0x6F12, 0x657C, 0x02,
	0x6F12, 0xC0F2, 0x02,
	0x6F12, 0x000C, 0x02,
	0x6F12, 0x6047, 0x02,
	0x6F12, 0x0001, 0x02,
	0x6F12, 0x0004, 0x02,
	0x6F12, 0x0008, 0x02,
	0x6F12, 0x0010, 0x02,
	/*-- TnP */
	/*++ Global */
	0x6028, 0x2000, 0x02,
	0x602A, 0x14CE, 0x02,
	0x6F12, 0x00CC, 0x02,
	0x602A, 0x1918, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x191C, 0x02,
	0x6F12, 0x8091, 0x02,
	0x602A, 0x1920, 0x02,
	0x6F12, 0x8093, 0x02,
	0x602A, 0x1924, 0x02,
	0x6F12, 0x80A4, 0x02,
	0x602A, 0x1928, 0x02,
	0x6F12, 0x80A6, 0x02,
	0x602A, 0x192C, 0x02,
	0x6F12, 0x80B7, 0x02,
	0x602A, 0x1930, 0x02,
	0x6F12, 0x82F8, 0x02,
	0x602A, 0x1934, 0x02,
	0x6F12, 0x8309, 0x02,
	0x602A, 0x1938, 0x02,
	0x6F12, 0x830B, 0x02,
	0x602A, 0x193C, 0x02,
	0x6F12, 0x831C, 0x02,
	0x602A, 0x1940, 0x02,
	0x6F12, 0x831E, 0x02,
	0x602A, 0x1944, 0x02,
	0x6F12, 0x832F, 0x02,
	0x602A, 0x1476, 0x02,
	0x6F12, 0x0316, 0x02,
	0x602A, 0x1F18, 0x02,
	0x6F12, 0x00AE, 0x02,
	0x602A, 0x224A, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x1C28, 0x02,
	0x6F12, 0x090F, 0x02,
	0x602A, 0x15EA, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x1C3A, 0x02,
	0x6F12, 0x1FE9, 0x02,
	0x6028, 0x4000, 0x02,
	0x602A, 0xF474, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0xF472, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0xF41A, 0x02,
	0x6F12, 0x0010, 0x02,
	0x602A, 0xF446, 0x02,
	0x6F12, 0x000E, 0x02,
	0x6F12, 0x001B, 0x02,
	0x6F12, 0x0010, 0x02,
	0x602A, 0xF450, 0x02,
	0x6F12, 0x0011, 0x02,
	0x6F12, 0x000E, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x1C34, 0x02,
	0x6F12, 0x4408, 0x02,
	0x602A, 0x138A, 0x02,
	0x6F12, 0x0046, 0x02,
	0x602A, 0x139A, 0x02,
	0x6F12, 0x003A, 0x02,
	0x602A, 0x13EE, 0x02,
	0x6F12, 0x003F, 0x02,
	0x602A, 0x1C30, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0280, 0x02,
	0x602A, 0x1C2A, 0x02,
	0x6F12, 0x0103, 0x02,
	/*-- Global */
};

/* 0. 2x5-26Mhz B8.2FD 2880x2156 2392MHz 30fps */
const u32 sensor_2x5_setfile_A_2880x2156_30fps[] = {
	0x6028, 0x2000, 0x02,
	0x602A, 0x11B0, 0x02,
	0x6F12, 0x0001, 0x02,
	0x602A, 0x11B2, 0x02,
	0x6F12, 0x0200, 0x02,
	0x602A, 0x11B6, 0x02,
	0x6F12, 0x0200, 0x02,
	0x602A, 0x11BA, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6028, 0x2001, 0x02,
	0x602A, 0x2AA4, 0x02,
	0x6F12, 0x01A7, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x12A2, 0x02,
	0x6F12, 0x0001, 0x02,
	0x602A, 0x2520, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x2DFA, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6028, 0x2001, 0x02,
	0x602A, 0x2A36, 0x02,
	0x6F12, 0x0101, 0x02,
	0x602A, 0x2AA4, 0x02,
	0x6F12, 0x01A7, 0x02,
	0xFCFC, 0x4000, 0x02,
	0x6000, 0x0005, 0x02,
	0xF466, 0x07FB, 0x02,
	0xF468, 0x2000, 0x02,
	0x0110, 0x1002, 0x02,
	0x0114, 0x0300, 0x02,
	0x0344, 0x0008, 0x02,
	0x0346, 0x000C, 0x02,
	0x0348, 0x1687, 0x02,
	0x034A, 0x10E3, 0x02,
	0x034C, 0x0B40, 0x02,
	0x034E, 0x086C, 0x02,
	0x0350, 0x0000, 0x02,
	0x0352, 0x0000, 0x02,
	0x0380, 0x0001, 0x02,
	0x0382, 0x0001, 0x02,
	0x0384, 0x0002, 0x02,
	0x0386, 0x0002, 0x02,
	0x0900, 0x0112, 0x02,
	0x0404, 0x1000, 0x02,
	0x0402, 0x1010, 0x02,
	0x0400, 0x2010, 0x02,
	0x0136, 0x1A00, 0x02,
	0x0304, 0x0004, 0x02,
	0x030C, 0x0001, 0x02,
	0x0306, 0x00DC, 0x02,
	0x0302, 0x0001, 0x02,
	0x0300, 0x0003, 0x02,
	0x030E, 0x0004, 0x02,
	0x0312, 0x0000, 0x02,
	0x0310, 0x0170, 0x02,
	0x030A, 0x0001, 0x02,
	0x0308, 0x0008, 0x02,
	0x0342, 0x1A80, 0x02,
	0x0340, 0x124C, 0x02,
	0x021E, 0x0000, 0x02,
	0x0D00, 0x0000, 0x02,
	0x0D02, 0x0001, 0x02,
	0x0114, 0x0300, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x2DF6, 0x02,
	0x6F12, 0x0100, 0x02,
	0x6028, 0x4000, 0x02,
	0x602A, 0x0B06, 0x02,
	0x6F12, 0x0101, 0x02,
	0x602A, 0x0B08, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x0FEA, 0x02,
	0x6F12, 0x2240, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x2E12, 0x02,
	0x6F12, 0x0100, 0x02,
	0x602A, 0x2E16, 0x02,
	0x6F12, 0x0101, 0x02,
	0x6028, 0x4000, 0x02,
	0x602A, 0x0B00, 0x02,
	0x6F12, 0x004D, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x3280, 0x02,
	0x6F12, 0x0100, 0x02,
};

/* 1. 2x5-26Mhz B9.2FD 2880x1620 2392MHz 30fps */
const u32 sensor_2x5_setfile_A_2880x1620_30fps[] = {
	0x6028, 0x2000, 0x02,
	0x602A, 0x11B0, 0x02,
	0x6F12, 0x0001, 0x02,
	0x602A, 0x11B2, 0x02,
	0x6F12, 0x0200, 0x02,
	0x602A, 0x11B6, 0x02,
	0x6F12, 0x0200, 0x02,
	0x602A, 0x11BA, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6028, 0x2001, 0x02,
	0x602A, 0x2AA4, 0x02,
	0x6F12, 0x01A7, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x12A2, 0x02,
	0x6F12, 0x0001, 0x02,
	0x602A, 0x2520, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x2DFA, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6028, 0x2001, 0x02,
	0x602A, 0x2A36, 0x02,
	0x6F12, 0x0101, 0x02,
	0x602A, 0x2AA4, 0x02,
	0x6F12, 0x01A7, 0x02,
	0xFCFC, 0x4000, 0x02,
	0x6000, 0x0005, 0x02,
	0xF466, 0x07FB, 0x02,
	0xF468, 0x2000, 0x02,
	0x0110, 0x1002, 0x02,
	0x0114, 0x0300, 0x02,
	0x0344, 0x0008, 0x02,
	0x0346, 0x0224, 0x02,
	0x0348, 0x1687, 0x02,
	0x034A, 0x0ECB, 0x02,
	0x034C, 0x0B40, 0x02,
	0x034E, 0x0654, 0x02,
	0x0350, 0x0000, 0x02,
	0x0352, 0x0000, 0x02,
	0x0380, 0x0001, 0x02,
	0x0382, 0x0001, 0x02,
	0x0384, 0x0002, 0x02,
	0x0386, 0x0002, 0x02,
	0x0900, 0x0112, 0x02,
	0x0404, 0x1000, 0x02,
	0x0402, 0x1010, 0x02,
	0x0400, 0x2010, 0x02,
	0x0136, 0x1A00, 0x02,
	0x0304, 0x0004, 0x02,
	0x030C, 0x0001, 0x02,
	0x0306, 0x00DC, 0x02,
	0x0302, 0x0001, 0x02,
	0x0300, 0x0003, 0x02,
	0x030E, 0x0004, 0x02,
	0x0312, 0x0000, 0x02,
	0x0310, 0x0170, 0x02,
	0x030A, 0x0001, 0x02,
	0x0308, 0x0008, 0x02,
	0x0342, 0x1A80, 0x02,
	0x0340, 0x124C, 0x02,
	0x021E, 0x0000, 0x02,
	0x0D00, 0x0000, 0x02,
	0x0D02, 0x0001, 0x02,
	0x0114, 0x0300, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x2DF6, 0x02,
	0x6F12, 0x0100, 0x02,
	0x6028, 0x4000, 0x02,
	0x602A, 0x0B06, 0x02,
	0x6F12, 0x0101, 0x02,
	0x602A, 0x0B08, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x0FEA, 0x02,
	0x6F12, 0x2240, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x2E12, 0x02,
	0x6F12, 0x0100, 0x02,
	0x602A, 0x2E16, 0x02,
	0x6F12, 0x0101, 0x02,
	0x6028, 0x4000, 0x02,
	0x602A, 0x0B00, 0x02,
	0x6F12, 0x004D, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x3280, 0x02,
	0x6F12, 0x0100, 0x02,
};

/* 2. 2x5-26Mhz B20.2FD 2880x1332 2392MHz 30fps */
const u32 sensor_2x5_setfile_A_2880x1332_30fps[] = {
	0x6028, 0x2000, 0x02,
	0x602A, 0x11B0, 0x02,
	0x6F12, 0x0001, 0x02,
	0x602A, 0x11B2, 0x02,
	0x6F12, 0x0200, 0x02,
	0x602A, 0x11B6, 0x02,
	0x6F12, 0x0200, 0x02,
	0x602A, 0x11BA, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6028, 0x2001, 0x02,
	0x602A, 0x2AA4, 0x02,
	0x6F12, 0x01A7, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x12A2, 0x02,
	0x6F12, 0x0001, 0x02,
	0x602A, 0x2520, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x2DFA, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6028, 0x2001, 0x02,
	0x602A, 0x2A36, 0x02,
	0x6F12, 0x0101, 0x02,
	0x602A, 0x2AA4, 0x02,
	0x6F12, 0x01A7, 0x02,
	0xFCFC, 0x4000, 0x02,
	0x6000, 0x0005, 0x02,
	0xF466, 0x07FB, 0x02,
	0xF468, 0x2000, 0x02,
	0x0110, 0x1002, 0x02,
	0x0114, 0x0300, 0x02,
	0x0344, 0x0008, 0x02,
	0x0346, 0x0344, 0x02,
	0x0348, 0x1687, 0x02,
	0x034A, 0x0DAB, 0x02,
	0x034C, 0x0B40, 0x02,
	0x034E, 0x0534, 0x02,
	0x0350, 0x0000, 0x02,
	0x0352, 0x0000, 0x02,
	0x0380, 0x0001, 0x02,
	0x0382, 0x0001, 0x02,
	0x0384, 0x0002, 0x02,
	0x0386, 0x0002, 0x02,
	0x0900, 0x0112, 0x02,
	0x0404, 0x1000, 0x02,
	0x0402, 0x1010, 0x02,
	0x0400, 0x2010, 0x02,
	0x0136, 0x1A00, 0x02,
	0x0304, 0x0004, 0x02,
	0x030C, 0x0001, 0x02,
	0x0306, 0x00DC, 0x02,
	0x0302, 0x0001, 0x02,
	0x0300, 0x0003, 0x02,
	0x030E, 0x0004, 0x02,
	0x0312, 0x0000, 0x02,
	0x0310, 0x0170, 0x02,
	0x030A, 0x0001, 0x02,
	0x0308, 0x0008, 0x02,
	0x0342, 0x0D40, 0x02,
	0x0340, 0x2498, 0x02,
	0x021E, 0x0000, 0x02,
	0x0D00, 0x0000, 0x02,
	0x0D02, 0x0001, 0x02,
	0x0114, 0x0300, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x2DF6, 0x02,
	0x6F12, 0x0100, 0x02,
	0x6028, 0x4000, 0x02,
	0x602A, 0x0B06, 0x02,
	0x6F12, 0x0101, 0x02,
	0x602A, 0x0B08, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x0FEA, 0x02,
	0x6F12, 0x2240, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x2E12, 0x02,
	0x6F12, 0x0100, 0x02,
	0x602A, 0x2E16, 0x02,
	0x6F12, 0x0101, 0x02,
	0x6028, 0x4000, 0x02,
	0x602A, 0x0B00, 0x02,
	0x6F12, 0x004D, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x3280, 0x02,
	0x6F12, 0x0100, 0x02,
};

/* 3. 2x5-26Mhz B11.2FD 2156x2156 2392MHz 30fps */
const u32 sensor_2x5_setfile_A_2156x2156_30fps[] = {
	0x6028, 0x2000, 0x02,
	0x602A, 0x11B0, 0x02,
	0x6F12, 0x0001, 0x02,
	0x602A, 0x11B2, 0x02,
	0x6F12, 0x0200, 0x02,
	0x602A, 0x11B6, 0x02,
	0x6F12, 0x0200, 0x02,
	0x602A, 0x11BA, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6028, 0x2001, 0x02,
	0x602A, 0x2AA4, 0x02,
	0x6F12, 0x01A7, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x12A2, 0x02,
	0x6F12, 0x0001, 0x02,
	0x602A, 0x2520, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x2DFA, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6028, 0x2001, 0x02,
	0x602A, 0x2A36, 0x02,
	0x6F12, 0x0101, 0x02,
	0x602A, 0x2AA4, 0x02,
	0x6F12, 0x01A7, 0x02,
	0xFCFC, 0x4000, 0x02,
	0x6000, 0x0005, 0x02,
	0xF466, 0x07FB, 0x02,
	0xF468, 0x2000, 0x02,
	0x0110, 0x1002, 0x02,
	0x0114, 0x0300, 0x02,
	0x0344, 0x02DC, 0x02,
	0x0346, 0x000C, 0x02,
	0x0348, 0x13B3, 0x02,
	0x034A, 0x10E3, 0x02,
	0x034C, 0x086C, 0x02,
	0x034E, 0x086C, 0x02,
	0x0350, 0x0000, 0x02,
	0x0352, 0x0000, 0x02,
	0x0380, 0x0001, 0x02,
	0x0382, 0x0001, 0x02,
	0x0384, 0x0002, 0x02,
	0x0386, 0x0002, 0x02,
	0x0900, 0x0112, 0x02,
	0x0404, 0x1000, 0x02,
	0x0402, 0x1010, 0x02,
	0x0400, 0x2010, 0x02,
	0x0136, 0x1A00, 0x02,
	0x0304, 0x0004, 0x02,
	0x030C, 0x0001, 0x02,
	0x0306, 0x00DC, 0x02,
	0x0302, 0x0001, 0x02,
	0x0300, 0x0003, 0x02,
	0x030E, 0x0004, 0x02,
	0x0312, 0x0000, 0x02,
	0x0310, 0x0170, 0x02,
	0x030A, 0x0001, 0x02,
	0x0308, 0x0008, 0x02,
	0x0342, 0x1A80, 0x02,
	0x0340, 0x124C, 0x02,
	0x021E, 0x0000, 0x02,
	0x0D00, 0x0000, 0x02,
	0x0D02, 0x0001, 0x02,
	0x0114, 0x0300, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x2DF6, 0x02,
	0x6F12, 0x0100, 0x02,
	0x6028, 0x4000, 0x02,
	0x602A, 0x0B06, 0x02,
	0x6F12, 0x0101, 0x02,
	0x602A, 0x0B08, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x0FEA, 0x02,
	0x6F12, 0x2240, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x2E12, 0x02,
	0x6F12, 0x0100, 0x02,
	0x602A, 0x2E16, 0x02,
	0x6F12, 0x0101, 0x02,
	0x6028, 0x4000, 0x02,
	0x602A, 0x0B00, 0x02,
	0x6F12, 0x004D, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x3280, 0x02,
	0x6F12, 0x0100, 0x02,
};

/* 4. 2x5-26Mhz B14.4FD 2880x2156 2392MHz 120fps */
const u32 sensor_2x5_setfile_A_2880x2156_120fps[] = {
	0x6028, 0x2000, 0x02,
	0x602A, 0x11B0, 0x02,
	0x6F12, 0x0001, 0x02,
	0x602A, 0x11B2, 0x02,
	0x6F12, 0x0200, 0x02,
	0x602A, 0x11B6, 0x02,
	0x6F12, 0x0200, 0x02,
	0x602A, 0x11BA, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6028, 0x2001, 0x02,
	0x602A, 0x2AA4, 0x02,
	0x6F12, 0x00A7, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x12A2, 0x02,
	0x6F12, 0x0101, 0x02,
	0x602A, 0x2520, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x2DFA, 0x02,
	0x6F12, 0x0002, 0x02,
	0x6028, 0x2001, 0x02,
	0x602A, 0x2A36, 0x02,
	0x6F12, 0x0100, 0x02,
	0x602A, 0x2AA4, 0x02,
	0x6F12, 0x00A7, 0x02,
	0xFCFC, 0x4000, 0x02,
	0x6000, 0x0005, 0x02,
	0xF466, 0x07FB, 0x02,
	0xF468, 0x2000, 0x02,
	0x0110, 0x1002, 0x02,
	0x0114, 0x0300, 0x02,
	0x0344, 0x0008, 0x02,
	0x0346, 0x000C, 0x02,
	0x0348, 0x1687, 0x02,
	0x034A, 0x10E3, 0x02,
	0x034C, 0x0B40, 0x02,
	0x034E, 0x086C, 0x02,
	0x0350, 0x0000, 0x02,
	0x0352, 0x0000, 0x02,
	0x0380, 0x0002, 0x02,
	0x0382, 0x0002, 0x02,
	0x0384, 0x0002, 0x02,
	0x0386, 0x0002, 0x02,
	0x0900, 0x0122, 0x02,
	0x0404, 0x1000, 0x02,
	0x0402, 0x1010, 0x02,
	0x0400, 0x1010, 0x02,
	0x0136, 0x1A00, 0x02,
	0x0304, 0x0004, 0x02,
	0x030C, 0x0001, 0x02,
	0x0306, 0x00DC, 0x02,
	0x0302, 0x0001, 0x02,
	0x0300, 0x0003, 0x02,
	0x030E, 0x0004, 0x02,
	0x0312, 0x0000, 0x02,
	0x0310, 0x0170, 0x02,
	0x030A, 0x0001, 0x02,
	0x0308, 0x0008, 0x02,
	0x0342, 0x0D40, 0x02,
	0x0340, 0x0926, 0x02,
	0x021E, 0x0000, 0x02,
	0x0D00, 0x0000, 0x02,
	0x0D02, 0x0001, 0x02,
	0x0114, 0x0300, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x2DF6, 0x02,
	0x6F12, 0x0100, 0x02,
	0x6028, 0x4000, 0x02,
	0x602A, 0x0B06, 0x02,
	0x6F12, 0x0101, 0x02,
	0x602A, 0x0B08, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x0FEA, 0x02,
	0x6F12, 0x2240, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x2E12, 0x02,
	0x6F12, 0x0100, 0x02,
	0x602A, 0x2E16, 0x02,
	0x6F12, 0x0101, 0x02,
	0x6028, 0x4000, 0x02,
	0x602A, 0x0B00, 0x02,
	0x6F12, 0x004D, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x3280, 0x02,
	0x6F12, 0x0100, 0x02,
};

/* 5. 2x5-26Mhz B2.FULL 5760x4312 2392Mbps 30fps */
const u32 sensor_2x5_setfile_A_5760x4312_30fps[] = {
	0x6028, 0x2000, 0x02,
	0x602A, 0x11B0, 0x02,
	0x6F12, 0x0100, 0x02,
	0x602A, 0x11B2, 0x02,
	0x6F12, 0x007F, 0x02,
	0x602A, 0x11B6, 0x02,
	0x6F12, 0x0080, 0x02,
	0x602A, 0x11BA, 0x02,
	0x6F12, 0x004B, 0x02,
	0x6F12, 0x066B, 0x02,
	0x6F12, 0xF466, 0x02,
	0x6F12, 0xA000, 0x02,
	0x6F12, 0x4000, 0x02,
	0x6F12, 0xF468, 0x02,
	0xFCFC, 0x4000, 0x02,
	0x6000, 0x0005, 0x02,
	0xF466, 0x066B, 0x02,
	0xF468, 0x4000, 0x02,
	0x0110, 0x1002, 0x02,
	0x0114, 0x0300, 0x02,
	0x0344, 0x0004, 0x02,
	0x0346, 0x0008, 0x02,
	0x0348, 0x168B, 0x02,
	0x034A, 0x10E7, 0x02,
	0x034C, 0x1680, 0x02,
	0x034E, 0x10D8, 0x02,
	0x0350, 0x0004, 0x02,
	0x0352, 0x0004, 0x02,
	0x0380, 0x0001, 0x02,
	0x0382, 0x0001, 0x02,
	0x0384, 0x0001, 0x02,
	0x0386, 0x0001, 0x02,
	0x0900, 0x0011, 0x02,
	0x0404, 0x1000, 0x02,
	0x0402, 0x1010, 0x02,
	0x0400, 0x1010, 0x02,
	0x0136, 0x1A00, 0x02,
	0x0304, 0x0004, 0x02,
	0x030C, 0x0001, 0x02,
	0x0306, 0x00DC, 0x02,
	0x0302, 0x0001, 0x02,
	0x0300, 0x0003, 0x02,
	0x030E, 0x0004, 0x02,
	0x0312, 0x0000, 0x02,
	0x0310, 0x0170, 0x02,
	0x030A, 0x0001, 0x02,
	0x0308, 0x0008, 0x02,
	0x0342, 0x1B90, 0x02,
	0x0340, 0x1196, 0x02,
	0x021E, 0x0000, 0x02,
	0x0D00, 0x0000, 0x02,
	0x0D02, 0x0001, 0x02,
	0x0114, 0x0300, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x2DF6, 0x02,
	0x6F12, 0x0100, 0x02,
	0x6028, 0x4000, 0x02,
	0x602A, 0x0B06, 0x02,
	0x6F12, 0x0101, 0x02,
	0x602A, 0x0B08, 0x02,
	0x6F12, 0x0001, 0x02,
	0x602A, 0x0FEA, 0x02,
	0x6F12, 0x2240, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x2E12, 0x02,
	0x6F12, 0x0101, 0x02,
	0x602A, 0x2E16, 0x02,
	0x6F12, 0x0001, 0x02,
	0x6028, 0x4000, 0x02,
	0x602A, 0x0B00, 0x02,
	0x6F12, 0x004D, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x3280, 0x02,
	0x6F12, 0x0000, 0x02,
};

/* 6. 2x5-26Mhz B3.FULL 5760x3240 2392MHz 30fps */
const u32 sensor_2x5_setfile_A_5760x3240_30fps[] = {
	0x6028, 0x2000, 0x02,
	0x602A, 0x11B0, 0x02,
	0x6F12, 0x0100, 0x02,
	0x602A, 0x11B2, 0x02,
	0x6F12, 0x007F, 0x02,
	0x602A, 0x11B6, 0x02,
	0x6F12, 0x0080, 0x02,
	0x602A, 0x11BA, 0x02,
	0x6F12, 0x004B, 0x02,
	0x6F12, 0x066B, 0x02,
	0x6F12, 0xF466, 0x02,
	0x6F12, 0xA000, 0x02,
	0x6F12, 0x4000, 0x02,
	0x6F12, 0xF468, 0x02,
	0xFCFC, 0x4000, 0x02,
	0x6000, 0x0005, 0x02,
	0xF466, 0x066B, 0x02,
	0xF468, 0x4000, 0x02,
	0x0110, 0x1002, 0x02,
	0x0114, 0x0300, 0x02,
	0x0344, 0x0004, 0x02,
	0x0346, 0x0220, 0x02,
	0x0348, 0x168B, 0x02,
	0x034A, 0x0ECF, 0x02,
	0x034C, 0x1680, 0x02,
	0x034E, 0x0CA8, 0x02,
	0x0350, 0x0004, 0x02,
	0x0352, 0x0004, 0x02,
	0x0380, 0x0001, 0x02,
	0x0382, 0x0001, 0x02,
	0x0384, 0x0001, 0x02,
	0x0386, 0x0001, 0x02,
	0x0900, 0x0011, 0x02,
	0x0404, 0x1000, 0x02,
	0x0402, 0x1010, 0x02,
	0x0400, 0x1010, 0x02,
	0x0136, 0x1A00, 0x02,
	0x0304, 0x0004, 0x02,
	0x030C, 0x0001, 0x02,
	0x0306, 0x00DC, 0x02,
	0x0302, 0x0001, 0x02,
	0x0300, 0x0003, 0x02,
	0x030E, 0x0004, 0x02,
	0x0312, 0x0000, 0x02,
	0x0310, 0x0170, 0x02,
	0x030A, 0x0001, 0x02,
	0x0308, 0x0008, 0x02,
	0x0342, 0x1B90, 0x02,
	0x0340, 0x1196, 0x02,
	0x021E, 0x0000, 0x02,
	0x0D00, 0x0000, 0x02,
	0x0D02, 0x0001, 0x02,
	0x0114, 0x0300, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x2DF6, 0x02,
	0x6F12, 0x0100, 0x02,
	0x6028, 0x4000, 0x02,
	0x602A, 0x0B06, 0x02,
	0x6F12, 0x0101, 0x02,
	0x602A, 0x0B08, 0x02,
	0x6F12, 0x0001, 0x02,
	0x602A, 0x0FEA, 0x02,
	0x6F12, 0x2240, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x2E12, 0x02,
	0x6F12, 0x0101, 0x02,
	0x602A, 0x2E16, 0x02,
	0x6F12, 0x0001, 0x02,
	0x6028, 0x4000, 0x02,
	0x602A, 0x0B00, 0x02,
	0x6F12, 0x004D, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x3280, 0x02,
	0x6F12, 0x0000, 0x02,
};

/* 7. 2x5-26Mhz B19.FULL 5760x2664 2392MHz 30fps */
const u32 sensor_2x5_setfile_A_5760x2664_30fps[] = {
	0x6028, 0x2000, 0x02,
	0x602A, 0x11B0, 0x02,
	0x6F12, 0x0100, 0x02,
	0x602A, 0x11B2, 0x02,
	0x6F12, 0x007F, 0x02,
	0x602A, 0x11B6, 0x02,
	0x6F12, 0x0080, 0x02,
	0x602A, 0x11BA, 0x02,
	0x6F12, 0x004B, 0x02,
	0x6F12, 0x066B, 0x02,
	0x6F12, 0xF466, 0x02,
	0x6F12, 0xA000, 0x02,
	0x6F12, 0x4000, 0x02,
	0x6F12, 0xF468, 0x02,
	0xFCFC, 0x4000, 0x02,
	0x6000, 0x0005, 0x02,
	0xF466, 0x066B, 0x02,
	0xF468, 0x4000, 0x02,
	0x0110, 0x1002, 0x02,
	0x0114, 0x0300, 0x02,
	0x0344, 0x0004, 0x02,
	0x0346, 0x0344, 0x02,
	0x0348, 0x168B, 0x02,
	0x034A, 0x0DAB, 0x02,
	0x034C, 0x1680, 0x02,
	0x034E, 0x0A68, 0x02,
	0x0350, 0x0004, 0x02,
	0x0352, 0x0000, 0x02,
	0x0380, 0x0001, 0x02,
	0x0382, 0x0001, 0x02,
	0x0384, 0x0001, 0x02,
	0x0386, 0x0001, 0x02,
	0x0900, 0x0011, 0x02,
	0x0404, 0x1000, 0x02,
	0x0402, 0x1010, 0x02,
	0x0400, 0x1010, 0x02,
	0x0136, 0x1A00, 0x02,
	0x0304, 0x0004, 0x02,
	0x030C, 0x0001, 0x02,
	0x0306, 0x00DC, 0x02,
	0x0302, 0x0001, 0x02,
	0x0300, 0x0003, 0x02,
	0x030E, 0x0004, 0x02,
	0x0312, 0x0000, 0x02,
	0x0310, 0x0170, 0x02,
	0x030A, 0x0001, 0x02,
	0x0308, 0x0008, 0x02,
	0x0342, 0x1B90, 0x02,
	0x0340, 0x1196, 0x02,
	0x021E, 0x0000, 0x02,
	0x0D00, 0x0000, 0x02,
	0x0D02, 0x0001, 0x02,
	0x0114, 0x0300, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x2DF6, 0x02,
	0x6F12, 0x0100, 0x02,
	0x6028, 0x4000, 0x02,
	0x602A, 0x0B06, 0x02,
	0x6F12, 0x0101, 0x02,
	0x602A, 0x0B08, 0x02,
	0x6F12, 0x0001, 0x02,
	0x602A, 0x0FEA, 0x02,
	0x6F12, 0x2240, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x2E12, 0x02,
	0x6F12, 0x0101, 0x02,
	0x602A, 0x2E16, 0x02,
	0x6F12, 0x0001, 0x02,
	0x6028, 0x4000, 0x02,
	0x602A, 0x0B00, 0x02,
	0x6F12, 0x004D, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x3280, 0x02,
	0x6F12, 0x0000, 0x02,
};

/* 8. 2x5-26Mhz B5.FULL 4312x4312 2392MHz 30fps */
const u32 sensor_2x5_setfile_A_4312x4312_30fps[] = {
	0x6028, 0x2000, 0x02,
	0x602A, 0x11B0, 0x02,
	0x6F12, 0x0100, 0x02,
	0x602A, 0x11B2, 0x02,
	0x6F12, 0x007F, 0x02,
	0x602A, 0x11B6, 0x02,
	0x6F12, 0x0080, 0x02,
	0x602A, 0x11BA, 0x02,
	0x6F12, 0x004B, 0x02,
	0x6F12, 0x066B, 0x02,
	0x6F12, 0xF466, 0x02,
	0x6F12, 0xA000, 0x02,
	0x6F12, 0x4000, 0x02,
	0x6F12, 0xF468, 0x02,
	0xFCFC, 0x4000, 0x02,
	0x6000, 0x0005, 0x02,
	0xF466, 0x066B, 0x02,
	0xF468, 0x4000, 0x02,
	0x0110, 0x1002, 0x02,
	0x0114, 0x0300, 0x02,
	0x0344, 0x02D8, 0x02,
	0x0346, 0x0008, 0x02,
	0x0348, 0x13B7, 0x02,
	0x034A, 0x10E7, 0x02,
	0x034C, 0x10D8, 0x02,
	0x034E, 0x10D8, 0x02,
	0x0350, 0x0004, 0x02,
	0x0352, 0x0004, 0x02,
	0x0380, 0x0001, 0x02,
	0x0382, 0x0001, 0x02,
	0x0384, 0x0001, 0x02,
	0x0386, 0x0001, 0x02,
	0x0900, 0x0011, 0x02,
	0x0404, 0x1000, 0x02,
	0x0402, 0x1010, 0x02,
	0x0400, 0x1010, 0x02,
	0x0136, 0x1A00, 0x02,
	0x0304, 0x0004, 0x02,
	0x030C, 0x0001, 0x02,
	0x0306, 0x00DC, 0x02,
	0x0302, 0x0001, 0x02,
	0x0300, 0x0003, 0x02,
	0x030E, 0x0004, 0x02,
	0x0312, 0x0000, 0x02,
	0x0310, 0x0170, 0x02,
	0x030A, 0x0001, 0x02,
	0x0308, 0x0008, 0x02,
	0x0342, 0x1B90, 0x02,
	0x0340, 0x1196, 0x02,
	0x021E, 0x0000, 0x02,
	0x0D00, 0x0000, 0x02,
	0x0D02, 0x0001, 0x02,
	0x0114, 0x0300, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x2DF6, 0x02,
	0x6F12, 0x0100, 0x02,
	0x6028, 0x4000, 0x02,
	0x602A, 0x0B06, 0x02,
	0x6F12, 0x0101, 0x02,
	0x602A, 0x0B08, 0x02,
	0x6F12, 0x0001, 0x02,
	0x602A, 0x0FEA, 0x02,
	0x6F12, 0x2240, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x2E12, 0x02,
	0x6F12, 0x0101, 0x02,
	0x602A, 0x2E16, 0x02,
	0x6F12, 0x0001, 0x02,
	0x6028, 0x4000, 0x02,
	0x602A, 0x0B00, 0x02,
	0x6F12, 0x004D, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x3280, 0x02,
	0x6F12, 0x0000, 0x02,
};

/* 2X5 Pre Calibration */
const u32 sensor_2x5_setfile_A_pre_cal[] = {
	0x6028, 0x4000, 0x02,
	0x0B00, 0x014D, 0x02,
	0x6028, 0x2000, 0x02,
	0x2B0C, 0x18C0, 0x02,
	0x2B0E, 0x1344, 0x02,
	0x2B6C, 0x0120, 0x02,
	0x2B6E, 0x0132, 0x02,
	0x2B60, 0x0003, 0x02,
	0x2B62, 0x0004, 0x02,
	0x2B5C, 0x0040, 0x02,
	0x2B5E, 0x0038, 0x02,
	0x6028, 0x2001, 0x02,
	0x24F0, 0x1388, 0x02,
	0x24B4, 0x0100, 0x02,
	0x24B6, 0x01D0, 0x02,
	0x24E4, 0x0410, 0x02,
	0x6028, 0x4000, 0x02,
	0x6226, 0x0001, 0x02,
	0x6028, 0x2001, 0x02,
	0x24E6, 0x123D, 0x02,
};

/* 2X5 After Calibration */
const u32 sensor_2x5_setfile_A_post_cal[] = {
	0x6028, 0x4000, 0x02,
	0x0100, 0x0000, 0x02,
	0x0FE8, 0x0000, 0x02,
	0x6028, 0x2004, 0x02,
	0x336C, 0x1234, 0x02,
	0x338C, 0x1234, 0x02,
	0x6028, 0x2001, 0x02,
	0x24B2, 0x0001, 0x02,
	0x6028, 0x2000, 0x02,
	0x20F0, 0x0101, 0x02,
};

/* 2X5 Tetra ISP 6MP */
const u32 sensor_2x5_setfile_A_tetra_isp_6MP[] = {
	0x6028, 0x2000, 0x02,
	0x602A, 0x3662, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x375E, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x3796, 0x02,
	0x6F12, 0x00AD, 0x02,
	0x602A, 0x379A, 0x02,
	0x6F12, 0x00AD, 0x02,
	0x602A, 0x37AA, 0x02,
	0x6F12, 0x000D, 0x02,
	0x602A, 0x37AE, 0x02,
	0x6F12, 0x000D, 0x02,
	0x602A, 0x37B2, 0x02,
	0x6F12, 0x000D, 0x02,
	0x602A, 0x37B6, 0x02,
	0x6F12, 0x0010, 0x02,
	0x602A, 0x37BA, 0x02,
	0x6F12, 0x0010, 0x02,
	0x602A, 0x37BE, 0x02,
	0x6F12, 0x0010, 0x02,
	0x602A, 0x37C2, 0x02,
	0x6F12, 0x000D, 0x02,
	0x602A, 0x37C6, 0x02,
	0x6F12, 0x000D, 0x02,
	0x602A, 0x37CA, 0x02,
	0x6F12, 0x000D, 0x02,
	0x602A, 0x37CE, 0x02,
	0x6F12, 0x0010, 0x02,
	0x602A, 0x37D2, 0x02,
	0x6F12, 0x0010, 0x02,
	0x602A, 0x37D6, 0x02,
	0x6F12, 0x0010, 0x02,
	0x602A, 0x37DA, 0x02,
	0x6F12, 0x000D, 0x02,
	0x602A, 0x37DE, 0x02,
	0x6F12, 0x000D, 0x02,
	0x602A, 0x37E2, 0x02,
	0x6F12, 0x000D, 0x02,
	0x602A, 0x37E6, 0x02,
	0x6F12, 0x0010, 0x02,
	0x602A, 0x37EA, 0x02,
	0x6F12, 0x0010, 0x02,
	0x602A, 0x37EE, 0x02,
	0x6F12, 0x0010, 0x02,
	0x602A, 0x37F2, 0x02,
	0x6F12, 0x000D, 0x02,
	0x602A, 0x37F6, 0x02,
	0x6F12, 0x000D, 0x02,
	0x602A, 0x37FA, 0x02,
	0x6F12, 0x000D, 0x02,
	0x602A, 0x37FE, 0x02,
	0x6F12, 0x0010, 0x02,
	0x602A, 0x3802, 0x02,
	0x6F12, 0x0010, 0x02,
	0x602A, 0x3806, 0x02,
	0x6F12, 0x0010, 0x02,
	0x602A, 0x3816, 0x02,
	0x6F12, 0x000D, 0x02,
	0x602A, 0x381A, 0x02,
	0x6F12, 0x000D, 0x02,
	0x602A, 0x381E, 0x02,
	0x6F12, 0x000D, 0x02,
	0x602A, 0x3822, 0x02,
	0x6F12, 0x0010, 0x02,
	0x602A, 0x3826, 0x02,
	0x6F12, 0x0010, 0x02,
	0x602A, 0x382A, 0x02,
	0x6F12, 0x0010, 0x02,
	0x602A, 0x382E, 0x02,
	0x6F12, 0x000D, 0x02,
	0x602A, 0x3832, 0x02,
	0x6F12, 0x000D, 0x02,
	0x602A, 0x3836, 0x02,
	0x6F12, 0x000D, 0x02,
	0x602A, 0x383A, 0x02,
	0x6F12, 0x0010, 0x02,
	0x602A, 0x383E, 0x02,
	0x6F12, 0x0010, 0x02,
	0x602A, 0x3842, 0x02,
	0x6F12, 0x0010, 0x02,
	0x602A, 0x3846, 0x02,
	0x6F12, 0x000D, 0x02,
	0x602A, 0x384A, 0x02,
	0x6F12, 0x000D, 0x02,
	0x602A, 0x384E, 0x02,
	0x6F12, 0x000D, 0x02,
	0x602A, 0x3852, 0x02,
	0x6F12, 0x0010, 0x02,
	0x602A, 0x3856, 0x02,
	0x6F12, 0x0010, 0x02,
	0x602A, 0x385A, 0x02,
	0x6F12, 0x0010, 0x02,
	0x602A, 0x385E, 0x02,
	0x6F12, 0x000D, 0x02,
	0x602A, 0x3862, 0x02,
	0x6F12, 0x000D, 0x02,
	0x602A, 0x3866, 0x02,
	0x6F12, 0x000D, 0x02,
	0x602A, 0x386A, 0x02,
	0x6F12, 0x0010, 0x02,
	0x602A, 0x386E, 0x02,
	0x6F12, 0x0010, 0x02,
	0x602A, 0x3872, 0x02,
	0x6F12, 0x0010, 0x02,
	0x602A, 0x366E, 0x02,
	0x6F12, 0x0001, 0x02,
	0x602A, 0x3672, 0x02,
	0x6F12, 0x0001, 0x02,
	0x602A, 0x3676, 0x02,
	0x6F12, 0x0001, 0x02,
	0x602A, 0x372E, 0x02,
	0x6F12, 0x0080, 0x02,
	0x602A, 0x3732, 0x02,
	0x6F12, 0x0080, 0x02,
	0x602A, 0x3736, 0x02,
	0x6F12, 0x0080, 0x02,
	0x602A, 0x373A, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x3742, 0x02,
	0x6F12, 0x000C, 0x02,
	0x602A, 0x3746, 0x02,
	0x6F12, 0x000C, 0x02,
	0x602A, 0x374A, 0x02,
	0x6F12, 0x000C, 0x02,
	0x602A, 0x3752, 0x02,
	0x6F12, 0x0064, 0x02,
	0x602A, 0x3756, 0x02,
	0x6F12, 0x0064, 0x02,
	0x602A, 0x375A, 0x02,
	0x6F12, 0x0064, 0x02,
	0x602A, 0x3776, 0x02,
	0x6F12, 0x0001, 0x02,
	0x602A, 0x3786, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x418A, 0x02,
	0x6F12, 0x8080, 0x02,
	0x6F12, 0x8080, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x4192, 0x02,
	0x6F12, 0x1080, 0x02,
	0x6F12, 0x8080, 0x02,
	0x6F12, 0x8080, 0x02,
	0x6F12, 0x8000, 0x02,
	0x602A, 0x419C, 0x02,
	0x6F12, 0x8080, 0x02,
	0x6F12, 0x8080, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x41A4, 0x02,
	0x6F12, 0x102D, 0x02,
	0x6F12, 0x2D2D, 0x02,
	0x6F12, 0x2D2D, 0x02,
	0x6F12, 0x2D00, 0x02,
	0x602A, 0x41AE, 0x02,
	0x6F12, 0x2D2D, 0x02,
	0x6F12, 0x2D2D, 0x02,
	0x6F12, 0x2D2D, 0x02,
	0x602A, 0x41B6, 0x02,
	0x6F12, 0x102D, 0x02,
	0x6F12, 0x2D2D, 0x02,
	0x6F12, 0x2D2D, 0x02,
	0x6F12, 0x2D00, 0x02,
	0x602A, 0x41C0, 0x02,
	0x6F12, 0x0505, 0x02,
	0x6F12, 0x0505, 0x02,
	0x6F12, 0x0505, 0x02,
	0x602A, 0x41C8, 0x02,
	0x6F12, 0x1005, 0x02,
	0x6F12, 0x0505, 0x02,
	0x6F12, 0x0505, 0x02,
	0x6F12, 0x0500, 0x02,
	0x602A, 0x41D2, 0x02,
	0x6F12, 0x0505, 0x02,
	0x6F12, 0x0505, 0x02,
	0x6F12, 0x0505, 0x02,
	0x602A, 0x41DA, 0x02,
	0x6F12, 0x1001, 0x02,
	0x6F12, 0x0101, 0x02,
	0x6F12, 0x0101, 0x02,
	0x6F12, 0x0100, 0x02,
	0x602A, 0x41E4, 0x02,
	0x6F12, 0x0101, 0x02,
	0x6F12, 0x0101, 0x02,
	0x6F12, 0x0101, 0x02,
	0x602A, 0x41EC, 0x02,
	0x6F12, 0x1001, 0x02,
	0x6F12, 0x0101, 0x02,
	0x6F12, 0x0101, 0x02,
	0x6F12, 0x0100, 0x02,
	0x602A, 0x421A, 0x02,
	0x6F12, 0xFFFF, 0x02,
	0x6F12, 0xFFFF, 0x02,
	0x6F12, 0xFFFF, 0x02,
	0x602A, 0x4222, 0x02,
	0x6F12, 0x10FF, 0x02,
	0x6F12, 0xFFFF, 0x02,
	0x6F12, 0xFFFF, 0x02,
	0x6F12, 0xFF00, 0x02,
	0x602A, 0x422C, 0x02,
	0x6F12, 0xFFFF, 0x02,
	0x6F12, 0xFFFF, 0x02,
	0x6F12, 0xFFFF, 0x02,
	0x602A, 0x10B6, 0x02,
	0x6F12, 0x0020, 0x02,
	0x602A, 0x1C38, 0x02,
	0x6F12, 0x0096, 0x02,
	0x602A, 0x1C1E, 0x02,
	0x6F12, 0x0E0D, 0x02,
	0x602A, 0x1322, 0x02,
	0x6F12, 0x0303, 0x02,
	0x602A, 0x117C, 0x02,
	0x6F12, 0x0200, 0x02,
	0x602A, 0x11A4, 0x02,
	0x6F12, 0x28C0, 0x02,
	0x602A, 0x1118, 0x02,
	0x6F12, 0x0040, 0x02,
	0x6F12, 0x2140, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x2DFA, 0x02,
	0x6F12, 0x0002, 0x02,
	0x602A, 0x10E0, 0x02,
	0x6F12, 0x2ED7, 0x02,
	0x6F12, 0x2DB4, 0x02,
	0x602A, 0x25A0, 0x02,
	0x6F12, 0x0200, 0x02,
	0x6028, 0x2001, 0x02,
	0x602A, 0x2480, 0x02,
	0x6F12, 0x02B6, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x2676, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x2678, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x267A, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x267C, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x267E, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x2680, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x2682, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x2684, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x2686, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x2688, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x268A, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x268C, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x268E, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x2690, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x2692, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x2694, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x2696, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x2698, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x269A, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x269C, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x269E, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x26A0, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x26A2, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x26A4, 0x02,
	0x6F12, 0x8080, 0x02,
	0x6028, 0x2001, 0x02,
	0x602A, 0x250C, 0x02,
	0x6F12, 0x0031, 0x02,
	0x6028, 0x4000, 0x02,
	0x602A, 0x0902, 0x02,
	0x6F12, 0x0001, 0x02,
	0x6028, 0x2001, 0x02,
	0x602A, 0x2528, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x602A, 0x24E8, 0x02,
	0x6F12, 0x01E0, 0x02,
	0x6F12, 0x0020, 0x02,
	0x6F12, 0x002C, 0x02,
	0x6F12, 0x0068, 0x02,
	0x602A, 0x2500, 0x02,
	0x6F12, 0x0300, 0x02,
	0x602A, 0x2502, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x2504, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x2506, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x2508, 0x02,
	0x6F12, 0x0002, 0x02,
	0x602A, 0x24F0, 0x02,
	0x6F12, 0x1388, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x2482, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x24B8, 0x02,
	0x6F12, 0x0404, 0x02,
	0x602A, 0x24BA, 0x02,
	0x6F12, 0x0404, 0x02,
	0x602A, 0x24BC, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x24BE, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x24C0, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x24C2, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x24C4, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x24C6, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x24C8, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x24CA, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x24CC, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x24CE, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x24D0, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x24D2, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x24D4, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x24D6, 0x02,
	0x6F12, 0x0001, 0x02,
	0x602A, 0x24D8, 0x02,
	0x6F12, 0x0406, 0x02,
	0x602A, 0x24DA, 0x02,
	0x6F12, 0x000A, 0x02,
	0x602A, 0x24DC, 0x02,
	0x6F12, 0x0C0E, 0x02,
	0x602A, 0x24DE, 0x02,
	0x6F12, 0x0F10, 0x02,
	0x602A, 0x24E0, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x24E2, 0x02,
	0x6F12, 0x0100, 0x02,
	0x602A, 0x24E4, 0x02,
	0x6F12, 0x0410, 0x02,
	0x602A, 0x248A, 0x02,
	0x6F12, 0x0505, 0x02,
	0x602A, 0x248C, 0x02,
	0x6F12, 0x0402, 0x02,
	0x602A, 0x248E, 0x02,
	0x6F12, 0x0008, 0x02,
	0x602A, 0x2490, 0x02,
	0x6F12, 0x0707, 0x02,
	0x602A, 0x2492, 0x02,
	0x6F12, 0x0606, 0x02,
	0x602A, 0x2494, 0x02,
	0x6F12, 0x0504, 0x02,
	0x602A, 0x2496, 0x02,
	0x6F12, 0x0301, 0x02,
	0x602A, 0x2498, 0x02,
	0x6F12, 0x0406, 0x02,
	0x602A, 0x249A, 0x02,
	0x6F12, 0x080A, 0x02,
	0x602A, 0x249C, 0x02,
	0x6F12, 0x0C0E, 0x02,
	0x602A, 0x249E, 0x02,
	0x6F12, 0x0F10, 0x02,
	0x602A, 0x24A0, 0x02,
	0x6F12, 0x0008, 0x02,
	0x602A, 0x24A2, 0x02,
	0x6F12, 0x0707, 0x02,
	0x602A, 0x24A4, 0x02,
	0x6F12, 0x0606, 0x02,
	0x602A, 0x24A6, 0x02,
	0x6F12, 0x0504, 0x02,
	0x602A, 0x24A8, 0x02,
	0x6F12, 0x0301, 0x02,
	0x602A, 0x24AA, 0x02,
	0x6F12, 0x0406, 0x02,
	0x602A, 0x24AC, 0x02,
	0x6F12, 0x080A, 0x02,
	0x602A, 0x24AE, 0x02,
	0x6F12, 0x0C0E, 0x02,
	0x602A, 0x24B0, 0x02,
	0x6F12, 0x0F10, 0x02,
	0x602A, 0x24B2, 0x02,
	0x6F12, 0x0001, 0x02,
	0x602A, 0x24B4, 0x02,
	0x6F12, 0x0100, 0x02,
	0x602A, 0x24B6, 0x02,
	0x6F12, 0x01D0, 0x02,
	0x602A, 0x24E6, 0x02,
	0x6F12, 0x123D, 0x02,
	0x602A, 0x250A, 0x02,
	0x6F12, 0x8001, 0x02,
	0x602A, 0x24FE, 0x02,
	0x6F12, 0x0800, 0x02,
	0x6028, 0x4000, 0x02,
	0x602A, 0x0D10, 0x02,
	0x6F12, 0x1388, 0x02,
	0x6028, 0x2001, 0x02,
	0x602A, 0x2516, 0x02,
	0x6F12, 0x0311, 0x02,
	0x602A, 0x2510, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x2512, 0x02,
	0x6F12, 0x8081, 0x02,
	0x602A, 0x2514, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x475A, 0x02,
	0x6F12, 0x0200, 0x02,
	0x6028, 0x2001, 0x02,
	0x602A, 0x2A88, 0x02,
	0x6F12, 0x0100, 0x02,
	0x602A, 0x2526, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x29D8, 0x02,
	0x6F12, 0x01CD, 0x02,
	0x602A, 0x2520, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x203C, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6028, 0x2001, 0x02,
	0x602A, 0x2A36, 0x02,
	0x6F12, 0x0100, 0x02,
	0x602A, 0x29E4, 0x02,
	0x6F12, 0x0002, 0x02,
	0x602A, 0x29DA, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0001, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0001, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x2524, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x29E6, 0x02,
	0x6F12, 0x0050, 0x02,
	0x6F12, 0x003C, 0x02,
	0x6F12, 0x0028, 0x02,
	0x6F12, 0x0014, 0x02,
	0x6F12, 0x0064, 0x02,
	0x6F12, 0x0050, 0x02,
	0x6F12, 0x003C, 0x02,
	0x6F12, 0x0028, 0x02,
	0x6F12, 0x0064, 0x02,
	0x6F12, 0x0050, 0x02,
	0x6F12, 0x003C, 0x02,
	0x6F12, 0x003C, 0x02,
	0x6F12, 0x00C8, 0x02,
	0x6F12, 0x0096, 0x02,
	0x6F12, 0x0064, 0x02,
	0x6F12, 0x0050, 0x02,
	0x6F12, 0x00C8, 0x02,
	0x6F12, 0x0096, 0x02,
	0x6F12, 0x0064, 0x02,
	0x6F12, 0x0050, 0x02,
	0x6F12, 0x0032, 0x02,
	0x6F12, 0x0025, 0x02,
	0x6F12, 0x0019, 0x02,
	0x6F12, 0x000C, 0x02,
	0x6F12, 0x0050, 0x02,
	0x6F12, 0x003C, 0x02,
	0x6F12, 0x0028, 0x02,
	0x6F12, 0x0014, 0x02,
	0x6F12, 0x0050, 0x02,
	0x6F12, 0x003C, 0x02,
	0x6F12, 0x0028, 0x02,
	0x6F12, 0x0014, 0x02,
	0x6F12, 0x00C8, 0x02,
	0x6F12, 0x0096, 0x02,
	0x6F12, 0x0064, 0x02,
	0x6F12, 0x0032, 0x02,
	0x6F12, 0x00C8, 0x02,
	0x6F12, 0x0096, 0x02,
	0x6F12, 0x0064, 0x02,
	0x6F12, 0x0032, 0x02,
	0x602A, 0x2A96, 0x02,
	0x6F12, 0x0096, 0x02,
	0x602A, 0x2A94, 0x02,
	0x6F12, 0x7FFF, 0x02,
	0x602A, 0x2A98, 0x02,
	0x6F12, 0x7FFF, 0x02,
	0x6F12, 0x005C, 0x02,
	0x6F12, 0x7FFF, 0x02,
	0x6F12, 0x0028, 0x02,
	0x6F12, 0x7FFF, 0x02,
	0x6F12, 0x001E, 0x02,
	0x602A, 0x2AA6, 0x02,
	0x6F12, 0x0620, 0x02,
	0x6F12, 0x0600, 0x02,
	0x602A, 0x2A48, 0x02,
	0x6F12, 0x006D, 0x02,
	0x6F12, 0x01B0, 0x02,
	0x6F12, 0x0258, 0x02,
	0x6F12, 0x033A, 0x02,
	0x6F12, 0x0463, 0x02,
	0x6F12, 0x05D9, 0x02,
	0x6F12, 0x078E, 0x02,
	0x6F12, 0x0940, 0x02,
	0x6F12, 0x09B3, 0x02,
	0x6F12, 0x09DB, 0x02,
	0x6F12, 0x09FB, 0x02,
	0x6F12, 0x0A13, 0x02,
	0x6F12, 0x0A25, 0x02,
	0x6F12, 0x0A55, 0x02,
	0x6F12, 0x0A8E, 0x02,
	0x6F12, 0x0AC8, 0x02,
	0x6F12, 0x0B04, 0x02,
	0x6F12, 0x0B41, 0x02,
	0x6F12, 0x0B7F, 0x02,
	0x6F12, 0x0BC0, 0x02,
	0x6F12, 0x0C03, 0x02,
	0x6F12, 0x0C48, 0x02,
	0x6F12, 0x0C90, 0x02,
	0x6F12, 0x0CDB, 0x02,
	0x6F12, 0x0D2A, 0x02,
	0x6F12, 0x0D7C, 0x02,
	0x6F12, 0x0DD5, 0x02,
	0x6F12, 0x0E33, 0x02,
	0x6F12, 0x0E9B, 0x02,
	0x6F12, 0x0F0E, 0x02,
	0x6F12, 0x0F91, 0x02,
	0x6F12, 0x0FFE, 0x02,
	0x6028, 0x4000, 0x02,
	0x602A, 0x0B09, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x11B0, 0x02,
	0x6F12, 0x0001, 0x02,
	0x602A, 0x11B4, 0x02,
	0x6F12, 0x0200, 0x02,
	0x602A, 0x11EE, 0x02,
	0x6F12, 0xE402, 0x02,
	0x602A, 0x11EA, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x11F4, 0x02,
	0x6F12, 0xE702, 0x02,
	0x602A, 0x11F0, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x119E, 0x02,
	0x6F12, 0x0103, 0x02,
	0x6F12, 0x0103, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x2B0E, 0x02,
	0x6F12, 0x1344, 0x02,
	0x602A, 0x2B0C, 0x02,
	0x6F12, 0x18C0, 0x02,
	0x602A, 0x2B6C, 0x02,
	0x6F12, 0x0120, 0x02,
	0x6F12, 0x0132, 0x02,
	0x602A, 0x2B60, 0x02,
	0x6F12, 0x0003, 0x02,
	0x6F12, 0x0004, 0x02,
	0x602A, 0x2B5C, 0x02,
	0x6F12, 0x0040, 0x02,
	0x6F12, 0x0038, 0x02,
	0x602A, 0x20F0, 0x02,
	0x6F12, 0x0101, 0x02,
	0x6028, 0x4000, 0x02,
	0x602A, 0x0FE8, 0x02,
	0x6F12, 0x28C0, 0x02,
	0x6028, 0x2001, 0x02,
	0x602A, 0x2A36, 0x02,
	0x6F12, 0x0100, 0x02,
	0x602A, 0x2A38, 0x02,
	0x6F12, 0x0100, 0x02,
	0x6F12, 0x0200, 0x02,
	0x6F12, 0x0400, 0x02,
	0x6F12, 0x0800, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x4030, 0x02,
	0x602A, 0x2A44, 0x02,
	0x6F12, 0x2820, 0x02,
	0x602A, 0x2A46, 0x02,
	0x6F12, 0x100D, 0x02,
	0x602A, 0x2A8A, 0x02,
	0x6F12, 0x0AAA, 0x02,
	0x602A, 0x250E, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x2A8C, 0x02,
	0x6F12, 0x0223, 0x02,
	0x6F12, 0x0112, 0x02,
	0x6F12, 0x0112, 0x02,
	0x602A, 0x2510, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x5790, 0x02,
	0x6F12, 0x0100, 0x02,
	0x6028, 0x2001, 0x02,
	0x602A, 0x2A92, 0x02,
	0x6F12, 0x1111, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x124E, 0x02,
	0x6F12, 0x0700, 0x02,
	0x6028, 0x4000, 0x02,
	0x602A, 0x080A, 0x02,
	0x6F12, 0x0100, 0x02,
	0x6000, 0x0085, 0x02,
	0x6214, 0xE9F0, 0x02,
	0x6218, 0xE9F0, 0x02,
};

/* 2X5 Tetra ISP 24MP */
const u32 sensor_2x5_setfile_A_tetra_isp_24MP[] = {
	0x6028, 0x2001, 0x02,
	0x602A, 0x2A36, 0x02,
	0x6F12, 0x0100, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x46B4, 0x02,
	0x6F12, 0x4078, 0x02,
	0x6F12, 0x785E, 0x02,
	0x6F12, 0x4236, 0x02,
	0x6F12, 0x3601, 0x02,
	0x602A, 0x46BE, 0x02,
	0x6F12, 0x7878, 0x02,
	0x6F12, 0x5E42, 0x02,
	0x6F12, 0x3636, 0x02,
	0x602A, 0x46C6, 0x02,
	0x6F12, 0x405A, 0x02,
	0x6F12, 0x5A78, 0x02,
	0x6F12, 0x96B4, 0x02,
	0x6F12, 0xB401, 0x02,
	0x602A, 0x46D0, 0x02,
	0x6F12, 0x6464, 0x02,
	0x6F12, 0x85A7, 0x02,
	0x602A, 0x46D8, 0x02,
	0x6F12, 0x4053, 0x02,
	0x6F12, 0x5370, 0x02,
	0x6F12, 0x8BA7, 0x02,
	0x6F12, 0xA701, 0x02,
	0x602A, 0x46E2, 0x02,
	0x6F12, 0x5353, 0x02,
	0x6F12, 0x708B, 0x02,
	0x6F12, 0xA7A7, 0x02,
	0x602A, 0x46EA, 0x02,
	0x6F12, 0x4064, 0x02,
	0x6F12, 0x6486, 0x02,
	0x6F12, 0xA7C8, 0x02,
	0x6F12, 0xC801, 0x02,
	0x602A, 0x46F4, 0x02,
	0x6F12, 0x1414, 0x02,
	0x6F12, 0x1B21, 0x02,
	0x6F12, 0x2828, 0x02,
	0x602A, 0x46FC, 0x02,
	0x6F12, 0x4014, 0x02,
	0x6F12, 0x141B, 0x02,
	0x6F12, 0x2128, 0x02,
	0x6F12, 0x2801, 0x02,
	0x602A, 0x4706, 0x02,
	0x6F12, 0x1B1B, 0x02,
	0x6F12, 0x232D, 0x02,
	0x6F12, 0x3636, 0x02,
	0x602A, 0x470E, 0x02,
	0x6F12, 0x403C, 0x02,
	0x6F12, 0x3C50, 0x02,
	0x6F12, 0x6478, 0x02,
	0x6F12, 0x7801, 0x02,
	0x602A, 0x4718, 0x02,
	0x6F12, 0x3C3C, 0x02,
	0x6F12, 0x5064, 0x02,
	0x6F12, 0x7878, 0x02,
	0x602A, 0x3876, 0x02,
	0x6F12, 0x0035, 0x02,
	0x602A, 0x387A, 0x02,
	0x6F12, 0x0028, 0x02,
	0x6028, 0x2001, 0x02,
	0x602A, 0x29E6, 0x02,
	0x6F12, 0x0050, 0x02,
	0x6F12, 0x003C, 0x02,
	0x6F12, 0x0028, 0x02,
	0x6F12, 0x0014, 0x02,
	0x6F12, 0x0064, 0x02,
	0x6F12, 0x0050, 0x02,
	0x6F12, 0x003C, 0x02,
	0x6F12, 0x0028, 0x02,
	0x6F12, 0x0064, 0x02,
	0x6F12, 0x0050, 0x02,
	0x6F12, 0x003C, 0x02,
	0x6F12, 0x003C, 0x02,
	0x6F12, 0x00C8, 0x02,
	0x6F12, 0x0096, 0x02,
	0x6F12, 0x0064, 0x02,
	0x6F12, 0x0050, 0x02,
	0x6F12, 0x00C8, 0x02,
	0x6F12, 0x0096, 0x02,
	0x6F12, 0x0064, 0x02,
	0x6F12, 0x0050, 0x02,
	0x6F12, 0x0032, 0x02,
	0x6F12, 0x0025, 0x02,
	0x6F12, 0x0019, 0x02,
	0x6F12, 0x000C, 0x02,
	0x6F12, 0x0050, 0x02,
	0x6F12, 0x003C, 0x02,
	0x6F12, 0x0028, 0x02,
	0x6F12, 0x0014, 0x02,
	0x6F12, 0x0050, 0x02,
	0x6F12, 0x003C, 0x02,
	0x6F12, 0x0028, 0x02,
	0x6F12, 0x0014, 0x02,
	0x6F12, 0x00C8, 0x02,
	0x6F12, 0x0096, 0x02,
	0x6F12, 0x0064, 0x02,
	0x6F12, 0x0032, 0x02,
	0x6F12, 0x00C8, 0x02,
	0x6F12, 0x0096, 0x02,
	0x6F12, 0x0064, 0x02,
	0x6F12, 0x0032, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x35D6, 0x02,
	0x6F12, 0x0038, 0x02,
	0x602A, 0x35DA, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x35E2, 0x02,
	0x6F12, 0x0006, 0x02,
	0x602A, 0x35E6, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x35F2, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x35FE, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x411E, 0x02,
	0x6F12, 0x0A0A, 0x02,
	0x6F12, 0x0A0A, 0x02,
	0x6F12, 0x0A0A, 0x02,
	0x602A, 0x4142, 0x02,
	0x6F12, 0x0505, 0x02,
	0x6F12, 0x0505, 0x02,
	0x6F12, 0x0505, 0x02,
	0x602A, 0x4166, 0x02,
	0x6F12, 0x3232, 0x02,
	0x6F12, 0x3232, 0x02,
	0x6F12, 0x3232, 0x02,
	0x602A, 0x4180, 0x02,
	0x6F12, 0x1040, 0x02,
	0x6F12, 0x4040, 0x02,
	0x6F12, 0x4040, 0x02,
	0x602A, 0x4690, 0x02,
	0x6F12, 0x1013, 0x02,
	0x6F12, 0x1313, 0x02,
	0x6F12, 0x1313, 0x02,
	0x6F12, 0x1300, 0x02,
	0x602A, 0x10B6, 0x02,
	0x6F12, 0x0020, 0x02,
	0x602A, 0x1C38, 0x02,
	0x6F12, 0x0096, 0x02,
	0x602A, 0x1C1E, 0x02,
	0x6F12, 0x0E0D, 0x02,
	0x602A, 0x1322, 0x02,
	0x6F12, 0x0303, 0x02,
	0x602A, 0x117C, 0x02,
	0x6F12, 0x0200, 0x02,
	0x602A, 0x11A4, 0x02,
	0x6F12, 0x28C0, 0x02,
	0x602A, 0x1118, 0x02,
	0x6F12, 0x0040, 0x02,
	0x6F12, 0x2140, 0x02,
	0x6028, 0x2001, 0x02,
	0x602A, 0x250C, 0x02,
	0x6F12, 0x0031, 0x02,
	0x602A, 0x2500, 0x02,
	0x6F12, 0x0300, 0x02,
	0x602A, 0x24E8, 0x02,
	0x6F12, 0x01E0, 0x02,
	0x6F12, 0x0020, 0x02,
	0x6F12, 0x002C, 0x02,
	0x6F12, 0x0068, 0x02,
	0x602A, 0x2500, 0x02,
	0x6F12, 0x0300, 0x02,
	0x602A, 0x2502, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x2504, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x2505, 0x02,
	0x602A, 0x2506, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x2508, 0x02,
	0x6F12, 0x0002, 0x02,
	0x602A, 0x24F0, 0x02,
	0x6F12, 0x1388, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x2482, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x24B8, 0x02,
	0x6F12, 0x0404, 0x02,
	0x602A, 0x24BA, 0x02,
	0x6F12, 0x0404, 0x02,
	0x602A, 0x24BC, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x24BE, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x24C0, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x24C2, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x24C4, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x24C6, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x24C8, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x24CA, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x24CC, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x24CE, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x24D0, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x24D2, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x24D4, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x24D6, 0x02,
	0x6F12, 0x0001, 0x02,
	0x602A, 0x24D8, 0x02,
	0x6F12, 0x0406, 0x02,
	0x602A, 0x24DA, 0x02,
	0x6F12, 0x000A, 0x02,
	0x602A, 0x24DC, 0x02,
	0x6F12, 0x0C0E, 0x02,
	0x602A, 0x24DE, 0x02,
	0x6F12, 0x0F10, 0x02,
	0x602A, 0x24E0, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x24E2, 0x02,
	0x6F12, 0x0100, 0x02,
	0x602A, 0x24E4, 0x02,
	0x6F12, 0x0410, 0x02,
	0x602A, 0x248A, 0x02,
	0x6F12, 0x0505, 0x02,
	0x602A, 0x248C, 0x02,
	0x6F12, 0x0402, 0x02,
	0x602A, 0x248E, 0x02,
	0x6F12, 0x0008, 0x02,
	0x602A, 0x2490, 0x02,
	0x6F12, 0x0707, 0x02,
	0x602A, 0x2492, 0x02,
	0x6F12, 0x0606, 0x02,
	0x602A, 0x2494, 0x02,
	0x6F12, 0x0504, 0x02,
	0x602A, 0x2496, 0x02,
	0x6F12, 0x0301, 0x02,
	0x602A, 0x2498, 0x02,
	0x6F12, 0x0406, 0x02,
	0x602A, 0x249A, 0x02,
	0x6F12, 0x080A, 0x02,
	0x602A, 0x249C, 0x02,
	0x6F12, 0x0C0E, 0x02,
	0x602A, 0x249E, 0x02,
	0x6F12, 0x0F10, 0x02,
	0x602A, 0x24A0, 0x02,
	0x6F12, 0x0008, 0x02,
	0x602A, 0x24A2, 0x02,
	0x6F12, 0x0707, 0x02,
	0x602A, 0x24A4, 0x02,
	0x6F12, 0x0606, 0x02,
	0x602A, 0x24A6, 0x02,
	0x6F12, 0x0504, 0x02,
	0x602A, 0x24A8, 0x02,
	0x6F12, 0x0301, 0x02,
	0x602A, 0x24AA, 0x02,
	0x6F12, 0x0406, 0x02,
	0x602A, 0x24AC, 0x02,
	0x6F12, 0x080A, 0x02,
	0x602A, 0x24AE, 0x02,
	0x6F12, 0x0C0E, 0x02,
	0x602A, 0x24B0, 0x02,
	0x6F12, 0x0F10, 0x02,
	0x602A, 0x24B2, 0x02,
	0x6F12, 0x0001, 0x02,
	0x602A, 0x24B4, 0x02,
	0x6F12, 0x0100, 0x02,
	0x602A, 0x24B6, 0x02,
	0x6F12, 0x01D0, 0x02,
	0x602A, 0x24E6, 0x02,
	0x6F12, 0x123D, 0x02,
	0x602A, 0x250A, 0x02,
	0x6F12, 0x8001, 0x02,
	0x602A, 0x24FE, 0x02,
	0x6F12, 0x0800, 0x02,
	0x6028, 0x4000, 0x02,
	0x602A, 0x0D10, 0x02,
	0x6F12, 0x1388, 0x02,
	0x6028, 0x2001, 0x02,
	0x602A, 0x2516, 0x02,
	0x6F12, 0x0311, 0x02,
	0x602A, 0x2510, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x2512, 0x02,
	0x6F12, 0x8081, 0x02,
	0x602A, 0x2514, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x475A, 0x02,
	0x6F12, 0x0200, 0x02,
	0x6028, 0x2001, 0x02,
	0x602A, 0x2A88, 0x02,
	0x6F12, 0x0100, 0x02,
	0x602A, 0x2526, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x29D8, 0x02,
	0x6F12, 0x0196, 0x02,
	0x602A, 0x2520, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x203C, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6028, 0x2001, 0x02,
	0x602A, 0x29E4, 0x02,
	0x6F12, 0x0002, 0x02,
	0x602A, 0x29DA, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0001, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0001, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x2524, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x2A96, 0x02,
	0x6F12, 0x0096, 0x02,
	0x602A, 0x2A94, 0x02,
	0x6F12, 0x7FFF, 0x02,
	0x602A, 0x2A98, 0x02,
	0x6F12, 0x7FFF, 0x02,
	0x6F12, 0x005C, 0x02,
	0x6F12, 0x7FFF, 0x02,
	0x6F12, 0x0028, 0x02,
	0x6F12, 0x7FFF, 0x02,
	0x6F12, 0x001E, 0x02,
	0x602A, 0x2AA6, 0x02,
	0x6F12, 0x0620, 0x02,
	0x6F12, 0x0600, 0x02,
	0x602A, 0x2A48, 0x02,
	0x6F12, 0x006D, 0x02,
	0x6F12, 0x01B0, 0x02,
	0x6F12, 0x0258, 0x02,
	0x6F12, 0x033A, 0x02,
	0x6F12, 0x0463, 0x02,
	0x6F12, 0x05D9, 0x02,
	0x6F12, 0x078E, 0x02,
	0x6F12, 0x0940, 0x02,
	0x6F12, 0x09B3, 0x02,
	0x6F12, 0x09DB, 0x02,
	0x6F12, 0x09FB, 0x02,
	0x6F12, 0x0A13, 0x02,
	0x6F12, 0x0A25, 0x02,
	0x6F12, 0x0A55, 0x02,
	0x6F12, 0x0A8E, 0x02,
	0x6F12, 0x0AC8, 0x02,
	0x6F12, 0x0B04, 0x02,
	0x6F12, 0x0B41, 0x02,
	0x6F12, 0x0B7F, 0x02,
	0x6F12, 0x0BC0, 0x02,
	0x6F12, 0x0C03, 0x02,
	0x6F12, 0x0C48, 0x02,
	0x6F12, 0x0C90, 0x02,
	0x6F12, 0x0CDB, 0x02,
	0x6F12, 0x0D2A, 0x02,
	0x6F12, 0x0D7C, 0x02,
	0x6F12, 0x0DD5, 0x02,
	0x6F12, 0x0E33, 0x02,
	0x6F12, 0x0E9B, 0x02,
	0x6F12, 0x0F0E, 0x02,
	0x6F12, 0x0F91, 0x02,
	0x6F12, 0x0FFE, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x25A0, 0x02,
	0x6F12, 0x0200, 0x02,
	0x6028, 0x2001, 0x02,
	0x602A, 0x2480, 0x02,
	0x6F12, 0x02B6, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x2676, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x2678, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x267A, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x267C, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x267E, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x2680, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x2682, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x2684, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x2686, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x2688, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x268A, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x268C, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x268E, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x2690, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x2692, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x2694, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x2696, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x2698, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x269A, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x269C, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x269E, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x26A0, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x26A2, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x26A4, 0x02,
	0x6F12, 0x8080, 0x02,
	0x602A, 0x119E, 0x02,
	0x6F12, 0x0130, 0x02,
	0x6028, 0x2001, 0x02,
	0x602A, 0x2528, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6028, 0x2001, 0x02,
	0x602A, 0x24B6, 0x02,
	0x6F12, 0x01D0, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x2B0E, 0x02,
	0x6F12, 0x1344, 0x02,
	0x602A, 0x2B0C, 0x02,
	0x6F12, 0x18C0, 0x02,
	0x602A, 0x2B6C, 0x02,
	0x6F12, 0x0120, 0x02,
	0x6F12, 0x0132, 0x02,
	0x602A, 0x2B60, 0x02,
	0x6F12, 0x0003, 0x02,
	0x6F12, 0x0004, 0x02,
	0x602A, 0x2B5C, 0x02,
	0x6F12, 0x0040, 0x02,
	0x6F12, 0x0038, 0x02,
	0x6028, 0x4000, 0x02,
	0x602A, 0x0FE8, 0x02,
	0x6F12, 0x28C0, 0x02,
	0x6028, 0x2001, 0x02,
	0x602A, 0x2A36, 0x02,
	0x6F12, 0x0100, 0x02,
	0x602A, 0x2A38, 0x02,
	0x6F12, 0x0100, 0x02,
	0x6F12, 0x0200, 0x02,
	0x6F12, 0x0400, 0x02,
	0x6F12, 0x0800, 0x02,
	0x6F12, 0x1000, 0x02,
	0x6F12, 0x4030, 0x02,
	0x602A, 0x2A44, 0x02,
	0x6F12, 0x2820, 0x02,
	0x602A, 0x2A46, 0x02,
	0x6F12, 0x100D, 0x02,
	0x602A, 0x2A8A, 0x02,
	0x6F12, 0x0AAA, 0x02,
	0x602A, 0x250E, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x2A8C, 0x02,
	0x6F12, 0x0223, 0x02,
	0x6F12, 0x0112, 0x02,
	0x6F12, 0x0112, 0x02,
	0x602A, 0x2510, 0x02,
	0x6F12, 0x0000, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x5790, 0x02,
	0x6F12, 0x0100, 0x02,
	0x6028, 0x2001, 0x02,
	0x602A, 0x2A92, 0x02,
	0x6F12, 0x1111, 0x02,
	0x602A, 0x2AA4, 0x02,
	0x6F12, 0x00A7, 0x02,
	0x6028, 0x2000, 0x02,
	0x602A, 0x11A0, 0x02,
	0x6F12, 0x0301, 0x02,
	0x602A, 0x4E22, 0x02,
	0x6F12, 0x0000, 0x02,
	0x602A, 0x122C, 0x02,
	0x6F12, 0x1230, 0x02,
	0x602A, 0x124E, 0x02,
	0x6F12, 0x0700, 0x02,
	0x6028, 0x4000, 0x02,
	0x602A, 0x080A, 0x02,
	0x6F12, 0x0100, 0x02,
	0x6000, 0x0085, 0x02,
	0x6214, 0xE9F0, 0x02,
	0x6218, 0xE9F0, 0x02,
};

/* 2x5-26Mhz 4-2_2880x2156 2392MHz 30fps */
const struct sensor_pll_info_compact sensor_2x5_pllinfo_A_2880x2156_30fps = {
	EXT_CLK_Mhz * 1000 * 1000, /* ext_clk */
	2392000000ULL, /* mipi_datarate */
	953000000ULL,  /* pclk  = VT pix CLK * 4(this value is different by cis) */
	0x124C, /* frame_length_lines	(0x0340) */
	0x1A80, /* line_length_pck		(0x0342) */
};

const struct sensor_pll_info_compact sensor_2x5_pllinfo_A_2880x1620_30fps = {
	EXT_CLK_Mhz * 1000 * 1000, /* ext_clk */
	2392000000ULL, /* mipi_datarate */
	953000000ULL,  /* pclk  = VT pix CLK * 4(this value is different by cis) */
	0x124C, /* frame_length_lines	(0x0340) */
	0x1A80, /* line_length_pck		(0x0342) */
};

/* 2. 2x5-26Mhz B20.2FD 2880x1332 2392MHz 30fps */
const struct sensor_pll_info_compact sensor_2x5_pllinfo_A_2880x1332_30fps = {
	EXT_CLK_Mhz * 1000 * 1000, /* ext_clk */
	2392000000ULL, /* mipi_datarate */
	953000000ULL,  /* pclk  = VT pix CLK * 4(this value is different by cis) */
	0x0D40, /* frame_length_lines	(0x0340) */
	0x2498, /* line_length_pck		(0x0342) */
};

const struct sensor_pll_info_compact sensor_2x5_pllinfo_A_2156x2156_30fps = {
	EXT_CLK_Mhz * 1000 * 1000, /* ext_clk */
	2392000000ULL, /* mipi_datarate */
	953000000ULL,  /* pclk  = VT pix CLK * 4(this value is different by cis) */
	0x124C, /* frame_length_lines	(0x0340) */
	0x1A80, /* line_length_pck		(0x0342) */
};

const struct sensor_pll_info_compact sensor_2x5_pllinfo_A_2880x2156_120fps = {
	EXT_CLK_Mhz * 1000 * 1000, /* ext_clk */
	2392000000ULL, /* mipi_datarate */
	953000000ULL,  /* pclk  = VT pix CLK * 4(this value is different by cis) */
	0x0926, /* frame_length_lines	(0x0340) */
	0x0D40, /* line_length_pck		(0x0342) */
};

/* 5. 2x5-26Mhz B2.FULL 5760x4312 2392MHz 30fps */
const struct sensor_pll_info_compact sensor_2x5_pllinfo_A_5760x4312_30fps = {
	EXT_CLK_Mhz * 1000 * 1000, /* ext_clk */
	2392000000ULL, /* mipi_datarate */
	953000000ULL,  /* pclk  = VT pix CLK * 4(this value is different by cis) */
	0x1196, /* frame_length_lines	(0x0340) */
	0x1B90, /* line_length_pck		(0x0342) */
};

/* 6. 2x5-26Mhz B3.FULL 5760x3240 2392MHz 30fps */
const struct sensor_pll_info_compact sensor_2x5_pllinfo_A_5760x3240_30fps = {
	EXT_CLK_Mhz * 1000 * 1000, /* ext_clk */
	2392000000ULL, /* mipi_datarate */
	953000000ULL,  /* pclk  = VT pix CLK * 4(this value is different by cis) */
	0x1196, /* frame_length_lines	(0x0340) */
	0x1B90, /* line_length_pck		(0x0342) */
};

/* 7. 2x5-26Mhz B19.FULL 5760x2664 2392MHz 30fps */
const struct sensor_pll_info_compact sensor_2x5_pllinfo_A_5760x2664_30fps = {
	EXT_CLK_Mhz * 1000 * 1000, /* ext_clk */
	2392000000ULL, /* mipi_datarate */
	953000000ULL,  /* pclk  = VT pix CLK * 4(this value is different by cis) */
	0x1196, /* frame_length_lines	(0x0340) */
	0x1B90, /* line_length_pck		(0x0342) */
};

/* 8. 2x5-26Mhz B5.FULL 4312x4312 2392MHz 30fps */
const struct sensor_pll_info_compact sensor_2x5_pllinfo_A_4312x4312_30fps = {
	EXT_CLK_Mhz * 1000 * 1000, /* ext_clk */
	2392000000ULL, /* mipi_datarate */
	953000000ULL,  /* pclk  = VT pix CLK * 4(this value is different by cis) */
	0x1196, /* frame_length_lines	(0x0340) */
	0x1B90, /* line_length_pck		(0x0342) */
};

static const u32 *sensor_2x5_setfiles_A[] = {
	/* 16x12 margin */
	sensor_2x5_setfile_A_2880x2156_30fps,
	sensor_2x5_setfile_A_2880x1620_30fps,
	sensor_2x5_setfile_A_2880x1332_30fps,
	sensor_2x5_setfile_A_2156x2156_30fps,
	sensor_2x5_setfile_A_2880x2156_120fps,
	sensor_2x5_setfile_A_5760x4312_30fps,
	sensor_2x5_setfile_A_5760x3240_30fps,
	sensor_2x5_setfile_A_5760x2664_30fps,
	sensor_2x5_setfile_A_4312x4312_30fps,
};

static const u32 sensor_2x5_setfile_A_sizes[] = {
	/* 16x12 margin */
	ARRAY_SIZE(sensor_2x5_setfile_A_2880x2156_30fps),
	ARRAY_SIZE(sensor_2x5_setfile_A_2880x1620_30fps),
	ARRAY_SIZE(sensor_2x5_setfile_A_2880x1332_30fps),
	ARRAY_SIZE(sensor_2x5_setfile_A_2156x2156_30fps),
	ARRAY_SIZE(sensor_2x5_setfile_A_2880x2156_120fps),
	ARRAY_SIZE(sensor_2x5_setfile_A_5760x4312_30fps),
	ARRAY_SIZE(sensor_2x5_setfile_A_5760x3240_30fps),
	ARRAY_SIZE(sensor_2x5_setfile_A_5760x2664_30fps),
	ARRAY_SIZE(sensor_2x5_setfile_A_4312x4312_30fps),
};

static const u32 *sensor_2x5_tetra_isp_A[] = {
	sensor_2x5_setfile_A_tetra_isp_6MP,
	sensor_2x5_setfile_A_tetra_isp_24MP,
};

static const u32 sensor_2x5_tetra_isp_A_sizes[] = {
	ARRAY_SIZE(sensor_2x5_setfile_A_tetra_isp_6MP),
	ARRAY_SIZE(sensor_2x5_setfile_A_tetra_isp_24MP),
};

static const struct sensor_pll_info_compact *sensor_2x5_pllinfos_A[] = {
	/* 16x12 margin */
	&sensor_2x5_pllinfo_A_2880x2156_30fps,
	&sensor_2x5_pllinfo_A_2880x1620_30fps,
	&sensor_2x5_pllinfo_A_2880x1332_30fps,
	&sensor_2x5_pllinfo_A_2156x2156_30fps,
	&sensor_2x5_pllinfo_A_2880x2156_120fps,
	&sensor_2x5_pllinfo_A_5760x4312_30fps,
	&sensor_2x5_pllinfo_A_5760x3240_30fps,
	&sensor_2x5_pllinfo_A_5760x2664_30fps,
	&sensor_2x5_pllinfo_A_4312x4312_30fps,
};

#endif
