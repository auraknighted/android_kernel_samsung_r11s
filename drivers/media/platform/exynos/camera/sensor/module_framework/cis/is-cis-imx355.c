...
static const u32 *sensor_imx355_global;
static u32 sensor_imx355_global_size;
static const u32 **sensor_imx355_setfiles;
static const u32 *sensor_imx355_setfile_sizes;
static const struct sensor_pll_info_compact **sensor_imx355_pllinfos;
static u32 sensor_imx355_max_setfile_num;

int sensor_imx355_cis_check_rev_on_init(struct v4l2_subdev *subdev)
{
	int ret = 0;
...
static void sensor_imx355_cis_data_calculation(const struct sensor_pll_info_compact *pll_info, cis_shared_data *cis_data)
{
	u64 vt_pix_clk_hz = 0;
	...
	/* Constant values */
	cis_data->min_fine_integration_time = SENSOR_IMX355_FINE_INTEGRATION_TIME_MIN;
	cis_data->max_fine_integration_time = SENSOR_IMX355_FINE_INTEGRATION_TIME_MAX;
	cis_data->min_coarse_integration_time = SENSOR_IMX355_COARSE_INTEGRATION_TIME_MIN;
	cis_data->max_margin_coarse_integration_time = SENSOR_IMX355_COARSE_INTEGRATION_TIME_MAX_MARGIN;
}

void sensor_imx355_cis_data_calc(struct v4l2_subdev *subdev, u32 mode)
{
	struct is_cis *cis = NULL;
	...
	if (mode >= sensor_imx355_max_setfile_num) {
		err("invalid mode(%d)!!", mode);
		return;
	}

	sensor_imx355_cis_data_calculation(sensor_imx355_pllinfos[mode], cis->cis_data);
}
...
int sensor_imx355_cis_init(struct v4l2_subdev *subdev)
{
	...
	cis->cis_data->cur_width = SENSOR_IMX355_MAX_WIDTH;
	cis->cis_data->cur_height = SENSOR_IMX355_MAX_HEIGHT;
	...
	ret = sensor_imx355_cis_check_rev_on_init(subdev);
	if (ret < 0) {
		warn("Checking imx355 rev is failed during cis init");
		ret = -EINVAL;
		goto p_err;
	}
#endif

	sensor_imx355_cis_data_calculation(sensor_imx355_pllinfos[setfile_index], cis->cis_data);
...
int sensor_imx355_cis_log_status(struct v4l2_subdev *subdev)
{
	...
	sensor_cis_dump_registers(subdev, sensor_imx355_setfiles[0], sensor_imx355_setfile_sizes[0]);
...
int sensor_imx355_cis_set_global_setting(struct v4l2_subdev *subdev)
{
	...
	ret = sensor_cis_set_registers(subdev, sensor_imx355_global, sensor_imx355_global_size);
	if (ret < 0) {
...
int sensor_imx355_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	...
	ret = sensor_cis_set_registers(subdev, sensor_imx355_setfiles[mode], sensor_imx355_setfile_sizes[mode]);
	if (ret < 0) {
		err("sensor_imx355_set_registers fail!!");
		goto p_err;
	}
...
int sensor_imx355_cis_get_min_exposure_time(struct v4l2_subdev *subdev, u32 *min_expo)
{
	int ret = 0;
	struct is_cis *cis = NULL;
...
int sensor_imx355_cis_get_max_exposure_time(struct v4l2_subdev *subdev, u32 *max_expo)
{
	int ret = 0;
	struct is_cis *cis;
	cis_shared_data *cis_data;
...
int sensor_imx355_cis_get_min_analog_gain(struct v4l2_subdev *subdev, u32 *min_again)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
	...
	read_value = 0; /* imx355 again range 0(x1) ~ 480(x16) */

	cis_data->min_analog_gain[0] = read_value;

	cis_data->min_analog_gain[1] = sensor_imx355_cis_calc_again_permile(read_value);
...
int sensor_imx355_cis_get_max_analog_gain(struct v4l2_subdev *subdev, u32 *max_again)
{
	int ret = 0;
	struct is_cis *cis;
	struct i2c_client *client;
	...
	cis_data->max_analog_gain[1] = sensor_imx355_cis_calc_again_permile(read_value);
...
int sensor_imx355_cis_get_min_digital_gain(struct v4l2_subdev *subdev, u32 *min_dgain)
{
	int ret = 0;
	struct is_cis *cis;
	cis_shared_data *cis_data;
	...
	cis_data->min_digital_gain[0] = SENSOR_IMX355_MIN_DGAIN_VALUE;
	cis_data->min_digital_gain[1] = sensor_imx355_cis_calc_dgain_permile(cis_data->min_digital_gain[0]);
...
int sensor_imx355_cis_get_max_digital_gain(struct v4l2_subdev *subdev, u32 *max_dgain)
{
	int ret = 0;
	struct is_cis *cis;
	cis_shared_data *cis_data;
	...
	cis_data->max_digital_gain[0] = SENSOR_IMX355_MAX_DGAIN_VALUE;
	cis_data->max_digital_gain[1] = sensor_imx355_cis_calc_dgain_permile(cis_data->max_digital_gain[0]);
...
static struct is_cis_ops cis_ops = {
	...
	.cis_data_calculation = sensor_imx355_cis_data_calc,
	.cis_set_exposure_time = sensor_imx355_cis_set_exposure_time,
	.cis_get_min_exposure_time = sensor_imx355_cis_get_min_exposure_time,
	.cis_get_max_exposure_time = sensor_imx355_cis_get_max_exposure_time,
	...
	.cis_adjust_analog_gain = sensor_imx355_cis_adjust_analog_gain,
	.cis_set_analog_gain = sensor_imx355_cis_set_analog_gain,
	.cis_get_analog_gain = sensor_imx355_cis_get_analog_gain,
	.cis_get_min_analog_gain = sensor_imx355_cis_get_min_analog_gain,
	.cis_get_max_analog_gain = sensor_imx355_cis_get_max_analog_gain,
	.cis_set_digital_gain = sensor_imx355_cis_set_digital_gain,
	.cis_get_digital_gain = sensor_imx355_cis_get_digital_gain,
	.cis_get_min_digital_gain = sensor_imx355_cis_get_min_digital_gain,
	.cis_get_max_digital_gain = sensor_imx355_cis_get_max_digital_gain,
	.cis_compensate_gain_for_extremely_br = sensor_imx355_cis_compensate_gain_for_extremely_br,
	.cis_check_rev_on_init = sensor_imx355_cis_check_rev_on_init,
};

static int cis_imx355_probe(struct i2c_client *client,
	const struct i2c_device_id *id)
{
	...
	if (strcmp(setfile, "default") == 0 ||
		...
		sensor_imx355_global = sensor_imx355_setfile_A_Global;
		sensor_imx355_global_size = ARRAY_SIZE(sensor_imx355_setfile_A_Global);

		sensor_imx355_setfiles = sensor_imx355_setfiles_A;
		sensor_imx355_setfile_sizes = sensor_imx355_setfile_A_sizes;
		sensor_imx355_pllinfos = sensor_imx355_pllinfos_A;
		sensor_imx355_max_setfile_num = ARRAY_SIZE(sensor_imx355_setfiles_A);
	} else if (strcmp(setfile, "setB") == 0) {
		probe_info("%s setfile_B\n", __func__);
		sensor_imx355_global = sensor_imx355_setfile_B_Global;
		sensor_imx355_global_size = ARRAY_SIZE(sensor_imx355_setfile_B_Global);

		sensor_imx355_setfiles = sensor_imx355_setfiles_B;
		sensor_imx355_setfile_sizes = sensor_imx355_setfile_B_sizes;
		sensor_imx355_pllinfos = sensor_imx355_pllinfos_B;
		sensor_imx355_max_setfile_num = ARRAY_SIZE(sensor_imx355_setfiles_B);
	} else {
