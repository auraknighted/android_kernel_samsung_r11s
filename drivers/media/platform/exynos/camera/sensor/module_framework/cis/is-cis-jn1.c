...
static void sensor_jn1_set_integration_min(u32 mode, cis_shared_data *cis_data)
{
	...
	switch (mode) {
	case SENSOR_JN1_REMOSAIC_CROP_4624x3468_30FPS:
		cis_data->min_coarse_integration_time = SENSOR_JN1_COARSE_INTEGRATION_TIME_MIN_FULL;
		dbg_sensor(1, "FULL min_coarse_integration_time(%d)\n",
			cis_data->min_coarse_integration_time);
		break;
	case SENSOR_JN1_4X4BIN_FULL_4080X3060_30FPS:
		...
		break;
#if 0
	case SENSOR_JN1_2X2BIN_FULL_4000X3000_30FPS:
	case SENSOR_JN1_2X2BIN_CROP_4000X2256_30FPS:
	case SENSOR_JN1_2X2BIN_CROP_4000X1844_30FPS:
	case SENSOR_JN1_2X2BIN_CROP_3664X3000_30FPS:
	case SENSOR_JN1_2X2BIN_CROP_3000X3000_30FPS:
		cis_data->min_coarse_integration_time = SENSOR_JN1_COARSE_INTEGRATION_TIME_MIN_2SUM;
		dbg_sensor(1, "2SUM2BIN min_coarse_integration_time(%d)\n",
			cis_data->min_coarse_integration_time);
		break;
	case SENSOR_JN1_4X4BIN_FULL_2000X1500_94FPS:
	case SENSOR_JN1_4X4BIN_CROP_2000X1128_120FPS:
		cis_data->min_coarse_integration_time = SENSOR_JN1_COARSE_INTEGRATION_TIME_MIN_4SUM;
		dbg_sensor(1, "4SUM4BIN min_coarse_integration_time(%d)\n",
			cis_data->min_coarse_integration_time);
		break;
	case SENSOR_JN1_4X4BIN_CROP_1920X1080_240FPS:
		cis_data->min_coarse_integration_time = SENSOR_JN1_COARSE_INTEGRATION_TIME_MIN_4SUM_240;
		dbg_sensor(1, "4SUM4BIN 240 min_coarse_integration_time(%d)\n",
			cis_data->min_coarse_integration_time);
		break;
#endif
	default:
		err("[%s] Unsupport jn1 sensor mode\n", __func__);
		cis_data->min_coarse_integration_time = SENSOR_JN1_COARSE_INTEGRATION_TIME_MIN_2SUM;
...
int sensor_jn1_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	...
	u16 long_coarse_int = 0x4000;
	u16 short_coarse_int = 0x4000;
	u32 line_length_pck = 0x4000;
	u32 min_fine_int = 0x4000;
	u64 numerator;
	u8 lte_shifter = 0;
	ktime_t st = ktime_get();
	...
	if (is_vender_wdr_mode_on(cis_data)) {
		ret = is_sensor_write16(client, SENSOR_JN1_LONG_COARSE_INTEG_TIME_ADDR, long_coarse_int);
		if (ret < 0)
			goto p_err;
	}
...
int sensor_jn1_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	...
	u16 long_gain = 0x4000;
	u16 short_gain = 0x4000;
	u16 dgains[4] = {0};
	ktime_t st = ktime_get();
