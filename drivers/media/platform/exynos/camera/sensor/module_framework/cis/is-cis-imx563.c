...
static int sensor_imx563_cis_get_frame_id(struct v4l2_subdev *subdev, u8 *embedded_buf, u16 *frame_id)
{
	...
	if (embedded_buf[frame_id_idx-1] == SENSOR_IMX563_VALID_TAG) {
		*frame_id = embedded_buf[frame_id_idx];
		...
		err("invalid valid tag(%x)", embedded_buf[frame_id_idx-1]);
		*frame_id = 1;
	}
...
int sensor_imx563_cis_log_status(struct v4l2_subdev *subdev)
{
	...
	sensor_cis_log_status(cis, client, log_imx563,
	...
	const struct i2c_device_id *id)
{
	...
	if (mclk_freq_khz == 19200) {
...
#if 0 // need to update mipi table for 19.2mhz setfile A
		cis->mipi_sensor_mode = sensor_imx563_setfile_A_19p2_mipi_sensor_mode;
		cis->mipi_sensor_mode_size = ARRAY_SIZE(sensor_imx563_setfile_A_19p2_mipi_sensor_mode);
		verify_sensor_mode = sensor_imx563_setfile_A_19p2_verify_sensor_mode;
		verify_sensor_mode_size = ARRAY_SIZE(sensor_imx563_setfile_A_19p2_verify_sensor_mode);
#endif
#else
		if (strcmp(setfile, "default") == 0 || strcmp(setfile, "setB") == 0)
			probe_info("%s setfile_B mclk: 19.2MHz\n", __func__);
		else
			err("setfile index out of bound, take default (setfile_B mclk: 19.2MHz)");
		...
		cis->mipi_sensor_mode = sensor_imx563_setfile_B_19p2_mipi_sensor_mode;
		cis->mipi_sensor_mode_size = ARRAY_SIZE(sensor_imx563_setfile_B_19p2_mipi_sensor_mode);
		...
		if (strcmp(setfile, "default") == 0 || strcmp(setfile, "setB") == 0)
			...
			err("setfile index out of bound, take default (setfile_B mclk: 26MHz)");
