...
static u8 rta_static_data[STATIC_DATA_SIZE_RTA];
static u8 ddk_static_data[STATIC_DATA_SIZE_DDK];
...
u32 request_frame_length_line(struct is_sensor_interface *itf, u32 framelengthline)
{
	...
	if (frameDuration == 0
		...
		frameDuration = ((1 * 1000 * 1000) / device->image.framerate);
		dbg_sensor(1, "[%s] frameDuration(0 -> %d)\n", __func__, frameDuration);
	}
#ifdef CAMERA_REAR2
		...
		struct vc_buf_info_t *buf_info)
{
	...
	if (!buf_info->width || !buf_info->height) {
		info("[%d][%s][%s] Unsupported mode for vc dma buf\n", module->instance, module->sensor_name, __func__);
		return -ENODEV;
	}
