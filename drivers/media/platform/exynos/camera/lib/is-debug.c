...
static int debug_sensor;
module_param(debug_sensor, int, S_IRUGO | S_IWUSR);
...
static unsigned int dbg_draw_digit_ctrl;
...
EXPORT_SYMBOL_GPL(is_get_debug_sensor);

int is_get_debug_assert_crash(void)
{
    return debug_assert_crash;
}
...
int is_debug_probe(struct device *dev)
{
	...
	is_debug.logfile = debugfs_create_file(DEBUG_FS_LOGFILE_NAME, 0444,
	...
	if (IS_ENABLED(CONFIG_EXYNOS_MEMORY_LOGGER)) {
		...
		ret = memlog_register("IS", dev, &is_debug.memlog_desc);
		if (ret)
...
KUNIT_EXPORT_SYMBOL(is_debug_open);
...
KUNIT_EXPORT_SYMBOL(is_debug_close);
