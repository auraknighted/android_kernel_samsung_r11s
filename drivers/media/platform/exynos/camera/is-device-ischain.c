...
static void is_set_static_dvfs(struct is_device_ischain *device, bool stream_on)
{
	int ret;
		...
		if (scenario_id >= 0) {
			...
			if (stream_on) {
				minfo("[ISC:D] tbl[%d] static scenario(%d)-[%s%s]\n", device,
					...
					dvfs_ctrl->static_ctrl->scenario_nm, "FAST_LAUNCH");
...
int is_itf_stream_on(struct is_device_ischain *device)
{
	...
	is_set_static_dvfs(device, true);
