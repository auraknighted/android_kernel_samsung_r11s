...
const u32 build_identifier_integer = 0x21052018;	/*YYMMDDHH*/
...
void enable_agc_config_wbrssi(struct s610_radio *radio, bool onoff)
{
	...
	APIEBUG(radio, "%s(%d):AGC CONFIG:%08X, AGC THRESH RSSI:%08X ",
		__func__,
		onoff,
...
void fm_set_freq(struct s610_radio *radio, u32 freq, bool mix_hi)
{
	...
	if (radio->trf_on && (radio->rfchip_ver == S620_REV_0)) {
		if (is_freq_in_spur(radio->low->fm_state.freq, fm_spur_trf_init, radio->trf_on)) {
			...
			if (radio->seek_status == FM_TUNER_PRESET_MODE)
				udelay(100);
			dev_info(radio->dev, "FREQ[%06d] TRF On: DEMOD CONFIG[0x%08X]",
	...
	if (radio->dual_clk_on && (radio->rfchip_ver == S620_REV_0)) {
		if (is_freq_in_spur(radio->low->fm_state.freq, fm_dual_clk_init, radio->dual_clk_on)) {
			...
			if (radio->seek_status == FM_TUNER_PRESET_MODE)
				udelay(100);
			dev_info(radio->dev, "7.5MHz Dual Clock ON FREQ[%06d]", radio->low->fm_state.freq);
		}
	}
...
{
	radio->low->fm_state.last_ifc = if_count_device_to_host(radio,
			fmspeedy_get_reg_work(0xFFF2B0));
}
...
void s610_sig2_work(struct work_struct *work)
{
	...
	FDEBUG(radio, "%s() ,normal ifc m [%d], ifc h [%d]",
...
void s610_rds_poll_work(struct work_struct *work)
{
	...
	if (radio->rds_flag == FM_RDS_ENABLE) {
		...
		if (radio->low->fm_state.rssi < RDS_VALID_THRESHOLD) {
			RDSEBUG(radio, "RDS RSSI invalid current:[%02d] thres:[%02d]",
...
void s610_idle_poll_work(struct work_struct *work)
{
	...
	APIEBUG(radio, "DEMOD CONFIG bit[7:6:4][%02d:%02d:%02d] RSSI ADJUST[%04x]",
...
{
	dev_info(gradio->dev,
		">>> FM version: DEMOD[%04X]ANA[%04X]PMU[%04X]SCG[%04X]SPY[%04X]TOP[%04X]DS[%d]BUILD[YYMMDD:%08X]",
	...
	if (!done) {
		...
		if (radio->low->fm_config.search_conf.weak_sig) {
			if (ifc_abs < radio->low->fm_config.search_conf.weak_ifca_l) {
				dev_info(radio->dev, ">SIG2 weak good %d done:TRUE check_ok:TRUE", ifc_abs);
				done = TRUE;
				check_ok = TRUE;
			}
			else if (ifc_abs > radio->low->fm_config.search_conf.weak_ifca_h) {
				dev_info(radio->dev, ">SIG2 weak bad %d done:TRUE check_ok:FALSE", ifc_abs);
				done = TRUE;
				check_ok = FALSE;
			}
			...
			if (ifc_abs < radio->low->fm_config.search_conf.normal_ifca_l) {
				dev_info(radio->dev, ">SIG2 normal good %d done:TRUE check_ok:TRUE", ifc_abs);
				done = TRUE;
				check_ok = TRUE;
			}
			else if (ifc_abs > radio->low->fm_config.search_conf.normal_ifca_h) {
				dev_info(radio->dev, ">SIG2 normal bad %d done:TRUE check_ok:TRUE", ifc_abs);
				done = TRUE;
				check_ok = FALSE;
			}
		...
		enum fm_tuner_state_low new_state)
{
	...
	case TUNER_IDLE:
		...
		fmspeedy_set_reg_field(0xFFF2B6, 10, (0x0001 << 10), 1);
#endif /*USE_IQ_IMBAL_SMOOTH*/
#ifdef USE_FILTER_SELECT_BY_FREQ
		...
		fmspeedy_set_reg(0xFFF2B2, IF_COUNT_INT_TIME-1);
...
struct fm_band_s fm_bands_init[] = { {87500, 108000}, {76000, 90000}, {76000,
		108000}};
...
int init_low_struc(struct s610_radio *radio)
{
	...
	memcpy(&radio->low->fm_bands, &fm_bands_init,
			sizeof(struct fm_band_s)
			* (sizeof(fm_bands_init) / sizeof(struct fm_band_s)));
