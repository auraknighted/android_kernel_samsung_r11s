...
static void slsi_set_sta_forward_beacon(struct slsi_dev *sdev, struct net_device *dev)
{
	...
	if (ret < 0)
		ret = slsi_send_forward_beacon_abort_vendor_event(sdev, SLSI_FORWARD_BEACON_ABORT_REASON_SCANNING);
}
#endif
		 ...
		 struct cfg80211_connect_params *sme)
{
	...
	slsi_conn_log2us_connecting(sdev, dev, sme->ssid, bssid, sme->bssid_hint, center_freq / 2,
				    center_freq_hint / 2, pairwise, group, ndev_vif->sta.crypto.akm_suites[0],
				    sme->auth_type,
				    sme->ie, sme->ie_len);
...
exit_with_error:
	slsi_conn_log2us_connecting_fail(sdev, dev, bssid, center_freq / 2, 3);
	r = -EINVAL;
exit:
	SLSI_MUTEX_UNLOCK(ndev_vif->vif_mutex);
	SLSI_MUTEX_UNLOCK(sdev->start_stop_mutex);
