...
#ifndef SLSI_TEST_DEV
	slsi_read_ini_config_file(sdev);
#endif
...
void slsi_collect_chipset_logs(struct work_struct *work)
{
	...
	void                    *buffer = NULL;
	size_t size = slsi_get_imp_buff_size();
	size_t bytes = 0;
	int ret = 0;
	...
	bytes = scsc_log_collect_buffer(service, SCSC_LOG_CHUNK_IMP, buffer, size);

	if (bytes) {
#ifdef CONFIG_SCSC_WLBTD
		ret = wlbtd_chipset_logging(buffer, bytes);
		if (ret)
