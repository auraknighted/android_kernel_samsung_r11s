...
#define WLAN_SW_CTRL_GPIO		"wlan-clk-req-gpio"
...
static irqreturn_t cnss_sw_ctrl_wake_handler(int irq, void* plat_priv)
{
...
int cnss_init_sw_ctrl_gpio(struct cnss_plat_data *plat_priv)
{
	...
	ret = gpio_request(plat_priv->clk_req_gpio, WLAN_SW_CTRL_GPIO);
	if (ret) {
		cnss_pr_err("Failed to request SW_CTRL GPIO, err = %d\n",
				ret);
		goto out;
	}
	...
	if (ret) {
		cnss_pr_err("Failed to set GPIO direction SW_CTRL GPIO, err = %d\n",
				ret);
		goto free_gpio;
	}
	...
	ret = request_irq(plat_priv->clk_req_irq, cnss_sw_ctrl_wake_handler,
