...
static void __iomem *gic; 

static void gpio_filter_set(void)
{
 // FILTER SET
 // FIL_SEL[22], FIL_EN[23] is already set by pinctrl driver
 // We only need to set FILTER_WIDTH as 0x20. 1 cycle is 20ns. Total filter cycle is 640ns
 if(!reg) {
     cnss_pr_err("gpio_filter_set reg is NULL");
     return;
 }
 writel(readl(reg + 0x800) | (0x20 << 16), reg + 0x800);
}; 

static void gpio_pending_clear(void)
{
 u32 reg_irq_type;
 
 if(!reg || !gic) {
     cnss_pr_err("gpio_pending_clear reg/gic is NULL");
     return;
 }
 // GPIO PENDING CLEAR
 writel((0x1 << 2), reg + 0xa00);
 
 reg_irq_type = (readl(reg + 0x700) >> 0x8) & 0x7;
 
 if (reg_irq_type != 2)
   cnss_pr_err("irq type isn't FALLING type. type:0x%08x", reg_irq_type);
   
 // GPIO SET TYPE FALLING EDGE
 writel((readl(reg + 0x700) & ~(0x7 << 8)) | 0x2 << 8, reg + 0x700);

 // GIC PENDING CLEAR. write to ICPENDR1
 writel((0x1 << 2), gic + 0x284); 
...
{
  if(!reg || !gic) {
      cnss_pr_err("print_gpio_status reg/gic is NULL");
	  return;
  }
 // PRINT REGISTER
 pr_info("@@@@ : GPA0[2]_CON 0x%x, GPA0[2]_DAT 0x%x, GPA0[2]_PUD 0x%x, GPA0[2]_DRV 0x%x, WEINT_GPA0[2]_CON 0x%x, WEINT_GPA0[2]_FILTCON0 0x%x, WEINT_GPA0[2]_MASK 0x%x, WEINT_GPA0[2]_PEND 0x%x",
 (readl(reg+0x0) >> 8) & 0xf, // CON: INPUT 0x0 OUTPUT 0x1 IRQ 0xf
 (readl(reg+0x4) >> 2) & 0x1, // DAT : LOW 0x0 HIGH 0x1
 (readl(reg+0x8) >> 8) & 0xf, // PUD : DISABLE PD/PU 0x0 PULLDOWN 0x1 PULLUP 0x3 ELSE : NOT USED
 (readl(reg+0xC) >> 8) & 0xf, // DRV_STR : 1x : 0x0, 2x: 0x1, 3x: 0x2, 4x: 0x3
 (readl(reg+0x700) >> 8) & 0x7, // WEINT_CON : Low level trig 0x0, High level trig 0x1, Falling edge 0x2, Rising Edge 0x3
 (readl(reg+0x800) >> 16) & 0xff, // FILTCON : FIL_WIDTH[21:16] FIL_SEL[22], FIL_EN[23]
 (readl(reg+0x900) >> 2) & 0x1, // WEINT_MASK : Enable 0x0, Disable 0x1
 (readl(reg+0xa00) >> 2) & 0x1); // WEINT_PENDING : NOPENDING 0x0, PENDEDING 0x1

  // PRINT GIC REGISTER (check [2] bit)
  pr_info("@@@@ : GICD1 ISENABLE1 0x%08x, GICD_ISPENDR1 0x%08x, GICD_ISACTIVER1 0x%08x\n",
  readl(gic + 0x104), // ISENABLE1
  readl(gic + 0x204), // GICD_ISPENDR1
  readl(gic + 0x304)); // GICD_ISACTIVER1

 // REGISTER INFO
 // readl(reg+0x0); // GPA0_CON
 // readl(reg+0x4); // GPA0_DAT
 // readl(reg+0x8); // GPA0_PUD
 // readl(reg+0xC); // GPA0_DRV
 // readl(reg+0x700); // WEINT_GPA0_CON
 // readl(reg+0x800); // WEINT_GPA0_FILTCON0
 // readl(reg+0x900); // WEINT_GPA0_MASK
 // readl(reg+0xa00); // WEINT_GPA0_PEND
}; 
...
#endif
	plat_priv->fw_init_ready = true;
	if (plat_priv->wake_irq_disable) {
		cnss_pr_dbg("Re-enable wlan wake irq\n");
		print_gpio_status();

		enable_irq(plat_priv->wake_irq);
		enable_irq_wake(plat_priv->wake_irq);
		plat_priv->wake_irq_disable = false;
...
static int cnss_probe(struct platform_device *plat_dev)
{
    ...
    reg = ioremap(0x15850000, 0x1000); 
    gic = ioremap(0x10200000, 0x1000);
    print_gpio_status(); 
    gpio_pending_clear(); 
    print_gpio_status(); 
