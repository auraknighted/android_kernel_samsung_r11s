...
static void test_slsi_nl80211_vendor_init_deinit(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, -EOPNOTSUPP, sdev->wiphy->vendor_commands[16].doit(sdev->wiphy, NULL, NULL, NULL));
	KUNIT_EXPECT_EQ(test, -EOPNOTSUPP, sdev->wiphy->vendor_commands[17].doit(sdev->wiphy, NULL, NULL, NULL));
...
static void test_slsi_gscan_handle_scan_result(struct kunit *test)
{
	...
	sdev->bucket[2].gscan->num_scans = 20;
...
{
	struct slsi_dev *sdev = kunit_kzalloc(test, sizeof(struct slsi_dev), GFP_KERNEL);
	struct net_device *dev = kunit_kzalloc(test, sizeof(struct net_device) + sizeof(struct netdev_vif),
						     GFP_KERNEL);
...
static void test_slsi_lls_set_stats(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, -EOPNOTSUPP, slsi_lls_set_stats(wiphy, 0, 0, 0));
...
static void test_slsi_lls_get_stats(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, -EOPNOTSUPP, slsi_lls_get_stats(wiphy, 0, 0, 0));
}
...
static void test_slsi_apf_set_filter(struct kunit *test)
{
	...
	sdev->device_config.fw_apf_supported = true;
	nla_attr = kunit_nla_set_nested_attr(nla_attr, SLSI_APF_ATTR_PROGRAM_LEN, &program_len, 0, 0, &len);
	init_len = len;
	nla_attr = kunit_nla_set_nested_attr(nla_attr, SLSI_APF_ATTR_PROGRAM, &program, 0, 0, &len);
	nla_attr = (struct nlattr *)((char *)nla_attr - len + init_len);

	KUNIT_EXPECT_EQ(test, -EINVAL, slsi_apf_set_filter(wiphy, wdev, nla_attr, len));

	wdev->netdev = kunit_kzalloc(test, sizeof(struct net_device), GFP_KERNEL);
	sdev->device_config.fw_apf_supported = true;

	KUNIT_EXPECT_EQ(test, 0, slsi_apf_set_filter(wiphy, wdev, nla_attr, len));

	nla_attr = kunit_nla_set_nested_attr(nla_attr, SLSI_APF_ATTR_PROGRAM + 3, &program, 0, 0, &len);
	nla_attr = (struct nlattr *)((char *)nla_attr - len + init_len);
...
static void test_slsi_rtt_set_get(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, -ENOTSUPP, slsi_rtt_get_capabilities(wiphy, 0, 0, 0));
	KUNIT_EXPECT_EQ(test, WIFI_HAL_ERROR_NOT_SUPPORTED, slsi_rtt_set_config(wiphy, 0, 0, 0));
	KUNIT_EXPECT_EQ(test, -ENOTSUPP, slsi_rtt_cancel_config(wiphy, 0, 0, 0));
}
...
static void test_slsi_configure_nd_offload(struct kunit *test)
{
	...
	wdev->netdev = kunit_kzalloc(test, sizeof(struct net_device) + sizeof(struct netdev_vif), GFP_KERNEL);
	ndev_vif = netdev_priv(wdev->netdev);
	ndev_vif->activated = false;
...
{
	KUNIT_EXPECT_STREQ(test, "WIFI_ROAMING_SEARCH_REASON_RESERVED",
			   slsi_get_roam_reason_str(SLSI_WIFI_ROAMING_SEARCH_REASON_RESERVED));
	...
	KUNIT_EXPECT_STREQ(test, "Table Mode", slsi_get_measure_mode(2));
