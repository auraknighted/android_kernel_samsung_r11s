...
static void test_slsi_tdls_manager_indication_timeout(struct kunit *test)
{
	...
	struct delayed_work *delayed_work_instance = &t_peer->tdls_peer_ind_timeout_work;
	...
	ndev_vif->sdev->netdev[SLSI_NET_INDEX_WLAN] = t_peer->dev;
	ndev_vif->ifnum = SLSI_NET_INDEX_WLAN;
	ndev_vif->activated = true;
	ndev_vif->sta.tdls_manager.active = 1;
	vif_sta = &ndev_vif->sta;
	t_peer->manager = &vif_sta->tdls_manager;
	...
	slsi_tdls_manager_indication_timeout(&t_peer->tdls_peer_ind_timeout_work);
...
static void test_slsi_tdls_manager_connected_ind(struct kunit *test)
{
	...
	ndev_vif->sta.tdls_peer_sta_records = 4/*SLSI_TDLS_PEER_CONNECTIONS_MAX*/;
	slsi_tdls_manager_connected_ind(manager, t_peer, flow_id);
}
...
static void test_slsi_tdls_manager_active_peer_mgmt(struct kunit *test)
{
	...
	//set tdls_supported_by_fw to true
	...
	slsi_tdls_manager_active_peer_mgmt(&manager->active_peer_manager.work);
...
static void test_slsi_tdls_manager_event_tx(struct kunit *test)
{
	...
	struct netdev_vif *ndev_vif = netdev_priv(dev);
	struct tdls_manager *manager = &ndev_vif->sta.tdls_manager;
	struct sk_buff *skb = fapi_alloc(mlme_tdls_peer_ind, MLME_TDLS_PEER_IND, 0, 1);
	struct tdls_peer *t_peer;
	u8 ac = 0;
	...
	mac[2] = 0x2C;
	ether_addr_copy(ndev_vif->sta.sta_bss->bssid, mac);
	slsi_tdls_manager_event_tx(NULL, dev, skb, ac);

	ndev_vif->activated = true;
	t_peer = slsi_tdls_manager_init_peer(dev, eth_hdr(skb)->h_dest, manager);
	...
	slsi_tdls_manager_event_tx(NULL, dev, skb, ac);
	t_peer = slsi_tdls_manager_find_peer(manager, eth_hdr(skb)->h_dest, mac);
	if (t_peer) {
		hlist_del(&t_peer->hlist);
		kfree(t_peer);
	}
...
static void test_slsi_tdls_manager_event_connected(struct kunit *test)
{
	...
	slsi_tdls_manager_init_peer(dev, mac, &ndev_vif->sta.tdls_manager);
