...
static void test_slsi_rx_netdev_mlme_work(struct kunit *test)
{
	...
	slsi_wake_lock_init(NULL, &work->sdev->wlan_wl.ws, "wlan");
#else
	slsi_wake_lock_init(&work->sdev->wlan_wl, WAKE_LOCK_SUSPEND, "wlan");
#endif
...
static void test_slsi_rx_enqueue_netdev_mlme(struct kunit *test)
{
	...
	skb = fapi_alloc(mlme_twt_notify_ind, MLME_TWT_NOTIFY_IND, 0, 1);
	sdev->netdev[vif] = kunit_kzalloc(test, sizeof(struct net_device) + sizeof(struct netdev_vif), GFP_KERNEL);
...
static void test_slsi_rx_action_enqueue_netdev_mlme(struct kunit *test)
{
	...
	dev = sdev->netdev[vif] = kunit_kzalloc(test, sizeof(struct net_device) + sizeof(struct netdev_vif), GFP_KERNEL);
	...
	p2p_dev = sdev->netdev[SLSI_NET_INDEX_P2P] = kunit_kzalloc(test, sizeof(struct net_device) +
