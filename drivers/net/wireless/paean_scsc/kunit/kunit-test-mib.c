...
{
	u8 *buffer = kunit_kzalloc(test, 0x20, GFP_KERNEL);
	struct slsi_mib_data *octet_value = kunit_kzalloc(test, sizeof(struct slsi_mib_data), GFP_KERNEL);

	octet_value->dataLength = 0x11;
	octet_value->data = kunit_kzalloc(test, 0x11, GFP_KERNEL);

	KUNIT_EXPECT_EQ(test, 19, slsi_mib_encode_octet_str(&buffer, octet_value));
}
...
static void test_slsi_mib_encode(struct kunit *test)
{
	...
	value->value.u.octetValue.dataLength = 100000000;
