...
static void test_slsi_src_sink_fake_sta_start(struct kunit *test)
{
	...
	struct slsi_rtt_id_params *rtt_id_param = kunit_kzalloc(test, sizeof(struct slsi_rtt_id_params), GFP_KERNEL);
	...
	dev->ieee80211_ptr = ieee80211_ptr;
...
{
	struct slsi_dev *sdev = kunit_kzalloc(test, sizeof(struct slsi_dev), GFP_KERNEL);
	struct net_device *dev_1 = kunit_kzalloc(test, sizeof(struct net_device)
				   + sizeof(struct netdev_vif), GFP_KERNEL);
	struct net_device *dev_2 = kunit_kzalloc(test, sizeof(struct net_device)
				   + sizeof(struct netdev_vif), GFP_KERNEL);
	struct netdev_vif *ndev_vif;
	u8 queue_idx = 0;

	sdev->netdev[SLSI_NET_INDEX_WLAN] = dev_1;
	ndev_vif = netdev_priv(dev_1);
	ndev_vif->activated = 1;

	sdev->netdev[SLSI_NET_INDEX_P2P] = dev_2;
	ndev_vif = netdev_priv(dev_2);
	ndev_vif->activated = 1;
...
static void test_slsi_src_sink_cdev_ioctl_cfg(struct kunit *test)
{
	...
	ndev_vif->scan[0].scan_results->probe_resp = alloc_skb(sizeof(u8)*100, GFP_KERNEL);
