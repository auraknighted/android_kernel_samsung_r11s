...
static void test_slsi_add_to_scan_list(struct kunit *test)
{
	...
	struct ieee80211_mgmt *mgmt;
	struct slsi_skb_cb *skb_cb;
	struct slsi_scan_result *scan_results;
	...
	ndev_vif->ifnum = SLSI_NET_INDEX_WLAN;
	ndev_vif->scan[0].scan_results = kunit_kzalloc(test, sizeof(struct slsi_scan_result), GFP_KERNEL);
	ndev_vif->scan[0].scan_results->next = kunit_kzalloc(test, sizeof(struct slsi_scan_result), GFP_KERNEL);
	scan_results = ndev_vif->scan[0].scan_results->next;
	scan_results->next = kunit_kzalloc(test, sizeof(struct slsi_scan_result), GFP_KERNEL);

	scan_results->ssid_length = 10;
	scan_results->hidden = true;
	scan_results->band = 2;
	scan_results->rssi = 9;
	...
	strcpy(scan_results->ssid, "kunit_ssid");
	memcpy(&scan_ssid[2], scan_results->ssid, 10);

	fapi_set_s16(skb, u.mlme_scan_ind.rssi, 10);
	fapi_set_s16(skb, u.mlme_scan_ind.channel_frequency, SLSI_FREQ_BAND_2GHZ);

	SLSI_MUTEX_INIT(ndev_vif->scan_result_mutex);
	mgmt = fapi_get_mgmt(skb);
	mgmt->frame_control = cpu_to_le16(IEEE80211_FTYPE_MGMT | IEEE80211_STYPE_BEACON);
	memset(mgmt->bssid, 0x0, sizeof(u8) * ETH_ALEN);
	memcpy(ndev_vif->scan[0].scan_results, mgmt->bssid, ETH_ALEN);

	memcpy(&mgmt->u.beacon.variable, "\x00\x0c\x0e\x54\x35\x35\x30\x38\x5f\x31\x58\x5f\x46\x54", 15);

	KUNIT_EXPECT_EQ(test, 0, slsi_add_to_scan_list(sdev, ndev_vif, skb, scan_ssid, 0));

	scan_results->band = 3;
	KUNIT_EXPECT_EQ(test, 0, slsi_add_to_scan_list(sdev, ndev_vif, skb, scan_ssid, 0));

	scan_results = ndev_vif->scan[0].scan_results->next;
	scan_results->rssi = 12;
	scan_results->beacon = NULL;
	KUNIT_EXPECT_EQ(test, 0, slsi_add_to_scan_list(sdev, ndev_vif, skb, scan_ssid, 0));
...
static void test_slsi_rx_rcl_ind(struct kunit *test)
{
	...
	memcpy(skb->data + fapi_get_siglen(skb), "\x01\x08\x03\x04\x35\x35\x30\x68\x13\x31\x02\x5f\x46\x54", 15);
...
static void test_slsi_acs_scan_results(struct kunit *test)
{
	...
	mgmt = fapi_get_mgmt(ndev_vif->scan[0].scan_results->beacon);
	mgmt->frame_control = cpu_to_le16(IEEE80211_FTYPE_MGMT | IEEE80211_STYPE_BEACON);
	...
	KUNIT_EXPECT_NE(test, NULL, slsi_acs_scan_results(sdev, ndev_vif, scan_id));
}
...
static void test_slsi_rx_ma_to_mlme_delba_req(struct kunit *test)
{
	...
	dev = kunit_kzalloc(test, sizeof(struct net_device) + sizeof(struct netdev_vif), GFP_KERNEL);
	skb = kunit_kzalloc(test, sizeof(struct sk_buff), GFP_KERNEL);
	skb->data = kunit_kzalloc(test, sizeof(unsigned char)*40, GFP_KERNEL);
	skb_cb = (struct slsi_skb_cb *)skb->cb;
	skb_cb->data_length = 140;
	skb_cb->sig_length = 10;
...
static void test_slsi_rx_blocking_signals(struct kunit *test)
{
	...
	((struct fapi_signal *)skb->data)->id = cpu_to_le16(MLME_DEL_VIF_CFM);
	((struct fapi_vif_signal_header *)skb->data)->vif = cpu_to_le16(SLSI_NET_INDEX_DETECT);

	sdev->netdev[1] = dev;
	ndev_vif->sig_wait.cfm_id = MLME_DEL_VIF_CFM;
	ndev_vif->sig_wait.cfm = kzalloc(sizeof(struct sk_buff), GFP_KERNEL);
	...
	((struct fapi_signal *)skb->data)->id = cpu_to_le16(FAPI_SIG_TYPE_IND);
	((struct fapi_vif_signal_header *)skb->data)->vif = cpu_to_le16(1);
	ndev_vif->sig_wait.ind_id = FAPI_SIG_TYPE_IND;
	ndev_vif->sig_wait.ind = kzalloc(sizeof(struct sk_buff), GFP_KERNEL);
	KUNIT_EXPECT_EQ(test, 0, slsi_rx_blocking_signals(sdev, skb));
...
static void test_slsi_rx_twt_setup_info_event(struct kunit *test)
{
	...
	slsi_rx_twt_setup_info_event(NULL, NULL, skb);
...
static void test_slsi_rx_twt_teardown_indication(struct kunit *test)
{
	...
	slsi_rx_twt_teardown_indication(NULL, NULL, skb);
