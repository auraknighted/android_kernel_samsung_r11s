...
{
	struct wiphy *wiphy = kunit_kzalloc(test, sizeof(struct wiphy) + sizeof(struct slsi_dev), GFP_KERNEL);
	...
	KUNIT_EXPECT_EQ(test, -EOPNOTSUPP, (int)slsi_add_virtual_intf(wiphy, name, '0', SLSI_NET_INDEX_P2PX_SWLAN, params));

	sdev->netdev_up_count = 2;
	sdev->netdev_ap = NULL;
	apndev_vif->activated = false;
	sdev->netdev[SLSI_NET_INDEX_P2PX_SWLAN]->ieee80211_ptr = kunit_kzalloc(test, sizeof(struct wireless_dev),
									       GFP_KERNEL);
	ndev_vif = netdev_priv(sdev->netdev[SLSI_NET_INDEX_P2PX_SWLAN]);
	atomic_set(&ndev_vif->is_registered, 0);
	KUNIT_EXPECT_TRUE(test, (int)slsi_add_virtual_intf(wiphy, name, '0', SLSI_NET_INDEX_P2PX_SWLAN, params));

	KUNIT_EXPECT_EQ(test, -ENODEV, (int)slsi_add_virtual_intf(wiphy, name, '0', SLSI_NET_INDEX_AP2, params));

	kunit_kfree(test, wiphy);
	kunit_kfree(test, params);
}
...
static void test_slsi_del_virtual_intf(struct kunit *test)
{
	...
	sdev->netdev_p2p = kunit_kzalloc(test, sizeof(struct net_device) + sizeof(struct netdev_vif), GFP_KERNEL);
	memcpy(sdev->netdev_p2p->name, name, 5);

	sdev->netdev[SLSI_NET_INDEX_P2PX_SWLAN] = NULL;
...
static void test_slsi_add_key(struct kunit *test)
{
	...
	sdev->netdev[SLSI_NET_INDEX_WLAN] = kunit_kzalloc(test, sizeof(struct net_device) + sizeof(struct netdev_vif),
							  GFP_KERNEL);
	ndev = sdev->netdev[SLSI_NET_INDEX_WLAN];
	ndev_vif = netdev_priv(ndev);
	ndev_vif->activated = false;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0))
...
static void test_slsi_channel_switch(struct kunit *test)
{
	...
	sdev->netdev[SLSI_NET_INDEX_P2PX_SWLAN] = kunit_kzalloc(test, sizeof(struct net_device) +
								sizeof(struct netdev_vif), GFP_KERNEL);
	ap_dev = sdev->netdev[SLSI_NET_INDEX_P2PX_SWLAN];
...
static void test_slsi_abort_scan(struct kunit *test)
{
	struct wiphy *wiphy = kunit_kzalloc(test, sizeof(struct wiphy) + sizeof(struct slsi_dev), GFP_KERNEL);
	struct slsi_dev *sdev = SDEV_FROM_WIPHY(wiphy);
	struct wireless_dev *wdev = kunit_kzalloc(test, sizeof(struct wireless_dev) + sizeof(struct netdev_vif),
						  GFP_KERNEL);
	struct net_device *ndev;
	struct netdev_vif *ndev_vif;

	ndev = wdev->netdev = kunit_kzalloc(test, sizeof(struct net_device) + sizeof(struct netdev_vif), GFP_KERNEL);
	ndev_vif = netdev_priv(ndev);
...
static void test_slsi_scan(struct kunit *test)
{
	...
	request->wdev = kunit_kzalloc(test, sizeof(struct wireless_dev), GFP_KERNEL);
	KUNIT_EXPECT_EQ(test, -EINVAL, slsi_scan(wiphy, request));

	request->wdev->netdev = kunit_kzalloc(test, sizeof(struct net_device) + sizeof(struct netdev_vif), GFP_KERNEL);
