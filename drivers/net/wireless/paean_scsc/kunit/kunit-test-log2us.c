...
/* For memory free */
void free_log2us_ctx_list(struct slsi_dev *sdev)
{
...
static void test_slsi_get_roam_reason_for_fw_reason(struct kunit *test)
{
	int roam_reason = SLSI_WIFI_ROAMING_SEARCH_REASON_LOW_RSSI;
	KUNIT_EXPECT_EQ(test, 1, slsi_get_roam_reason_for_fw_reason(roam_reason));

	roam_reason = SLSI_WIFI_ROAMING_SEARCH_REASON_LINK_LOSS;
	KUNIT_EXPECT_EQ(test, 3, slsi_get_roam_reason_for_fw_reason(roam_reason));

	roam_reason = SLSI_WIFI_ROAMING_SEARCH_REASON_BTM_REQ;
	KUNIT_EXPECT_EQ(test, 5, slsi_get_roam_reason_for_fw_reason(roam_reason));

	roam_reason = SLSI_WIFI_ROAMING_SEARCH_REASON_CU_TRIGGER;
	KUNIT_EXPECT_EQ(test, 2, slsi_get_roam_reason_for_fw_reason(roam_reason));

	roam_reason = SLSI_WIFI_ROAMING_SEARCH_REASON_EMERGENCY;
	KUNIT_EXPECT_EQ(test, 4, slsi_get_roam_reason_for_fw_reason(roam_reason));

	roam_reason = SLSI_WIFI_ROAMING_SEARCH_REASON_IDLE;
	KUNIT_EXPECT_EQ(test, 6, slsi_get_roam_reason_for_fw_reason(roam_reason));

	roam_reason = SLSI_WIFI_ROAMING_SEARCH_REASON_WTC;
	KUNIT_EXPECT_EQ(test, 7, slsi_get_roam_reason_for_fw_reason(roam_reason));

	roam_reason = SLSI_WIFI_ROAMING_SEARCH_REASON_BT_COEX;
	KUNIT_EXPECT_EQ(test, 8, slsi_get_roam_reason_for_fw_reason(roam_reason));

	roam_reason = SLSI_WIFI_ROAMING_SEARCH_REASON_RESERVED;
	KUNIT_EXPECT_EQ(test, 0, slsi_get_roam_reason_for_fw_reason(roam_reason));
}
...
static void test_slsi_conn_log2us_roam_scan_start(struct kunit *test)
{
	...
	ndev_vif->iftype = NL80211_IFTYPE_STATION;
	slsi_conn_log2us_roam_scan_start(sdev, dev, reason, roam_rssi_val, chan_utilisation,
					 expired_timer_value, rssi_thresh, timestamp);
	free_log2us_ctx_list(sdev);
}
...
static void test_slsi_conn_log2us_deauth(struct kunit *test)
{
	...
	ndev_vif->iftype = NL80211_IFTYPE_AP;
	slsi_conn_log2us_deauth(sdev, dev, str_type, bssid, sn, status);
...
static void test_slsi_conn_log2us_disassoc(struct kunit *test)
{
	...
	ndev_vif->iftype = NL80211_IFTYPE_AP;
	slsi_conn_log2us_disassoc(sdev, dev, str_type, bssid, sn, status);
...
static struct kunit_case log2us_test_cases[] = {
	...
	KUNIT_CASE(test_slsi_get_roam_reason_for_fw_reason),
