...
int slsi_set_external_assoc_ies(struct wiphy *wiphy, struct net_device *dev,
				struct cfg80211_external_fils_assoc_params *filsie)
{
	...
	SLSI_NET_INFO(dev, "\n");
	SLSI_MUTEX_LOCK(ndev_vif->vif_mutex);
	r = slsi_mlme_set_external_assoc_ies(sdev, dev, filsie);
	SLSI_MUTEX_UNLOCK(ndev_vif->vif_mutex);
	return r;
}
#endif
...
#ifdef CONFIG_SCSC_WLAN_OCE
	.set_fils_assoc_params = slsi_set_external_assoc_ies
#endif
};
...
#endif
#ifdef CONFIG_SCSC_WLAN_OCE
	wiphy_ext_feature_set(wiphy, NL80211_EXT_FEATURE_EXTERNAL_AUTH_FILS);
