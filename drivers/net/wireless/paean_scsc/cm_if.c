...
int slsi_sm_recovery_service_start(struct slsi_dev *sdev)
{
	...
	err = scsc_mx_service_mifram_alloc(sdev->service, 4.65 * 1024 * 1024, &sdev->hip.hip_ref, 4096);
#else
	err = scsc_mx_service_mifram_alloc(sdev->service, 2.75 * 1024 * 1024, &sdev->hip.hip_ref, 4096);
#endif
	if (err) {
