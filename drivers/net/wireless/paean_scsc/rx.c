...
void slsi_rx_channel_switched_ind(struct slsi_dev *sdev, struct net_device *dev, struct sk_buff *skb)
{
	...
	ndev_vif->sta.ch_width = width;
	ndev_vif->sta.bss_cf = freq;
...
void slsi_rx_synchronised_ind(struct slsi_dev *sdev, struct net_device *dev, struct sk_buff *skb)
{
	...
	if (!sae_auth) {
		...
		if (cpu_to_be32(ndev_vif->sta.crypto.akm_suites[0]) == SLSI_KEY_MGMT_FILS_SHA256 ||
		    cpu_to_be32(ndev_vif->sta.crypto.akm_suites[0]) == SLSI_KEY_MGMT_FILS_SHA384 ||
		    cpu_to_be32(ndev_vif->sta.crypto.akm_suites[0]) == SLSI_KEY_MGMT_FT_FILS_SHA256 ||
		    cpu_to_be32(ndev_vif->sta.crypto.akm_suites[0]) == SLSI_KEY_MGMT_FT_FILS_SHA384)
...
#endif
#endif
					     const unsigned char *bssid, int status,
...
static bool slsi_rx_connect_ind_fw_result_success(struct slsi_dev *sdev, struct net_device *dev, struct sk_buff **skb,
						  ...
						  u8 **assoc_rsp_ie, int *assoc_rsp_ie_len)
{
	struct netdev_vif *ndev_vif = netdev_priv(dev);
	struct ieee80211_channel *cur_channel = NULL;
...
#endif
#endif
					 bssid, status, assoc_ie, assoc_ie_len, assoc_rsp_ie, assoc_rsp_ie_len);
#else
