...
#define REG_GPIO_SETH				(0x05)
#define GPIO_RANGE				(5)
#define GPIO_DEFAULT_NUM			(5)
...
static const char *const pmic_gpio_groups[] = {
	"gpio_mh", "gpio_m2", "gpio_m3", "gpio_m4", "gpio_m1",
};
...
static int pmic_gpio_read(struct pmic_gpio_state *state, uint8_t addr)
{
	...
	ret = s2mpu13_read_reg(state->i2c, addr, &val);
	if (ret < 0)
...
static int pmic_gpio_write(struct pmic_gpio_state *state, uint8_t addr, uint8_t val)
{
	...
	ret = s2mpu13_write_reg(state->i2c, addr, val);
	if (ret < 0)
				...
				struct seq_file *s, unsigned pin)
{
	...
	uint8_t reg_set = REG_GPIO_SETH + pin;
...
static int pmic_gpio_parse_dt(struct s2mpu13_dev *iodev, struct pmic_gpio_state *state)
{
	...
	gpio_np = of_find_node_by_name(mfd_np, "s2mpu13-gpio");
	if (!gpio_np) {
