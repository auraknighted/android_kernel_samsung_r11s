...
#if IS_ENABLED(CONFIG_DEBUG_GPIO_SETTING)
	if ((strcmp(bank->name, "gpa0") == 0) && (irqd->hwirq == 2)) { // check GPA0-2
		pr_info("Set gpa0-2 type %d reg_val %d\n", type, trig_type);
		pr_info("EINT_CON 0x%08x\n", readl(bank->eint_base + reg_con));
		WARN(type != IRQ_TYPE_EDGE_FALLING, "type must not falling type[%d]\n", type);
	};
#endif
...
#if IS_ENABLED(CONFIG_DEBUG_GPIO_SETTING)
	if ((strcmp(bank->name, "gpa0") == 0) && (irqd->hwirq == 2)) {
		pr_info("EINT_CON 0x%08x\n", readl(bank->eint_base + reg_con));
		WARN(((readl(bank->eint_base + reg_con) >> shift) & 0xf) != EXYNOS_EINT_EDGE_FALLING,
				"type isn't be set falling type\n");
	}
#endif
