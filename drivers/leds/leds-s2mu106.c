...
static int s2mu106_fled_set_mode(struct s2mu106_fled_data *fled, int chan, int mode)
{
	...
	case S2MU106_FLED_MODE_OFF:
		mask = S2MU106_CHX_FLASH_FLED_EN | S2MU106_CHX_TORCH_FLED_EN;
		bit = 0;
		break;
...
static void s2mu106_fled_init(struct s2mu106_fled_data *fled)
{
	...
	if (gpio_is_valid(fled->pdata->flash_gpio) &&
		...
		s2mu106_fled_set_mode(fled, 0, S2MU106_FLED_MODE_FLASH);
		/* CAM_TORCH_EN -> TORCH gpio mode */
		s2mu106_fled_set_mode(fled, 0, S2MU106_FLED_MODE_TORCH);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (mode == S2MU106_FLED_MODE_TORCH) {
		...
		s2mu106_fled_set_mode(g_fled_data, 1, S2MU106_FLED_MODE_TORCH);
		...
		s2mu106_fled_set_mode(g_fled_data, 1, S2MU106_FLED_MODE_FLASH);
	} else {
		...
		/* torch current set */
		s2mu106_fled_set_torch_curr(g_fled_data, 1, torch_current);
		s2mu106_fled_set_mode(g_fled_data, 1, S2MU106_FLED_MODE_OFF);
	}
