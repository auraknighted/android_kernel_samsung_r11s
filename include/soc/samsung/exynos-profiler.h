					...
					bool dyn_flag, bool st_flag)
{
	...
	if (!table) {
		pr_err("migov: failed to alloc table(min=%d max=%d cnt=%d)\n",
	...
	if (!cal_ftable || !cal_vtable) {
		pr_err("migov: failed to alloc cal-table(cal-cnt=%d)\n",
								cal_table_cnt);
		kfree(table);
		return NULL;
	}
	...
	for (cal_idx = 0; cal_idx < cal_table_cnt; cal_idx++) {
		...
		if (idx >= table_cnt) {
			pr_err("migov: table_cnt is wrong(tcnt=%d freq=%d minf=%d maxf=%d\n",
